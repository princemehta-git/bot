...
${escapeHtml(adminMessage)}`;
  ...
  if (state.step === 'await_syriatel_amount') {
    ...
    const newBalance = Number(updated.balance ?? 0);
    logTransaction({ telegramUserId: msg.from.id, type: 'withdrawal', amount: afterTax, method: 'syriatel', status: 'confirmed', taxAmount: amount - afterTax }).catch((e) => console.warn('logTransaction:', e.message));
