...
const syriatelRetryInProgress = new Set();
...
async function getAccountAvailability(entry, amountNeeded) {
  ...
  const available = Math.min(balance, remainingLimit);
  return {
    ...
    lowBalance: balResult.success && balance < amountNeeded,
    entry,
  };
}
...
/**
 * Send admin notification for failed/partial Syriatel withdrawal with retry/ignore buttons.
...
async function sendSyriatelPendingAdminNotification(pendingId, fromUser, opts) {
  ...
  const tgUsername = fromUser.username ? `@${fromUser.username}` : '—';
  const name = [fromUser.first_name, fromUser.last_name].filter(Boolean).join(' ') || '—';
  const { phone, amountNewCurrency, amountOldCurrency, amountSent, remaining, failureReason, accountDetails, taxPercent } = opts;
  const amountSentOld = Math.round((amountSent || 0) * OLD_CURRENCY_MULTIPLE);
  const remainingOld = Math.round((remaining || 0) * OLD_CURRENCY_MULTIPLE);
  const taxNote = (taxPercent && Number(taxPercent) > 0) ? `\n📉 ضريبة السحب: ${Number(taxPercent).toFixed(1)}%` : '';

  let header, body;
  if (failureReason === 'low_balance') {
    header = '⚠️ طلب سحب سيرياتيل - رصيد غير كافٍ';
    const accountLines = (accountDetails || [])
      .filter(a => a.balanceSuccess)
      .map(a => `📱 ${escapeHtml(a.entry.gsm || a.entry.number)}: <code>${formatNumber(a.balance)}</code> ل.س`)
      .join('\n');
    body = `\n💰 المبلغ المطلوب: <code>${formatNumber(amountNewCurrency)}</code> ل.س (جديدة) / <code>${formatNumber(amountOldCurrency)}</code> ل.س (قديمة)${taxNote}\n📱 رقم الاستلام: <code>${escapeHtml(phone)}</code>\n\nأرصدة الحسابات:\n${accountLines}\n\nيرجى شحن الحسابات وإعادة المحاولة.`;
  } else if (failureReason === 'limit_hit') {
    header = '⚠️ طلب سحب سيرياتيل - تم الوصول للحد اليومي';
    body = `\n💰 المبلغ المطلوب: <code>${formatNumber(amountNewCurrency)}</code> ل.س (جديدة) / <code>${formatNumber(amountOldCurrency)}</code> ل.س (قديمة)${taxNote}\n📱 رقم الاستلام: <code>${escapeHtml(phone)}</code>\n\nجميع الحسابات وصلت للحد اليومي (${formatNumber(SYRIATEL_DAILY_LIMIT_DISPLAY)} ل.س).\nيرجى إعادة المحاولة بعد الساعة 12 صباحاً.`;
  } else {
    // partial or transfer_failed
    header = amountSent > 0 ? '⚠️ سحب سيرياتيل جزئي' : '⚠️ طلب سحب سيرياتيل - فشل التحويل';
    body = `\n💰 المبلغ المطلوب: <code>${formatNumber(amountNewCurrency)}</code> ل.س (جديدة) / <code>${formatNumber(amountOldCurrency)}</code> ل.س (قديمة)${taxNote}\n📱 رقم الاستلام: <code>${escapeHtml(phone)}</code>`;
    if (amountSent > 0) {
      body += `\n\n✅ تم إرساله: <code>${formatNumber(amountSent)}</code> ل.س (جديدة) / <code>${formatNumber(amountSentOld)}</code> ل.س (قديمة)`;
      body += `\n❌ متبقي: <code>${formatNumber(remaining)}</code> ل.س (جديدة) / <code>${formatNumber(remainingOld)}</code> ل.س (قديمة)`;
    }
    body += `\n\nالسبب: ${failureReason === 'transfer_failed' ? 'فشل التحويل' : 'فشل جزئي'}`;
  }

  const msg = `${header}\n\n👤 المستخدم: ${escapeHtml(name)}\n🆔 Telegram: ${escapeHtml(tgUsername)} (<code>${fromUser.id}</code>)${body}\n\n📅 ${formatDateTimeForNotification(new Date())}`;
  const keyboard = {
    reply_markup: {
      inline_keyboard: [
        [{ text: '🔄 إعادة المحاولة', callback_data: `syr_retry_${pendingId}` }, { text: '❌ تجاهل (إعادة الرصيد)', callback_data: `syr_ignore_${pendingId}` }],
      ],
    },
  };
  try {
...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'create_account') {
      ...
      try {
        ...
        if (result.success && result.fullSuccess) {
          ...
          await bot.sendMessage(pending.telegram_user_id,
            `✅ تم إتمام عملية السحب بنجاح.\n\n💰 تم إرسال <code>${formatNumber(totalNewCurrency)}</code> ل.س (عملة جديدة) / <code>${formatNumber(totalOldCurrency)}</code> ل.س (عملة قديمة) إلى <code>${escapeHtml(pending.phone)}</code>.\n📊 رصيدك المتبقي: <code>${formatNumber(newBalance)}</code> ل.س`,
            { parse_mode: 'HTML' }
          ).catch(err => console.warn('syr_retry user notify:', err.message));
          // Edit admin message
          try {
            await bot.editMessageReplyMarkup(
              { inline_keyboard: [[{ text: '✅ تمت المعالجة بنجاح', callback_data: 'syr_done' }]] },
        ...
        if (result.partial && result.totalSent > 0) {
          ...
          // Edit admin message with new amounts
          try {
            const newText = `⚠️ سحب سيرياتيل جزئي (بعد إعادة المحاولة)\n\n👤 ID: <code>${pending.telegram_user_id}</code>\n📱 الرقم: <code>${escapeHtml(pending.phone)}</code>\n💰 المطلوب: <code>${formatNumber(Number(pending.amount_new_currency))}</code> ل.س\n✅ تم إرساله: <code>${formatNumber(newAmountSent)}</code> ل.س\n❌ متبقي: <code>${formatNumber(newRemaining)}</code> ل.س\n\n📅 ${formatDateTimeForNotification(new Date())}`;
            await bot.editMessageText(newText, {
              chat_id: query.message.chat.id,
              message_id: query.message.message_id,
              parse_mode: 'HTML',
              reply_markup: { inline_keyboard: [[{ text: '🔄 إعادة المحاولة', callback_data: `syr_retry_${pendingId}` }, { text: '❌ تجاهل (إعادة الرصيد)', callback_data: `syr_ignore_${pendingId}` }]] },
            });
          } catch (_) {}
          return;
        }

        // Still failing — show alert popup, keep buttons intact (don't edit message text to avoid losing HTML formatting)
        await bot.answerCallbackQuery(query.id, { text: `⚠️ إعادة المحاولة فشلت (${formatDateTimeForNotification(new Date())}). حاول لاحقاً.`, show_alert: true }).catch(() => {});
        return;
      } finally {
    ...
    if (data.startsWith('syr_ignore_')) {
      ...
      if (!Number.isFinite(pendingId)) return;
      // Prevent ignore while a retry is actively transferring money (could double-spend)
      if (syriatelRetryInProgress.has(pendingId)) {
        await bot.answerCallbackQuery(query.id, { text: 'جاري إعادة المحاولة حالياً. يرجى الانتظار حتى الانتهاء.' }).catch(() => {});
        return;
      }
      const pending = await getSyriatelPendingById(pendingId);
      if (!pending || (pending.status !== 'pending' && pending.status !== 'partial')) {
      ...
      try {
        ...
        try {
          await bot.editMessageReplyMarkup(
            { inline_keyboard: [[{ text: `❌ تم التجاهل — استرداد ${formatNumber(refundAmount)} ل.س`, callback_data: 'syr_done' }]] },
            { chat_id: query.message.chat.id, message_id: query.message.message_id }
          );
        } catch (_) {}
      } catch (err) {
  ...
  if (state.step === 'await_syriatel_amount') {
    ...
    try {
      ...
      if (result.success && result.fullSuccess) {
        ...
        return bot.sendMessage(
          chatId,
          `✅ تم التحويل بنجاح.\n\n💰 تم إرسال <code>${escapeHtml(formatNumber(amountToSendViaApi))}</code> ل.س (عملة جديدة) / <code>${escapeHtml(formatNumber(afterTax))}</code> ل.س (عملة قديمة) إلى <code>${escapeHtml(phone.trim())}</code>.${taxNote}\n📊 رصيدك المتبقي: <code>${escapeHtml(formatNumber(newBalance))}</code> ل.س`,
          { parse_mode: 'HTML' }
        );
      }
      ...
      // Notify user: please wait
      await bot.sendMessage(chatId, '⏳ يرجى الانتظار، يتم معالجة طلبك.', { parse_mode: 'HTML' });
