...
async function syriatelTransfer(to, amount, options = {}) {
  ...
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] syriatelTransfer START to=${to} amount=${amount} from=${fromNum || 'default'}`);
  const baseUrl = (process.env.SYRIATEL_API_BASE_URL || 'http://31.97.205.230:3009').replace(/\/$/, '');
  ...
  try {
    ...
    if (data && data.success === true) {
      if (DEBUG_LOGS) console.log(`[SyriatelCashout] syriatelTransfer SUCCESS to=${to} amount=${amount} from=${fromNum || 'default'} message=${data.message}`);
      return { success: true, message: data.message };
    }
    const errMsg = data?.message || res.statusText || 'Transfer failed';
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] syriatelTransfer FAILED to=${to} amount=${amount} from=${fromNum || 'default'} error=${errMsg} httpStatus=${res.status}`);
    return { success: false, error: errMsg };
  } catch (err) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] syriatelTransfer EXCEPTION to=${to} amount=${amount} from=${fromNum || 'default'} error=${err.message}`);
    return { success: false, error: err.message };
  }
}
...
async function syriatelTransferTryAllNumbers(to, amount) {
  if (!SYRIATEL_DEPOSIT_ENTRIES || SYRIATEL_DEPOSIT_ENTRIES.length === 0) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] TryAllNumbers to=${to} amount=${amount} — no deposit entries, using default transfer`);
    return syriatelTransfer(to, amount);
  }
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] TryAllNumbers START to=${to} amount=${amount} totalNumbers=${SYRIATEL_DEPOSIT_ENTRIES.length} numbers=[${SYRIATEL_DEPOSIT_ENTRIES.map(e => e.number).join(', ')}]`);
  ...
  for (let i = 0; i < SYRIATEL_DEPOSIT_ENTRIES.length; i++) {
    ...
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] TryAllNumbers ATTEMPT ${i + 1}/${SYRIATEL_DEPOSIT_ENTRIES.length} from=${entry.number} to=${to} amount=${amount}`);
    const result = await syriatelTransfer(to, amount, { from: entry.number, apiKey: entry.apiKey });
    if (result.success) {
      if (DEBUG_LOGS) console.log(`[SyriatelCashout] TryAllNumbers SUCCESS on attempt ${i + 1}/${SYRIATEL_DEPOSIT_ENTRIES.length} from=${entry.number}`);
      return result;
    }
    lastResult = result;
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] TryAllNumbers FAILED attempt ${i + 1}/${SYRIATEL_DEPOSIT_ENTRIES.length} from=${entry.number} error=${result.error}`);
  }
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] TryAllNumbers ALL_FAILED to=${to} amount=${amount} — tried all ${SYRIATEL_DEPOSIT_ENTRIES.length} numbers, lastError=${lastResult.error}`);
  return lastResult;
}
...
async function smartSyriatelCashout({ phone, amountNewCurrency, fromEntries }) {
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] ========== smartSyriatelCashout START phone=${phone} amount=${amountNewCurrency} mode=${SYRIATEL_CASHOUT_MODE} ==========`);

  // Phase 1: Direct transfer attempt (preserve current behavior)
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase1: trying full amount ${amountNewCurrency} from each number one by one...`);
  const directResult = await syriatelTransferTryAllNumbers(phone, amountNewCurrency);
  if (directResult.success) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase1 SUCCESS — full amount ${amountNewCurrency} sent in single transfer`);
    return { success: true, fullSuccess: true, totalSent: amountNewCurrency, remaining: 0 };
  }
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase1 FAILED — no single number could send full amount. lastError=${directResult.error}`);
  ...
  if (directResult.error && directResult.error.toLowerCase().includes('exceeded account maximum limit')) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase1 recipient_limit_exceeded — skipping Phase2 entirely`);
  ...
  if (SYRIATEL_CASHOUT_MODE === 'consolidate') {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2: mode=consolidate — delegating to consolidateSyriatelCashout`);
    return consolidateSyriatelCashout({ phone, amountNewCurrency, fromEntries });
  }

  if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2: mode=split — starting smart split analysis...`);

  // Phase 2 (split mode): Smart analysis
  if (!SYRIATEL_DEPOSIT_ENTRIES || SYRIATEL_DEPOSIT_ENTRIES.length === 0) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 ABORT — no deposit entries configured`);
  ...
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 fetching balance & daily limit for ${entries.length} accounts: [${entries.map(e => e.number).join(', ')}]`);
  ...
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 summary: amountNeeded=${amountNewCurrency} totalAvailable=${totalAvailable} accountsWithBalance=${availableAccounts.length}/${accountDetails.length} allLimitHit=${allLimitHit}`);

  // Scenario D: All accounts hit daily limit
  if (allLimitHit) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 EXIT — all accounts hit daily limit (limit=${SYRIATEL_DAILY_LIMIT}). No transfer attempted.`);
  ...
  if (totalAvailable < amountNewCurrency) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 EXIT — low_balance: totalAvailable=${totalAvailable} < amountNeeded=${amountNewCurrency}. Shortfall=${amountNewCurrency - totalAvailable}. No transfer attempted.`);
  ...
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 PLAN: ${plan.length} transfers — ${plan.map((p, i) => `#${i + 1} from=${p.entry.number} amount=${p.amountToSend}`).join(' | ')}`);
  ...
  for (let planIdx = 0; planIdx < plan.length; planIdx++) {
    ...
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 EXECUTE plan step ${planIdx + 1}/${plan.length}: from=${item.entry.number} amount=${item.amountToSend} to=${phone}`);
    ...
    if (result.success) {
      ...
      if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 plan step ${planIdx + 1} SUCCESS — sent ${item.amountToSend} from ${item.entry.number}. totalSent so far=${totalSent}/${amountNewCurrency}`);
    } else {
      if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 plan step ${planIdx + 1} FAILED — from=${item.entry.number} amount=${item.amountToSend} error=${result.error}. Trying fallback...`);
      ...
      if (result.error && result.error.toLowerCase().includes('exceeded account maximum limit')) {
        if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 recipient_limit_exceeded at plan step ${planIdx + 1} — stopping all transfers`);
      ...
      for (const fallback of sorted) {
        ...
        if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 FALLBACK: trying from=${fallback.entry.number} for failedAmount=${failedAmount}`);
        ...
        if (fallbackResult.success) {
          ...
          if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 FALLBACK SUCCESS — sent ${failedAmount} from ${fallback.entry.number}. totalSent so far=${totalSent}/${amountNewCurrency}`);
          break;
        }
        if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 FALLBACK FAILED from=${fallback.entry.number} error=${fallbackResult.error}`);
        if (fallbackResult.error && fallbackResult.error.toLowerCase().includes('exceeded account maximum limit')) {
          if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 FALLBACK recipient_limit_exceeded — stopping all transfers`);
          recipientLimitHit = true;
          break;
        }
      }
      if (!fallbackSuccess && !recipientLimitHit) {
        if (DEBUG_LOGS) console.log(`[SyriatelCashout] Phase2 all fallbacks failed for step ${planIdx + 1} amount=${failedAmount}`);
      }
  ...
  if (totalSent >= amountNewCurrency) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] ========== RESULT: FULL SUCCESS totalSent=${totalSent} ==========`);
  ...
  if (recipientLimitHit) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] ========== RESULT: FAILED recipient_limit_exceeded totalSent=${totalSent} remaining=${amountNewCurrency - totalSent} ==========`);
    if (totalSent > 0) {
  ...
  sendSyriatelTxFailBalanceAlert(accountDetails, amountNewCurrency);
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] ========== RESULT: FAILED transfer_failed totalSent=${totalSent} remaining=${amountNewCurrency - totalSent} ==========`);
  if (totalSent > 0) {
...
async function consolidateSyriatelCashout({ phone, amountNewCurrency, fromEntries }) {
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate START phone=${phone} amount=${amountNewCurrency}`);
  if (!SYRIATEL_DEPOSIT_ENTRIES || SYRIATEL_DEPOSIT_ENTRIES.length === 0) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate ABORT — no deposit entries`);
    return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'transfer_failed', accountDetails: [] };
  }

  const entries = fromEntries || SYRIATEL_DEPOSIT_ENTRIES;
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate fetching availability for ${entries.length} accounts`);
  const accountDetails = await Promise.all(entries.map(entry => getAccountAvailability(entry, amountNewCurrency)));
  ...
  if (candidates.length === 0) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate EXIT — no candidates with remaining daily limit`);
    return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'limit_hit', accountDetails };
  }

  const target = candidates[0];
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate target=${target.entry.number} balance=${target.balance} usableBalance=${target.usableBalance} remainingLimit=${target.remainingLimit} available=${target.available}`);

  // Target's daily limit must be enough to send the full amount to user
  if (target.remainingLimit < amountNewCurrency) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate EXIT — target remainingLimit=${target.remainingLimit} < amount=${amountNewCurrency}`);
    sendSyriatelTxFailBalanceAlert(accountDetails, amountNewCurrency);
    return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'limit_hit', accountDetails };
  }
  ...
  if (target.available >= amountNewCurrency) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate target can fulfill alone (available=${target.available} >= amount=${amountNewCurrency}) — sending directly`);
    const result = await syriatelTransfer(phone, amountNewCurrency, { from: target.entry.number, apiKey: target.entry.apiKey });
    if (result.success) {
      if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate direct send SUCCESS`);
      return { success: true, fullSuccess: true, totalSent: amountNewCurrency, remaining: 0, accountDetails };
    }
    if (result.error && result.error.toLowerCase().includes('exceeded account maximum limit')) {
      if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate direct send FAILED — recipient_limit_exceeded`);
      return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'recipient_limit_exceeded', accountDetails };
    }
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate direct send FAILED error=${result.error} — falling through to consolidation`);
  ...
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate rawBalanceNeeded=${rawBalanceNeeded} targetBalance=${target.balance} deficit=${deficit}`);

  if (deficit === 0) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate no deficit — trying direct send from target`);
    ...
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate direct send FAILED error=${result.error}`);
    return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'transfer_failed', accountDetails };
  }
  ...
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate PLAN: deficit=${deficit} steps=${consolidationPlan.length} — ${consolidationPlan.map((p, i) => `#${i + 1} ${p.sourceEntry.number}->${target.entry.number} amount=${p.amount}`).join(' | ')} remainingDeficit=${remainingDeficit}`);

  if (remainingDeficit > 0) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate EXIT — not enough across all accounts to cover deficit. remainingDeficit=${remainingDeficit}`);
  ...
  for (let ci = 0; ci < consolidationPlan.length; ci++) {
    ...
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate EXECUTE step ${ci + 1}/${consolidationPlan.length}: ${item.sourceEntry.number} -> ${targetReceiveAddress} amount=${item.amount}`);
    ...
    if (result.success) {
      if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate step ${ci + 1} SUCCESS — moved ${item.amount} from ${item.sourceEntry.number} to target`);
    } else {
      if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate step ${ci + 1} FAILED — from=${item.sourceEntry.number} error=${result.error}. Aborting consolidation.`);
  ...
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate FINAL SEND: from=${target.entry.number} to=${phone} amount=${amountNewCurrency}`);
  const finalResult = await syriatelTransfer(phone, amountNewCurrency, { from: target.entry.number, apiKey: target.entry.apiKey });
  if (finalResult.success) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate FINAL SEND SUCCESS — full amount sent to user`);
    return { success: true, fullSuccess: true, totalSent: amountNewCurrency, remaining: 0, accountDetails };
  }
  if (finalResult.error && finalResult.error.toLowerCase().includes('exceeded account maximum limit')) {
    if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate FINAL SEND FAILED — recipient_limit_exceeded`);
    return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'recipient_limit_exceeded', accountDetails };
  }
  if (DEBUG_LOGS) console.log(`[SyriatelCashout] consolidate FINAL SEND FAILED — error=${finalResult.error}. Money consolidated in target but user didn't receive.`);
