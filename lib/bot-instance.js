...
async function adminUserDetailMessage(telegramUserId) {
  ...
  let undistributedCount = 0, distributedCount = 0, distributedAmount = 0;
  try {
    const netDetails = await getReferralNetDetails(telegramUserId);
    for (const d of netDetails) {
      if (Number(d.net_balance || 0) > 0) undistributedCount++;
    }
  ...
  text += `💸 الأرباح المستحقة: ${undistributedCount} = ${n(commData.totalCommission)} ل.س\n`;
