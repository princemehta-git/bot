...
async function getRatesForPayment() {
  ...
  // Withdrawal min USD — always derived from exchange rate
...
async function consolidateSyriatelCashout({ phone, amountNewCurrency, fromEntries }) {
  ...
  for (const item of consolidationPlan) {
    ...
    if (result.success) {
      if (DEBUG_LOGS) debugLog('consolidateCashout: internal transfer', item.amount, 'from', item.sourceEntry.number, 'to target', target.entry.number);
    } else {
      if (DEBUG_LOGS) debugLog('consolidateCashout: FAILED internal transfer', item.amount, 'from', item.sourceEntry.number, 'error:', result.error);
...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'create_account') {
    ...
    if (data === 'admin_rates_pick_syriatel' || data === 'admin_rates_pick_shamcash' || data === 'admin_rates_pick_oxapay') {
      ...
      // ShamCash + min_deposit or max_cashout → show SYP/USD sub-choice
      if (provider === 'shamcash' && (state.ratesField === 'min_deposit_syp' || state.ratesField === 'max_cashout_syp')) {
        const usdField = state.ratesField === 'min_deposit_syp' ? 'min_deposit_usd' : 'max_cashout_usd';
...
${escapeHtml(adminMessage)}`;
  ...
  if (state.step === 'await_admin_rates_single') {
    ...
    const isUsdField = field === 'min_deposit_usd' || field === 'max_cashout_usd';
