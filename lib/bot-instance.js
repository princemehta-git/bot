 ...
 * New compact format with cashier balance from ichancy.
 * @param {Object} s - result of getAdminStats()
 * @param {string|null|undefined} agentWalletBalance - Agent wallet balance from getAgentWallet (formatted or raw), or null if unavailable
 */
function adminStatsMessage(s, agentWalletBalance = null) {
...
🎉 أرباح الأكواد: ${n(s.codeProfits)} ل.س
...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'create_account') {
    ...
    if (data === 'admin_stats') {
      ...
      const text = adminStatsMessage(stats, agentWalletBalance);
      await bot.editMessageText(text, {
        chat_id: chatId,
        message_id: messageId,
        parse_mode: 'HTML',
