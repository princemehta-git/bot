...
/** Per-account (GSM) last alerted balance. key=gsm, value=balance at last alert. Resets when balance recovers above threshold. */
const syriatelLastAlertedBalances = new Map();
 ...
 *  Uses per-account drop% logic: first alert when balance < threshold, re-alert only when balance drops by SYRIATEL_ALERT_DROP_PERCENT from last alerted balance.
 *  Resets when balance recovers above threshold. Fire-and-forget. */
function sendSyriatelLowBalanceAlert(accountDetails) {
  if (!FLAG_SYRIATEL_LOW_BALANCE_ALERT || !ALERT_CHANNEL_TRANSACTIONS) return;

  const accountsToAlert = [];
  for (const a of accountDetails) {
    if (!a.balanceSuccess) continue;
    const gsm = a.entry.gsm || a.entry.number;
    const balance = a.balance;

    // Balance OK — reset alert state for this account
    if (balance >= SYRIATEL_LOW_BALANCE_THRESHOLD) {
      syriatelLastAlertedBalances.delete(gsm);
      continue;
    }

    // Balance below threshold — decide whether to alert
    const lastAlerted = syriatelLastAlertedBalances.get(gsm);
    if (lastAlerted != null) {
      // Already alerted before — only re-alert if balance dropped by X% from last alerted balance
      if (balance >= lastAlerted) continue;
      const dropThreshold = lastAlerted * (1 - SYRIATEL_ALERT_DROP_PERCENT / 100);
      if (balance > dropThreshold) continue;
    }

    // Alert this account
    syriatelLastAlertedBalances.set(gsm, balance);
    accountsToAlert.push(a);
  }

  if (accountsToAlert.length === 0) return;
  const lines = accountsToAlert.map(a =>
