...
async function syriatelFetchHistoryAllNumbers(transferIdToFind) {
  ...
  if (failed) {
    ...
    console.log('[SyriatelVerify] historyAllNumbers FAILED firstFailureIndex=', failedIndex, 'number=', SYRIATEL_DEPOSIT_ENTRIES[failedIndex]?.number, 'error=', failed.error);
  ...
  console.log('[SyriatelVerify] historyAllNumbers SUCCESS mergedCount=', merged.length, 'transferIdFound=', transferIdToFind != null && transferIdToFind !== '' ? merged.some((t) => String(t.transactionNo || '').trim() === String(transferIdToFind).trim()) : 'n/a');
...
async function syriatelFetchTransactionAllNumbers(transferId) {
  ...
  console.log('[SyriatelVerify] transactionAllNumbers FAILED firstFailureIndex=', failedIndex, 'number=', failedIndex >= 0 ? SYRIATEL_DEPOSIT_ENTRIES[failedIndex]?.number : null, 'error=', firstError?.error);
  ...
  if (data === 'create_account') {
    ...
    if (data.startsWith('syr_retry_')) {
      ...
      try {
        ...
        const failureReasonText = result.failureReason === 'limit_hit'
