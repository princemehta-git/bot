...
📌 Game conditions: Deposit ${depositRequired} L.S. or ${referralsRequired} active referrals (within the last 24 hours) to activate daily spins.
...
function sendShamcashWithdrawalToChannel(pendingId, fromUser, botName, opts) {
  if (!ALERT_CHANNEL_TRANSACTIONS) return;
  const { currency, amountAskedDisplay, amountAfterTaxDisplay, amountToTransferDisplay, clientCode, taxPercent } = opts;
  const curLabel = currency === 'usd' ? 'USD' : 'ل.س';
  const tgUsername = fromUser.username ? `@${fromUser.username}` : '—';
  const name = [fromUser.first_name, fromUser.last_name].filter(Boolean).join(' ') || '—';
  const taxLine = (taxPercent != null && Number(taxPercent) > 0)
    ? `\n📉 بعد خصم ضريبة السحب (${Number(taxPercent).toFixed(1)}%): <code>${escapeHtml(amountAfterTaxDisplay)}</code> ${curLabel}`
    : '';
  const transferLine = currency === 'syp'
    ? `\n💸 المبلغ للتحويل (÷${OLD_CURRENCY_MULTIPLE}): <code>${escapeHtml(amountToTransferDisplay)}</code> ل.س`
    : `\n💸 المبلغ للتحويل: <code>${escapeHtml(amountToTransferDisplay)}</code> ${curLabel}`;
...
async function adminSpinPrizesMessage() {
  ...
  const lines = prizes.map((p, i) => `${i + 1}. ${escapeHtml(String(p.text))} — وزن: ${p.weight}`);
...
async function adminBoxPrizesMessage() {
  ...
  const lines = prizes.map((p, i) => `صندوق ${i + 1}: مبلغ ${formatNumber(Number(p.amount || 0))} ل.س — وزن: ${p.weight || 0}%`);
  return `🎁 جوائز الصناديق

📌 الإعدادات الحالية:
${lines.join('\n')}

الوزن = نسبة احتمال ظهور الجائزة. اختر صندوقاً للتعديل.`;
}
...
function adminBoxPrizeDetailKeyboard(index) {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: '✏️ تعديل المبلغ', callback_data: `admin_bpp_amount_${index}` }, { text: '✏️ تعديل الوزن', callback_data: `admin_bpp_weight_${index}` }],
...
async function adminManageRatesMessage() {
  const rate = await getConfigValue('EXCHANGE_RATE_SYP_PER_USD', 15000);
  const syr = await getProviderConfig('syriatel');
  const sham = await getProviderConfig('shamcash');
...
bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
  ...
  if (hasAccount && SPIN_BASE_URL) {
    try {
      ...
      const spinUrl = `${SPIN_BASE_URL}/?bot_id=${encodeURIComponent(BOT_ID)}`;
      ...
      await bot.sendMessage(chatId, '🎡 استخدم الزر أدناه لتدوير العجلة الذهبية', {
        reply_markup: {
          keyboard: [[{ text: '🎡 تدوير العجلة', web_app: { url: spinUrl } }]],
          resize_keyboard: true,
        },
...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'create_account') {
    ...
    if (data === 'admin_broadcast_send_message') {
      ...
      await bot.editMessageText('📨 أرسل الرسالة التي تريد إرسالها لجميع المستخدمين:', {
        chat_id: chatId,
        message_id: messageId,
        reply_markup: {
    ...
    if (data.startsWith('admin_spp_')) {
      ...
      if (data === 'admin_spp_add') {
        ...
        await bot.editMessageText('➕ أرسل النص والوزن (سطر واحد: <code>نص، وزن</code>). مثال: 💰 5000، 5', {
          chat_id: chatId,
          message_id: messageId,
          parse_mode: 'HTML',
      ...
      if (data === 'admin_spp_add_luck') {
        ...
        await bot.editMessageText(`➕ أرسل الوزن فقط (رقم موجب). النص سيكون: ${LUCK_PRIZE_TEXT}`, {
          chat_id: chatId,
          message_id: messageId,
          reply_markup: { inline_keyboard: [[{ text: '❌ إلغاء', callback_data: 'admin_spin_prizes' }]] },
        });
      ...
      if (editMatch) {
        ...
        if (idx >= 0 && idx < prizes.length) {
          ...
          if (isLuck) {
            ...
            await bot.editMessageText('✏️ أرسل الوزن الجديد (رقم موجب):', {
              chat_id: chatId,
              message_id: messageId,
              reply_markup: { inline_keyboard: [[{ text: '❌ إلغاء', callback_data: 'admin_spin_prizes' }]] },
            });
            ...
            await bot.editMessageText('✏️ أرسل النص والوزن (سطر واحد: <code>نص، وزن</code>):', {
              chat_id: chatId,
              message_id: messageId,
              parse_mode: 'HTML',
              reply_markup: { inline_keyboard: [[{ text: '❌ إلغاء', callback_data: 'admin_spin_prizes' }]] },
    ...
    if (data.startsWith('admin_bpp_')) {
      ...
      if (weightMatch) {
        ...
        if (idx >= 0 && idx <= 2) {
          ...
          await bot.editMessageText('✏️ أرسل الوزن (نسبة مئوية) للصندوق ' + (idx + 1) + ':',
            {
              chat_id: chatId,
              message_id: messageId,
              reply_markup: { inline_keyboard: [[{ text: '❌ إلغاء', callback_data: 'admin_box_prizes' }]] },
    ...
    if (data.startsWith('sham_reject_')) {
      ...
      await bot.sendMessage(pending.telegram_user_id, userMsg, { parse_mode: 'HTML' }).catch((err) => console.warn('sendMessage to user (reject):', err.message));
    ...
    if (data === 'withdraw_method_syriatel') {
      ...
      const msg = `🔑 الرجاء إدخال رقم الهاتف أو الرمز السري الخاص بالعميل.\nمثال رقم الهاتف: 0912345678\nمثال الرمز السري: 40123456`;
      await bot.editMessageText(msg, {
        chat_id: chatId,
        message_id: messageId,
    ...
    if (data === 'charge_method_syriatel') {
      ...
      const maxFormatted = formatNumber(syr.max_cashout_syp);
    ...
    if (data === 'charge_sham_usd') {
      ...
      const maxFormatted = rates.shamcash.charge_sham_usd_max % 1 === 0 ? String(rates.shamcash.charge_sham_usd_max) : rates.shamcash.charge_sham_usd_max.toFixed(1);
    ...
    if (data === 'charge_sham_syp') {
      ...
      const maxFormatted = formatNumber(sham.max_cashout_syp);
...
bot.on('message', async (msg) => {
  ...
  if (state.step === 'await_admin_spin_prize_add') {
    ...
    if (!parsed) {
      return bot.sendMessage(chatId, '❌ استخدم الصيغة: <code>نص الجائزة، الوزن</code> (رقم موجب للوزن). مثال: 💰 5000، 5');
    }
    try {
      const prizes = await getSpinPrizes();
      prizes.push(parsed);
  ...
  if (state.step === 'await_admin_spin_prize_add_luck') {
    ...
    if (!Number.isFinite(w) || w <= 0) {
      return bot.sendMessage(chatId, '❌ أرسل رقماً موجباً للوزن فقط.');
    }
    try {
      const prizes = await getSpinPrizes();
      ...
      await setConfigValue('spin_prizes', prizes);
      delete userState[chatId];
      await bot.sendMessage(chatId, `✅ تمت إضافة "${LUCK_PRIZE_TEXT}" بالوزن ${w}.\n\n` + (await adminSpinPrizesMessage()), { parse_mode: 'HTML', ...(await adminSpinPrizesKeyboard()) });
    } catch (err) {
      console.warn('setConfigValue spin_prizes:', err.message);
      await bot.sendMessage(chatId, '❌ حدث خطأ أثناء الحفظ.');
    }
  ...
  if (state.step === 'await_admin_spin_prize_weight') {
    ...
    const w = parseInt(text.trim(), 10);
    if (!Number.isFinite(w) || w <= 0) {
      return bot.sendMessage(chatId, '❌ أرسل رقماً موجباً للوزن.');
    }
    ...
    try {
      ...
      await setConfigValue('spin_prizes', prizes);
      delete userState[chatId];
      await bot.sendMessage(chatId, '✅ تم تحديث الوزن.\n\n' + (await adminSpinPrizesMessage()), { parse_mode: 'HTML', ...(await adminSpinPrizesKeyboard()) });
    } catch (err) {
      console.warn('setConfigValue spin_prizes:', err.message);
      await bot.sendMessage(chatId, '❌ حدث خطأ أثناء الحفظ.');
    }
  ...
  if (state.step === 'await_admin_spin_prize_edit') {
    ...
    const parsed = parseTextWeight(text.trim());
    if (!parsed) {
      return bot.sendMessage(chatId, '❌ استخدم الصيغة: <code>نص الجائزة، الوزن</code>. مثال: 💰 5000، 5');
    }
    const idx = state.prizeIndex;
    const prizes = await getSpinPrizes();
    if (idx < 0 || idx >= prizes.length) {
  ...
  if (state.step === 'await_admin_box_prize_weight') {
    ...
    const w = parseInt(text.trim(), 10);
    if (!Number.isFinite(w) || w < 0) {
      return bot.sendMessage(chatId, '❌ أرسل رقماً صحيحاً للوزن (نسبة مئوية، 0 أو أكثر).');
    }
    const idx = state.boxIndex;
    if (idx < 0 || idx > 2) {
      delete userState[chatId];
  ...
  if (state.step === 'await_admin_user_add_money') {
    ...
    try {
      ...
      const amountFormatted = formatCurrencySyp(amount);
      const userMsg = `🎉 **تم إضافة رصيد إلى حسابك**
...
${escapeHtml(adminMessage)}`;
  ...
  if (state.step === 'await_sham_usd_amount') {
    ...
    sendShamcashWithdrawalToChannel(pending.id, msg.from, botName, {
      ...
      amountAfterTaxDisplay: amountAfterTaxUsd % 1 === 0 ? String(amountAfterTaxUsd) : amountAfterTaxUsd.toFixed(2),
  ...
  if (state.step === 'await_sham_syp_amount') {
    ...
    sendShamcashWithdrawalToChannel(pending.id, msg.from, botName, {
      ...
      amountAfterTaxDisplay: formatNumber(amountAfterTaxSyp),
  ...
  if (state.step === 'await_syriatel_phone') {
    ...
    const { syriatel: syr } = await getRatesForPayment();
    const syriatelMinFormatted = formatNumber(syr.min_cashout_syp);
    const syriatelMaxFormatted = formatNumber(syr.max_cashout_syp);
  ...
  if (state.step === 'await_charge_syriatel_amount') {
    ...
    if (amount < syr.min_deposit_syp || amount > syr.max_cashout_syp) {
      return bot.sendMessage(chatId, `❌ المبلغ يجب أن يكون بين ${formatNumber(syr.min_deposit_syp)} و ${formatNumber(syr.max_cashout_syp)} ل.س`);
    }
...
• غير مخصص لك`;
  ...
  if (state.step === 'await_charge_sham_usd_amount') {
    ...
    if (amount < sham.charge_sham_usd_min || amount > sham.charge_sham_usd_max) {
      const minStr = sham.charge_sham_usd_min % 1 === 0 ? String(sham.charge_sham_usd_min) : sham.charge_sham_usd_min.toFixed(1);
      const maxStr = sham.charge_sham_usd_max % 1 === 0 ? String(sham.charge_sham_usd_max) : sham.charge_sham_usd_max.toFixed(1);
  ...
  if (state.step === 'await_charge_sham_syp_amount') {
    ...
    if (amount < sham.min_deposit_syp || amount > sham.max_cashout_syp) {
      return bot.sendMessage(chatId, `❌ المبلغ يجب أن يكون بين ${formatNumber(sham.min_deposit_syp)} و ${formatNumber(sham.max_cashout_syp)} ل.س`);
    }
