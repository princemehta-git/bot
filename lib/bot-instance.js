...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'create_account') {
    ...
    if (data === 'admin_sham_msg_cancel' && isAdminUser(query.from)) {
      ...
      const _cancelFmtDate = (d) => { try { return new Date(d).toLocaleString('ar-SY', { timeZone: _cancelTz, dateStyle: 'short', timeStyle: 'short' }); } catch (_) { return new Date(d).toLocaleString(); } };
    ...
    if (data.startsWith('admin_user_logs_') && isAdminUser(query.from)) {
      ...
      try {
        ...
        const typeLabels = {
          deposit: { syriatel: '📥 إيداع', sham_usd: '📥 إيداع', sham_syp: '📥 إيداع', balance_to_site: '💻 إيداع للموقع', site_to_balance: '💻 سحب من الموقع', spin_wheel: '🎡 ربح العجلة', box_game: '🎮 ربح الصندوق' },
          withdrawal: { syriatel: '📤 سحب', sham_usd: '📤 سحب', sham_syp: '📤 سحب', shamcash: '📤 سحب', balance_to_site: '💻 إيداع للموقع', site_to_balance: '💻 سحب من الموقع' },
        };
        let text = `━━━━━━━━━━━━━━━\n📜 سجل عمليات المستخدم\n━━━━━━━━━━━━━━━\n\n`;
        if (result.rows.length === 0) {
          ...
          for (const tx of result.rows) {
            ...
            if (tx.type === 'withdrawal' && ['sham_usd', 'sham_syp', 'shamcash'].includes(tx.method) && (tx.status === 'rejected' || tx.status === 'cancelled')) {
              label = '📤 سحب ❌ ( سحب شام كاش فاشل )❌';
            }
