  ...
  if (data === 'create_account') {
    ...
    async function completeShamAccept(pending, pendingId, query, resolvedBy, channelStatusText, skipAnswer) {
      try {
        ...
        if (!skipAnswer) await bot.answerCallbackQuery(query.id, { text: 'حدث خطأ.' }).catch(() => {});
        return false;
      }
      let userMsg;
      try {
    ...
    function handleShamAutoFailure(error, pendingId, query) {
      ...
      if (msgText.startsWith('📤 طلب سحب شام كاش')) {
        ...
        // In-bot admin panel — send a new message with error + 3 buttons
        bot.sendMessage(query.message.chat.id, `${errorLine}\n\n🆔 طلب #${pendingId}`, {
          reply_markup: failureButtons,
        }).catch(() => {});
      }
    }
