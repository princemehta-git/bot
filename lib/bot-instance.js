  ...
  if (data === 'create_account') {
    ...
    if (data.startsWith('admin_user_logs_') && isAdminUser(query.from)) {
      ...
      try {
        ...
        const typeLabels = {
          deposit: { syriatel: '📥 إيداع', sham_usd: '📥 إيداع', sham_syp: '📥 إيداع', balance_to_site: '💻 إيداع للموقع', site_to_balance: '💻 سحب من الموقع', spin_wheel: '🎡 ربح العجلة', box_game: '🎮 ربح الصندوق', admin: '👨‍💼 إيداع للحساب بواسطة الإدارة', send_balance: '🎁 استلام إهداء رصيد' },
...
${escapeHtml(adminMessage)}`;
  ...
  if (state.step === 'await_send_balance_amount') {
    ...
    // Add to recipient
    const updatedRecipient = await adjustBalance(state.recipientTelegramId, { balanceDelta: roundedAmount });
    if (!updatedRecipient) {
