...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'create_account') {
    ...
    if (data === 'admin_sham_balance' && isAdminUser(query.from)) {
      ...
      const header = `💰 رصيد شام كاش\n\n💵 الرصيد الحالي:\n${balanceText}\n\n📋 سجل طلبات السحب (شام كاش) — آخر ${history.length} طلب:\n\n`;
      const historyLines = [];
      if (history.length === 0) {
        historyLines.push('لا توجد عمليات حتى الآن.');
      } else {
        for (const p of history) {
          ...
          historyLines.push(`• ${formatDate(p.created_at)} — ${escapeHtml(botName)} — <code>${escapeHtml(p.amount_display)}</code>${curLabel} — <code>${escapeHtml(p.client_code)}</code> — ${statusLabel(p.status, p.resolved_by)}${resolved}`);
        }
      }
      const TG_MAX = 3900;
      const chunks = [];
      ...
      for (const line of historyLines) {
        ...
        if (next.length > TG_MAX && current.length > 0) {
          chunks.push(current);
          current = line + '\n';
        } else {
          current = next;
      ...
      const backKb = { inline_keyboard: [[{ text: '🔙 العودة للوحة الأدمن', callback_data: 'admin_panel' }]] };
      await bot.editMessageText(chunks[0], {
        chat_id: chatId,
        message_id: messageId,
        parse_mode: 'HTML',
        reply_markup: chunks.length === 1 ? backKb : undefined,
      });
