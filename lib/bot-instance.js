...
function alertTransaction(fromUser, type, amount, method, transferId) {
  ...
  const typeLabel = type === 'deposit' ? `إيداع ${methodLabel}` : `سحب ${methodLabel}`;
  const tgUsername = fromUser.username ? `@${fromUser.username}` : '—';
  const name = [fromUser.first_name, fromUser.last_name].filter(Boolean).join(' ') || '—';
...
${escapeHtml(adminMessage)}`;
  ...
  if (state.step === 'await_charge_syriatel_transfer_id') {
    ...
    const userIdentifier = msg.from.username ? `@${msg.from.username}` : 'N/A';
    const amountNewCurrency = formatNumber(Math.round(creditAmount / OLD_CURRENCY_MULTIPLE));
    return bot.sendMessage(
      chatId,
      `✅ إيداع ناجح (آلي)\n\n👤 المستخدم: ${escapeHtml(userName)} (<code>${msg.from.id}</code>)\n📧 المعرف: ${escapeHtml(userIdentifier)}\n💰 المبلغ بالعملة القديمة : <code>${escapeHtml(amountFormatted)}</code> ل.س\n💵 المبلغ بالعملة الجديدة : <code>${escapeHtml(amountNewCurrency)}</code> ل.س\n📲 الطريقة: سيرياتيل كاش\n🧾 رقم العملية: <code>${escapeHtml(transferId)}</code>${bonusNote}\n\n📊 رصيدك الحالي: <code>${escapeHtml(updatedBalance)}</code> ل.س`,
      { parse_mode: 'HTML' }
    );
  }
