    ...
    if (data.startsWith('syr_retry_')) {
      ...
      try {
        ...
        if (result.partial && result.totalSent > 0 && result.failureReason === 'recipient_limit_exceeded') {
          ...
          if (!refundOk) {
            // Refund failed — keep as partial so admin can retry manually
            ...
            try {
              const partialStatusLine = `حالة الطلب : معلق (جزئي) 🙏\n✅ تم إرساله: <code>${formatNumber(newAmountSent)}</code> ل.س | ❌ متبقي: <code>${formatNumber(newRemaining)}</code> ل.س\n⚠️ فشل استرداد الرصيد — يرجى المعالجة يدوياً`;
              const fromUserRLFail = await bot.getChat(pending.telegram_user_id).catch(() => ({ id: pending.telegram_user_id, first_name: '—', last_name: '', username: null }));
              const { syriatel: syrRLFail } = await getRatesForPayment().catch(() => ({ syriatel: { cashout_tax_percent: 0 } }));
              const taxPctRLFail = Number(syrRLFail?.cashout_tax_percent ?? 0);
              const rlFailAdminMsg = buildSyriatelPendingAdminMessageBody(fromUserRLFail, pending.phone, amountSyp, amountNewCurrencyTotal, taxPctRLFail, partialStatusLine);
              await bot.editMessageText(rlFailAdminMsg, {
                chat_id: query.message.chat.id,
                message_id: query.message.message_id,
                parse_mode: 'HTML',
...
${escapeHtml(adminMessage)}`;
  ...
  if (state.step === 'await_syriatel_amount') {
    ...
    try {
      ...
      if (result.partial && result.totalSent > 0 && result.failureReason === 'recipient_limit_exceeded') {
        // Partial success but recipient limit exceeded — refund remaining immediately, no admin retry needed
        ...
        if (!refundOk) {
          // Refund failed — fall through to normal partial flow so admin can handle
          ...
          const adminMsg = await sendSyriatelPendingAdminNotification(pending.id, msg.from, {
            ...
            failureReason: 'recipient_limit_exceeded', accountDetails: result.accountDetails, taxPercent: syr.cashout_tax_percent,
          });
          if (adminMsg) {
      ...
      if (result.failureReason === 'recipient_limit_exceeded') {
        ...
        if (!refunded) {
          ...
          // Fall through to normal failure path — admin will handle manually
        } else {
