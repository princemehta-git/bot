...
const {
  getUserByTelegramId, findUserForTransfer, createOrUpdateUser, adjustBalance, useSpinCredit, ensureDailySpinEligibility, moveUserToDeletedUsers,
  ...
  getGiftRedemptionsCountForUser, getUserTransactionHistory, getTotalDepositsForUser, getWithdrawalCountForUser, getSyriatelDepositCountInLastN, getUserExtendedStats, distributeSingleUserReferralCommission,
  getAdminStats, getTopUsersByNetDeposits,
  getExportDateBounds, getTransactionsForExport,
  createShamcashPendingWithdrawal, getShamcashPendingById, getShamcashPendingByUser, getAllShamcashPending, updateShamcashPendingStatus, updateShamcashChannelMessageId, getShamcashWithdrawalHistory,
...
function getTermsText(depositRequired, referralsRequired) {
  return `📜 الشروط والأحكام لاستخدام بوت Ichancy 📜
...
(أ) تعبئة رصيد بقيمة ${formatNumber(depositRequired)} ل.س أو ما يعادلها.
(ب) إحالة ${referralsRequired} مستخدمين نشطين قاموا فعلياً بالتعبئة على البوت.
...
const ALT_TERMS_TEXT = `📜 الشروط والأحكام لاستخدام بوت Ichancy 📜
...
4️⃣ نظام السحب:
يقوم البوت باقتصاص تكاليف تشغيلية كنسبة قدرها عشرة بالمئة لعمليات السحب القادمة من أرباح الموقع
و يتم تطبيق عشرون بالمئة للسحوبات التي تتجاوز عشرة ملايين .
...
⛔️ تنبيه:
أي محاولة للتحايل أو مخالفة الشروط او استخدام البكجات الرياضية بطريقة مكررة بشكل كبير او عكسي ستؤدي إلى إيقاف الحساب وتجميد الأرصدة.
...
// Back button after account created (hidden for bots listed in HIDE_BUTTON_FOR)
function successBackKeyboard() {
  const hideList = (process.env.HIDE_BUTTON_FOR || '').split(',').map(s => s.trim()).filter(Boolean);
  if (hideList.includes(BOT_ID)) {
    return {};
  }
  return {
    reply_markup: {
...
function formatDateManualList(date) {
  ...
  try {
    ...
    return d.toLocaleString('sv-SE', { timeZone: getBotTimezone(), year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }).replace(',', '');
  }
}
...
async function profileMessage(user, siteBalance = null) {
  ...
  const hideSpinFor = (process.env.HIDE_SPIN_FOR || '').split(',').map(s => s.trim()).filter(Boolean);
  const showSpin = !hideSpinFor.includes(BOT_ID);
...
📌 شروط اللعبة: إيداع ${depositRequired} ل.س أو ${referralsRequired} إحالات نشطة (خلال آخر 24س) لتفعيل اللفات اليومية.`;
      ...
      console.warn('profileMessage spin section error:', err.message);
    }
  }
...
let SUPPORT_WHATSAPP_NUMBER = '';
let SUPPORT_TELEGRAM_ENABLED = false;
let SUPPORT_WHATSAPP_ENABLED = false;
let ALERT_CHANNEL_ACCOUNTS = '';
let ALERT_CHANNEL_TRANSACTIONS = '';
let REFERRAL_PERCENTS = [5, 3, 2];

/** Notify the referral chain (up to 3 levels) when a new referral is recorded. Fire-and-forget. */
function notifyReferralChain(newUserTelegramId, ichancyLogin) {
  (async () => {
    try {
      const levelLabels = ['المستوى الأول (L1)', 'المستوى الثاني (L2)', 'المستوى الثالث (L3)'];
      let currentUserId = newUserTelegramId;
      for (let level = 0; level < 3; level++) {
        const currentUser = await getUserByTelegramId(currentUserId);
        if (!currentUser || !currentUser.referred_by) break;
        const referrerId = currentUser.referred_by;
        const message =
          `🎉 مبروك! لديك إحالة جديدة من ${levelLabels[level]}\n` +
          `🆔 معرف المستخدم: <code>${escapeHtml(String(newUserTelegramId))}</code>\n` +
          `👤 اسم المستخدم في ichancy: <code>${escapeHtml(ichancyLogin || '')}</code>\n` +
          `💰 ستحصل على أرباحك وفقاً لذلك.`;
        await bot.sendMessage(referrerId, message, { parse_mode: 'HTML' }).catch(() => {});
        currentUserId = referrerId;
      }
    } catch (err) {
      debugLog('notifyReferralChain error:', err.message);
    }
  })();
}

// Payment limits (SYP) and derived USD — filled from payment_providers + exchange_rate_syp_per_usd in loadLocalConfig
let EXCHANGE_RATE_SYP_PER_USD = 11500;
...
async function getRatesForPayment() {
  const exchangeRate = Number(await getConfigValue('EXCHANGE_RATE_SYP_PER_USD', 11500)) || 11500;
  ...
  // Withdrawal min USD — use direct DB value if configured, otherwise derive
  const shamUsdMin = sham.min_cashout_usd != null
    ? Number(sham.min_cashout_usd)
    : Math.max(1, Math.ceil(shamMinCashout / exchangeRate));
  // Withdrawal max USD — use direct DB value if configured, otherwise derive
  const shamUsdMaxRaw = sham.max_cashout_usd != null
    ? Number(sham.max_cashout_usd)
    : Math.max(1, Math.floor(shamMaxCashout / exchangeRate));
  // Deposit min USD — use direct DB value if configured, otherwise derive
  const chargeShamUsdMin = sham.min_deposit_usd != null
    ? Number(sham.min_deposit_usd)
    : Math.max(0, Math.ceil(shamMinDeposit / exchangeRate));
  // Deposit max USD — use direct max_cashout_usd if configured, otherwise derive
  const chargeShamUsdMaxRaw = sham.max_cashout_usd != null
    ? Number(sham.max_cashout_usd)
    : Math.max(0, Math.floor(shamMaxCashout / exchangeRate));
  const shamcash = {
...
/** Generate QR code image for shamCode, send as photo with caption, then delete temp file. Falls back to text-only message on failure. */
async function sendShamcashQR(chatId, caption, shamCode, extraOpts = {}) {
  const tmpFile = path.join(__dirname, `..`, `qr_${chatId}_${Date.now()}.png`);
  try {
    await QRCode.toFile(tmpFile, shamCode, { width: 300, margin: 2 });
    await bot.sendPhoto(chatId, tmpFile, { caption, parse_mode: 'HTML', ...extraOpts }, { contentType: 'image/png' });
  } catch (err) {
    console.error(`[Bot:${BOT_ID}] sendShamcashQR failed, falling back to text:`, err.message);
    await bot.sendMessage(chatId, caption, { parse_mode: 'HTML', ...extraOpts });
  } finally {
...
/** Cashout mode: 'split' (partial payments from multiple accounts) or 'consolidate' (move funds into one account, single payment). From env SYRIATEL_CASHOUT_MODE, default 'split'. */
let SYRIATEL_CASHOUT_MODE = (process.env.SYRIATEL_CASHOUT_MODE || 'split').trim().toLowerCase();
let FLAG_SYRIATEL_LOW_BALANCE_ALERT = false;
let SYRIATEL_LOW_BALANCE_THRESHOLD = 1500;
let FLAG_SYRIATEL_TX_FAIL_BALANCE_ALERT = true;
const SYRIATEL_ALERT_DROP_PERCENT = Math.max(0, Math.min(100, Number(process.env.SYRIATEL_ALERT_DROP_PERCENT) || 20));
/** Last alerted total balance across all enabled Syriatel accounts. Resets when total recovers above threshold. */
let syriatelLastAlertedTotalBalance = null;
let FLAG_ICHANCY_LOW_BALANCE_ALERT = true;
let ICHANCY_LOW_BALANCE_THRESHOLD = 20000;
let ichancyLastAlertedBalance = null; // null = no alert sent yet; number = last balance we alerted at. Resets to null when balance recovers above threshold.
const ICHANCY_ALERT_USERIDS = (process.env.ICHANCY_ALERT_USERIDS || '').split(',').map(u => u.trim()).filter(Boolean);
const ICHANCY_ALERT_DROP_PERCENT = Math.max(0, Math.min(100, Number(process.env.ICHANCY_ALERT_DROP_PERCENT) || 20));
...
const shamcashRetryInProgress = new Set(); // shared lock per pendingId — used by accept auto, retry to prevent races
...
function alertTransaction(fromUser, type, amount, method, transferId) {
  ...
  const methodLabels = { syriatel: 'سيرياتيل كاش', sham_usd: 'شام كاش', sham_syp: 'شام كاش' };
  const methodIcons = { syriatel: '📲', sham_usd: '💳', sham_syp: '💳' };
  const methodLabel = methodLabels[method] || method;
  const methodIcon = methodIcons[method] || '💳';
  const name = [fromUser.first_name, fromUser.last_name].filter(Boolean).join(' ') || '—';
  const userIdentifier = fromUser.username ? `@${fromUser.username}` : 'N/A';
  const amountNew = Math.round(amount / OLD_CURRENCY_MULTIPLE);
  const title = type === 'deposit' ? '✅ إيداع ناجح (آلي)' : '📤 سحب ناجح (آلي)';
  let msg = `${title}\n\n👤 المستخدم: ${escapeHtml(name)} (<code>${fromUser.id}</code>)\n📧 المعرف: ${escapeHtml(userIdentifier)}\n💰 المبلغ بالعملة القديمة : <code>${formatNumber(amount)}</code> ل.س\n💵 المبلغ بالعملة الجديدة : <code>${formatNumber(amountNew)}</code> ل.س\n${methodIcon} الطريقة: ${escapeHtml(methodLabel)}`;
...
function formatDateTimeForNotification(date) {
  ...
  try {
    ...
    return d.toLocaleString('sv-SE', { timeZone: getBotTimezone() });
  }
}
...
async function sendShamcashWithdrawalToChannel(pendingId, fromUser, botName, opts) {
  if (!ALERT_CHANNEL_TRANSACTIONS) return;
  ...
  let syriatelWarning = '';
  try {
    const syrCount = await getSyriatelDepositCountInLastN(fromUser.id, 3);
    if (syrCount > 0) {
      syriatelWarning = `\n\n🟥🟥🟥🟥🟥\n📣📣 محاولة احتيال 📣📣\nبناء على عمليات التعبئة الأخيرة لهذا الزبون على البوت قام بالشحن عبر سريتل وهو الآن يطلب السحب عبر شام كاش يرجى التدقيق قبل الموافقة .\n🟥🟥🟥🟥🟥`;
    }
  } catch (_) {}
  const msg = `📤 طلب سحب شام كاش\n\n👤 المستخدم: ${escapeHtml(botName || name)}\n🆔 Telegram: ${escapeHtml(tgUsername)} (<code>${fromUser.id}</code>)\n\n💰 المبلغ المطلوب: <code>${escapeHtml(amountAskedDisplay)}</code> ${curLabel}${taxLine}${transferLine}\n📋 رمز العميل (ShamCash): <code>${escapeHtml(clientCode)}</code>\n\n📅 التاريخ والوقت: ${formatDateTimeForNotification(new Date())}${syriatelWarning}\n\n⏳ في انتظار الموافقة.`;
  const keyboard = {
    inline_keyboard: [
      [{ text: '✅ قبول', callback_data: `sham_accept_${pendingId}` }, { text: '❌ رفض', callback_data: `sham_reject_${pendingId}` }],
    ],
  };
  bot.sendMessage(ALERT_CHANNEL_TRANSACTIONS, msg, { parse_mode: 'HTML', reply_markup: keyboard }).then((sent) => {
    if (sent && sent.message_id) {
      updateShamcashChannelMessageId(pendingId, sent.message_id).catch(() => {});
    }
  }).catch((err) =>
...
/** Re-apply HTML formatting to a plain-text ShamCash channel message (query.message.text strips HTML tags). */
function reApplyShamcashChannelHtml(text) {
  let html = escapeHtml(text);
  // Wrap client code after ShamCash label in <code> tags (may span two lines if user pasted with newline)
  html = html.replace(/(رمز العميل \(ShamCash\): )(\S+(?:\n\S+)?)/g, (_, prefix, code) => `${prefix}<code>${code.replace(/\n/g, '')}</code>`);
  // Wrap telegram user ID (digits inside parentheses after Telegram line) in <code> tags
  html = html.replace(/(Telegram: [^\n]*?\()(\d{5,})(\))/g, '$1<code>$2</code>$3');
  // Wrap amount values in <code> tags
  html = html.replace(/(المبلغ المطلوب: )([0-9.,]+)/g, '$1<code>$2</code>');
  html = html.replace(/(المبلغ للتحويل بالعملة الجديدة : )([0-9.,]+)/g, '$1<code>$2</code>');
  html = html.replace(/(الخصم \( للإدارة \) : )([0-9.,]+)/g, '$1<code>$2</code>');
  return html;
}

/** Notify payment channel when user cancels their own ShamCash withdrawal request — edits the original channel message to show cancellation and removes buttons. */
function sendShamcashUserRejectToChannel(pending, fromUser, botName) {
  if (!ALERT_CHANNEL_TRANSACTIONS) return;
  const curLabel = pending.currency === 'usd' ? ' USD' : ' ل.س';
  const tgUsername = fromUser.username ? `@${fromUser.username}` : '—';
  const name = [fromUser.first_name, fromUser.last_name].filter(Boolean).join(' ') || '—';
  const cancelMsg = `❌ رفض المستخدم لطلب السحب (شام كاش)\n\n👤 المستخدم: ${escapeHtml(botName || name)}\n🆔 Telegram: ${escapeHtml(tgUsername)} (<code>${fromUser.id}</code>)\n\n💰 المبلغ: <code>${escapeHtml(pending.amount_display)}</code> ${curLabel}\n📋 رمز العميل (ShamCash): <code>${escapeHtml(pending.client_code)}</code>\n\n📅 التاريخ والوقت: ${formatDateTimeForNotification(new Date())}`;
  const channelMsgId = pending.channel_message_id ? Number(pending.channel_message_id) : null;
  if (channelMsgId) {
    bot.editMessageText(cancelMsg, {
      chat_id: ALERT_CHANNEL_TRANSACTIONS,
      message_id: channelMsgId,
      parse_mode: 'HTML',
      reply_markup: { inline_keyboard: [] },
    }).catch((err) => {
      console.warn('sendShamcashUserRejectToChannel (edit):', err.message);
      // Fallback: send as new message if edit fails
      bot.sendMessage(ALERT_CHANNEL_TRANSACTIONS, cancelMsg, { parse_mode: 'HTML' }).catch(() => {});
    });
  } else {
    // No stored message ID (old pending row) — fall back to sending a new message
    bot.sendMessage(ALERT_CHANNEL_TRANSACTIONS, cancelMsg, { parse_mode: 'HTML' }).catch((err) =>
...
async function handleOxapayPaymentSuccess(trackId, invoice, userId, messageId, paymentData) {
  ...
  // Alert channel
  alertTransaction({ id: userId, username: null, first_name: '' }, 'deposit', Math.round(baseSyp * 100) / 100, 'oxapay', trackId);
}
...
async function fetchSiteBalanceForUser(user) {
  ...
  try {
    ...
    silentCheckIchancyBalance(cookies).catch(() => {});
...
/**
 * Ensure ichancy_user_id is available for a user. If missing from DB, attempt lazy resolution
 * via resolvePlayerIdWithRetry, persist to DB, and return the resolved ID.
 * @param {object} user - User record from DB (must have ichancy_login)
 * @param {string|number} telegramUserId - Telegram user ID for DB update
 * @returns {Promise<string|null>} ichancy_user_id or null if unresolvable
 */
async function ensureIchancyUserId(user, telegramUserId) {
  if (!user || !user.ichancy_login) return null;
  if (user.ichancy_user_id) return user.ichancy_user_id;

  debugLog('ensureIchancyUserId: attempting lazy resolve for', user.ichancy_login);
  try {
    const cookies = await getAgentSession();
    silentCheckIchancyBalance(cookies).catch(() => {});
    const playerId = await resolvePlayerIdWithRetry(cookies, user.ichancy_login);
    if (playerId) {
      await createOrUpdateUser(telegramUserId, { ichancy_user_id: playerId });
      debugLog('ensureIchancyUserId: resolved and saved', playerId);
      return playerId;
    }
    debugLog('ensureIchancyUserId: all retries exhausted');
    return null;
  } catch (err) {
    console.warn('ensureIchancyUserId:', err.message);
    return null;
  }
}
...
async function syriatelTransfer(to, amount, options = {}) {
  const fromNum = (options.from != null && String(options.from).trim()) ? String(options.from).trim() : null;
  console.log(`[SyriatelCashout] syriatelTransfer START to=${to} amount=${amount} from=${fromNum || 'default'}`);
  const baseUrl = (process.env.SYRIATEL_API_BASE_URL || 'http://31.97.205.230:3009').replace(/\/$/, '');
  ...
  if (fromNum) {
    url += `&from=${encodeURIComponent(fromNum)}`;
  }
  try {
    const res = await fetch(url);
    const data = await res.json();
    if (data && data.success === true) {
      console.log(`[SyriatelCashout] syriatelTransfer SUCCESS to=${to} amount=${amount} from=${fromNum || 'default'} message=${data.message}`);
      return { success: true, message: data.message };
    }
    const errMsg = data?.message || res.statusText || 'Transfer failed';
    console.log(`[SyriatelCashout] syriatelTransfer FAILED to=${to} amount=${amount} from=${fromNum || 'default'} error=${errMsg} httpStatus=${res.status}`);
    return { success: false, error: errMsg };
  } catch (err) {
    console.log(`[SyriatelCashout] syriatelTransfer EXCEPTION to=${to} amount=${amount} from=${fromNum || 'default'} error=${err.message}`);
    return { success: false, error: err.message };
  }
}
...
async function syriatelTransferTryAllNumbers(to, amount) {
  if (!SYRIATEL_DEPOSIT_ENTRIES || SYRIATEL_DEPOSIT_ENTRIES.length === 0) {
    console.log(`[SyriatelCashout] TryAllNumbers to=${to} amount=${amount} — no deposit entries, using default transfer`);
    return syriatelTransfer(to, amount);
  }
  console.log(`[SyriatelCashout] TryAllNumbers START to=${to} amount=${amount} totalNumbers=${SYRIATEL_DEPOSIT_ENTRIES.length} numbers=[${SYRIATEL_DEPOSIT_ENTRIES.map(e => e.number).join(', ')}]`);
  let lastResult = { success: false, error: 'No deposit numbers configured' };
  for (let i = 0; i < SYRIATEL_DEPOSIT_ENTRIES.length; i++) {
    const entry = SYRIATEL_DEPOSIT_ENTRIES[i];
    console.log(`[SyriatelCashout] TryAllNumbers ATTEMPT ${i + 1}/${SYRIATEL_DEPOSIT_ENTRIES.length} from=${entry.number} to=${to} amount=${amount}`);
    const result = await syriatelTransfer(to, amount, { from: entry.number, apiKey: entry.apiKey });
    if (result.success) {
      console.log(`[SyriatelCashout] TryAllNumbers SUCCESS on attempt ${i + 1}/${SYRIATEL_DEPOSIT_ENTRIES.length} from=${entry.number}`);
      return result;
    }
    lastResult = result;
    console.log(`[SyriatelCashout] TryAllNumbers FAILED attempt ${i + 1}/${SYRIATEL_DEPOSIT_ENTRIES.length} from=${entry.number} error=${result.error}`);
  }
  console.log(`[SyriatelCashout] TryAllNumbers ALL_FAILED to=${to} amount=${amount} — tried all ${SYRIATEL_DEPOSIT_ENTRIES.length} numbers, lastError=${lastResult.error}`);
  return lastResult;
}
...
/** Send low-balance alert to admin channel (proactive, based on threshold).
 *  Compares TOTAL balance of all enabled accounts against the threshold.
 *  Uses drop% logic on total: first alert when total < threshold, re-alert only when total drops by SYRIATEL_ALERT_DROP_PERCENT from last alerted total.
 *  Resets when total recovers above threshold. Fire-and-forget. */
function sendSyriatelLowBalanceAlert(accountDetails) {
  if (!FLAG_SYRIATEL_LOW_BALANCE_ALERT || !ALERT_CHANNEL_TRANSACTIONS) return;

  // Only consider accounts with successful balance fetch
  const validAccounts = accountDetails.filter(a => a.balanceSuccess);
  if (validAccounts.length === 0) return;

  const totalBalance = validAccounts.reduce((sum, a) => sum + a.balance, 0);

  // Total balance OK — reset alert state
  if (totalBalance >= SYRIATEL_LOW_BALANCE_THRESHOLD) {
    syriatelLastAlertedTotalBalance = null;
    return;
  }

  // Total below threshold — decide whether to alert
  if (syriatelLastAlertedTotalBalance != null) {
    // Already alerted before — only re-alert if total dropped by X% from last alerted total
    if (totalBalance >= syriatelLastAlertedTotalBalance) return;
    const dropThreshold = syriatelLastAlertedTotalBalance * (1 - SYRIATEL_ALERT_DROP_PERCENT / 100);
    if (totalBalance > dropThreshold) return;
  }

  // Alert — show all enabled accounts with their individual balances
  syriatelLastAlertedTotalBalance = totalBalance;
  const lines = validAccounts.map(a =>
    `📱 GSM: <code>${escapeHtml(a.entry.gsm || a.entry.number)}</code> — 💰 الرصيد: <code>${formatNumber(a.balance)}</code> ل.س`
  ).join('\n');
  const msg = `⚠️ تنبيه: رصيد منخفض في حسابات سيرياتيل\n\n${lines}\n\n💰 الرصيد الإجمالي: <code>${formatNumber(totalBalance)}</code> ل.س\n\nيرجى شحن الحسابات.`;
  bot.sendMessage(ALERT_CHANNEL_TRANSACTIONS, msg, { parse_mode: 'HTML' }).catch(err =>
...
/** Silent iChancy agent wallet balance check. Fire-and-forget, non-blocking.
 *  @param {object} cookies - agent session cookies
 *  @param {string|number} [prefetchedBalance] - if already fetched, skip the API call
 */
async function silentCheckIchancyBalance(cookies, prefetchedBalance) {
  try {
    if (!FLAG_ICHANCY_LOW_BALANCE_ALERT) return;
    if (!ALERT_CHANNEL_TRANSACTIONS && ICHANCY_ALERT_USERIDS.length === 0) return;
    let balance;
    if (prefetchedBalance != null) {
      balance = parseFloat(prefetchedBalance);
      console.log('[iChancy balance check] using prefetched balance:', balance);
    } else {
      console.log('[iChancy balance check] fetching agent wallet...');
      const walletRes = await getAgentWallet(cookies);
      if (!walletRes.success) { console.log('[iChancy balance check] getAgentWallet failed, skipping'); return; }
      balance = parseFloat(walletRes.balance);
    }
    if (!Number.isFinite(balance)) { console.log('[iChancy balance check] invalid balance value:', balance); return; }
    console.log(`[iChancy balance check] balance=${balance}, threshold=${ICHANCY_LOW_BALANCE_THRESHOLD}, lastAlerted=${ichancyLastAlertedBalance}, dropPercent=${ICHANCY_ALERT_DROP_PERCENT}%`);
    if (balance >= ICHANCY_LOW_BALANCE_THRESHOLD) {
      console.log('[iChancy balance check] balance OK, no alert needed');
      ichancyLastAlertedBalance = null; // reset when balance recovers
      return;
    }
    // Balance is below threshold — decide whether to alert
    if (ichancyLastAlertedBalance != null) {
      // Already alerted before — only re-alert if balance dropped by X% from last alerted balance
      if (balance >= ichancyLastAlertedBalance) {
        console.log(`[iChancy balance check] balance ${balance} >= last alerted ${ichancyLastAlertedBalance}, skipping`);
        return;
      }
      const dropThreshold = ichancyLastAlertedBalance * (1 - ICHANCY_ALERT_DROP_PERCENT / 100);
      console.log(`[iChancy balance check] already alerted at ${ichancyLastAlertedBalance}, re-alert if balance <= ${dropThreshold}`);
      if (balance > dropThreshold) {
        console.log('[iChancy balance check] not enough drop since last alert, skipping');
        return;
      }
      console.log(`[iChancy balance check] balance dropped ${ICHANCY_ALERT_DROP_PERCENT}%+ from last alert — re-alerting`);
    } else {
      console.log('[iChancy balance check] first alert — balance below threshold');
    }
    ichancyLastAlertedBalance = balance;
    console.log('[iChancy balance check] TRIGGERING alert — balance below threshold');
    const msg = `⚠️ تنبيه: رصيد منخفض في حساب وكيل iChancy\n\n💰 الرصيد الحالي: <code>${formatNumber(balance)}</code> ل.س\n📉 حد التنبيه: <code>${formatNumber(ICHANCY_LOW_BALANCE_THRESHOLD)}</code> ل.س\n\nيرجى شحن حساب الوكيل.`;
    // Send to channel (if configured)
    if (ALERT_CHANNEL_TRANSACTIONS) {
      console.log(`[iChancy balance check] sending to channel ${ALERT_CHANNEL_TRANSACTIONS}`);
      bot.sendMessage(ALERT_CHANNEL_TRANSACTIONS, msg, { parse_mode: 'HTML' }).catch(err =>
        console.warn('silentCheckIchancyBalance channel:', err.message)
      );
    }
    // DM support user IDs from .env
    for (const uid of ICHANCY_ALERT_USERIDS) {
      console.log(`[iChancy balance check] sending DM to user ${uid}`);
      bot.sendMessage(uid, msg, { parse_mode: 'HTML' }).catch(err =>
        console.warn(`silentCheckIchancyBalance DM ${uid}:`, err.message)
      );
    }
  } catch (err) {
    console.warn('silentCheckIchancyBalance:', err.message);
  }
}
...
async function smartSyriatelCashout({ phone, amountNewCurrency, fromEntries }) {
  console.log(`[SyriatelCashout] ========== smartSyriatelCashout START phone=${phone} amount=${amountNewCurrency} mode=${SYRIATEL_CASHOUT_MODE} ==========`);

  // Phase 1: Direct transfer attempt (preserve current behavior)
  console.log(`[SyriatelCashout] Phase1: trying full amount ${amountNewCurrency} from each number one by one...`);
  const directResult = await syriatelTransferTryAllNumbers(phone, amountNewCurrency);
  if (directResult.success) {
    console.log(`[SyriatelCashout] Phase1 SUCCESS — full amount ${amountNewCurrency} sent in single transfer`);
    return { success: true, fullSuccess: true, totalSent: amountNewCurrency, remaining: 0 };
  }
  console.log(`[SyriatelCashout] Phase1 FAILED — no single number could send full amount. lastError=${directResult.error}`);
  ...
  if (directResult.error && directResult.error.toLowerCase().includes('exceeded account maximum limit')) {
    console.log(`[SyriatelCashout] Phase1 recipient_limit_exceeded — skipping Phase2 entirely`);
  ...
  // Phase 2: Mode-dependent strategy
  if (SYRIATEL_CASHOUT_MODE === 'consolidate') {
    console.log(`[SyriatelCashout] Phase2: mode=consolidate — delegating to consolidateSyriatelCashout`);
    return consolidateSyriatelCashout({ phone, amountNewCurrency, fromEntries });
  }

  console.log(`[SyriatelCashout] Phase2: mode=split — starting smart split analysis...`);

  // Phase 2 (split mode): Smart analysis
  if (!SYRIATEL_DEPOSIT_ENTRIES || SYRIATEL_DEPOSIT_ENTRIES.length === 0) {
    console.log(`[SyriatelCashout] Phase2 ABORT — no deposit entries configured`);
  ...
  console.log(`[SyriatelCashout] Phase2 fetching balance & daily limit for ${entries.length} accounts: [${entries.map(e => e.number).join(', ')}]`);
  const accountDetails = await Promise.all(entries.map(entry => getAccountAvailability(entry, amountNewCurrency)));

  // Log each account's availability details
  for (const a of accountDetails) {
    console.log(`[SyriatelCashout] Phase2 Account ${a.entry.number}: balance=${a.balance} usableBalance=${a.usableBalance} (feeReserve=${SYRIATEL_FEE_RESERVE_PERCENT}%) remainingLimit=${a.remainingLimit} available=${a.available} canFulfill=${a.canFulfill} balanceOk=${a.balanceSuccess} limitOk=${a.limitSuccess}`);
  }
  ...
  console.log(`[SyriatelCashout] Phase2 summary: amountNeeded=${amountNewCurrency} totalAvailable=${totalAvailable} accountsWithBalance=${availableAccounts.length}/${accountDetails.length} allLimitHit=${allLimitHit}`);

  // Scenario D: All accounts hit daily limit
  if (allLimitHit) {
    console.log(`[SyriatelCashout] Phase2 EXIT — all accounts hit daily limit (limit=${SYRIATEL_DAILY_LIMIT}). No transfer attempted.`);
  ...
  if (totalAvailable < amountNewCurrency) {
    console.log(`[SyriatelCashout] Phase2 EXIT — low_balance: totalAvailable=${totalAvailable} < amountNeeded=${amountNewCurrency}. Shortfall=${amountNewCurrency - totalAvailable}. No transfer attempted.`);
  ...
  console.log(`[SyriatelCashout] Phase2 PLAN: ${plan.length} transfers — ${plan.map((p, i) => `#${i + 1} from=${p.entry.number} amount=${p.amountToSend}`).join(' | ')}`);
  ...
  for (let planIdx = 0; planIdx < plan.length; planIdx++) {
    ...
    console.log(`[SyriatelCashout] Phase2 EXECUTE plan step ${planIdx + 1}/${plan.length}: from=${item.entry.number} amount=${item.amountToSend} to=${phone}`);
    ...
    if (result.success) {
      ...
      console.log(`[SyriatelCashout] Phase2 plan step ${planIdx + 1} SUCCESS — sent ${item.amountToSend} from ${item.entry.number}. totalSent so far=${totalSent}/${amountNewCurrency}`);
    } else {
      console.log(`[SyriatelCashout] Phase2 plan step ${planIdx + 1} FAILED — from=${item.entry.number} amount=${item.amountToSend} error=${result.error}. Trying fallback...`);
      ...
      if (result.error && result.error.toLowerCase().includes('exceeded account maximum limit')) {
        console.log(`[SyriatelCashout] Phase2 recipient_limit_exceeded at plan step ${planIdx + 1} — stopping all transfers`);
      ...
      for (const fallback of sorted) {
        ...
        console.log(`[SyriatelCashout] Phase2 FALLBACK: trying from=${fallback.entry.number} for failedAmount=${failedAmount}`);
        ...
        if (fallbackResult.success) {
          ...
          console.log(`[SyriatelCashout] Phase2 FALLBACK SUCCESS — sent ${failedAmount} from ${fallback.entry.number}. totalSent so far=${totalSent}/${amountNewCurrency}`);
          break;
        }
        console.log(`[SyriatelCashout] Phase2 FALLBACK FAILED from=${fallback.entry.number} error=${fallbackResult.error}`);
        if (fallbackResult.error && fallbackResult.error.toLowerCase().includes('exceeded account maximum limit')) {
          console.log(`[SyriatelCashout] Phase2 FALLBACK recipient_limit_exceeded — stopping all transfers`);
          recipientLimitHit = true;
          break;
        }
      }
      if (!fallbackSuccess && !recipientLimitHit) {
        console.log(`[SyriatelCashout] Phase2 all fallbacks failed for step ${planIdx + 1} amount=${failedAmount}`);
      }
  ...
  if (totalSent >= amountNewCurrency) {
    console.log(`[SyriatelCashout] ========== RESULT: FULL SUCCESS totalSent=${totalSent} ==========`);
  ...
  if (recipientLimitHit) {
    console.log(`[SyriatelCashout] ========== RESULT: FAILED recipient_limit_exceeded totalSent=${totalSent} remaining=${amountNewCurrency - totalSent} ==========`);
    if (totalSent > 0) {
  ...
  sendSyriatelTxFailBalanceAlert(accountDetails, amountNewCurrency);
  console.log(`[SyriatelCashout] ========== RESULT: FAILED transfer_failed totalSent=${totalSent} remaining=${amountNewCurrency - totalSent} ==========`);
  if (totalSent > 0) {
...
/**
 * Consolidate Syriatel cashout: move funds into one account first, then send a single payment to the user.
 * No partial payments — result is either full success or complete failure.
 * Returns same shape as smartSyriatelCashout: { success, fullSuccess, partial, totalSent, remaining, failureReason, accountDetails }.
 */
async function consolidateSyriatelCashout({ phone, amountNewCurrency, fromEntries }) {
  console.log(`[SyriatelCashout] consolidate START phone=${phone} amount=${amountNewCurrency}`);
  if (!SYRIATEL_DEPOSIT_ENTRIES || SYRIATEL_DEPOSIT_ENTRIES.length === 0) {
    console.log(`[SyriatelCashout] consolidate ABORT — no deposit entries`);
    return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'transfer_failed', accountDetails: [] };
  }

  const entries = fromEntries || SYRIATEL_DEPOSIT_ENTRIES;
  console.log(`[SyriatelCashout] consolidate fetching availability for ${entries.length} accounts`);
  const accountDetails = await Promise.all(entries.map(entry => getAccountAvailability(entry, amountNewCurrency)));

  for (const a of accountDetails) {
    console.log(`[SyriatelCashout] consolidate Account ${a.entry.number}: balance=${a.balance} usableBalance=${a.usableBalance} remainingLimit=${a.remainingLimit} available=${a.available}`);
  }

  // Proactive low-balance alert
  sendSyriatelLowBalanceAlert(accountDetails);

  // Filter accounts with daily limit headroom, sort by best combo (available DESC, then remainingLimit DESC)
  const candidates = accountDetails
    .filter(a => a.success && a.remainingLimit > 0)
    .sort((a, b) => {
  ...
  if (candidates.length === 0) {
    console.log(`[SyriatelCashout] consolidate EXIT — no candidates with remaining daily limit`);
    return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'limit_hit', accountDetails };
  }

  const target = candidates[0];
  console.log(`[SyriatelCashout] consolidate target=${target.entry.number} balance=${target.balance} usableBalance=${target.usableBalance} remainingLimit=${target.remainingLimit} available=${target.available}`);

  // Target's daily limit must be enough to send the full amount to user
  if (target.remainingLimit < amountNewCurrency) {
    console.log(`[SyriatelCashout] consolidate EXIT — target remainingLimit=${target.remainingLimit} < amount=${amountNewCurrency}`);
    sendSyriatelTxFailBalanceAlert(accountDetails, amountNewCurrency);
    return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'limit_hit', accountDetails };
  }

  // If target can fulfill alone — send directly
  if (target.available >= amountNewCurrency) {
    console.log(`[SyriatelCashout] consolidate target can fulfill alone (available=${target.available} >= amount=${amountNewCurrency}) — sending directly`);
    const result = await syriatelTransfer(phone, amountNewCurrency, { from: target.entry.number, apiKey: target.entry.apiKey });
    if (result.success) {
      console.log(`[SyriatelCashout] consolidate direct send SUCCESS`);
      return { success: true, fullSuccess: true, totalSent: amountNewCurrency, remaining: 0, accountDetails };
    }
    if (result.error && result.error.toLowerCase().includes('exceeded account maximum limit')) {
      console.log(`[SyriatelCashout] consolidate direct send FAILED — recipient_limit_exceeded`);
      return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'recipient_limit_exceeded', accountDetails };
    }
    console.log(`[SyriatelCashout] consolidate direct send FAILED error=${result.error} — falling through to consolidation`);
    // Direct send failed — fall through to try consolidation anyway (target might have stale balance info)
  }

  // Calculate how much raw balance the target needs to send amountNewCurrency (accounting for fee reserve on final transfer)
  const feeMultiplier = 1 - SYRIATEL_FEE_RESERVE_PERCENT / 100;
  const rawBalanceNeeded = Math.ceil(amountNewCurrency / feeMultiplier);
  const deficit = Math.max(0, rawBalanceNeeded - target.balance);
  console.log(`[SyriatelCashout] consolidate rawBalanceNeeded=${rawBalanceNeeded} targetBalance=${target.balance} deficit=${deficit}`);

  if (deficit === 0) {
    console.log(`[SyriatelCashout] consolidate no deficit — trying direct send from target`);
    // Target has enough raw balance (usableBalance might have been borderline due to rounding) — try direct send
    const result = await syriatelTransfer(phone, amountNewCurrency, { from: target.entry.number, apiKey: target.entry.apiKey });
    if (result.success) {
      console.log(`[SyriatelCashout] consolidate direct send SUCCESS`);
      return { success: true, fullSuccess: true, totalSent: amountNewCurrency, remaining: 0, accountDetails };
    ...
    console.log(`[SyriatelCashout] consolidate direct send FAILED error=${result.error}`);
    return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'transfer_failed', accountDetails };
  }

  // Build consolidation plan: pull funds from other accounts into target
  const sources = accountDetails
    .filter(a => a.success && a.available > 0 && a.entry.number !== target.entry.number)
  ...
  for (const src of sources) {
    ...
    const canSend = src.available; // already accounts for fee reserve and daily limit
    const sendAmount = Math.min(canSend, remainingDeficit);
    if (sendAmount > 0) {
      consolidationPlan.push({ sourceEntry: src.entry, amount: sendAmount });
      remainingDeficit -= sendAmount;
    }
  }

  console.log(`[SyriatelCashout] consolidate PLAN: deficit=${deficit} steps=${consolidationPlan.length} — ${consolidationPlan.map((p, i) => `#${i + 1} ${p.sourceEntry.number}->${target.entry.number} amount=${p.amount}`).join(' | ')} remainingDeficit=${remainingDeficit}`);

  if (remainingDeficit > 0) {
    console.log(`[SyriatelCashout] consolidate EXIT — not enough across all accounts to cover deficit. remainingDeficit=${remainingDeficit}`);
    // Not enough total across all accounts to cover the deficit
    sendSyriatelTxFailBalanceAlert(accountDetails, amountNewCurrency);
    return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'low_balance', accountDetails };
  }

  // Execute consolidation: transfer from each source into the target account
  const targetReceiveAddress = target.entry.gsm || target.entry.secretCode || target.entry.number;
  for (let ci = 0; ci < consolidationPlan.length; ci++) {
    const item = consolidationPlan[ci];
    console.log(`[SyriatelCashout] consolidate EXECUTE step ${ci + 1}/${consolidationPlan.length}: ${item.sourceEntry.number} -> ${targetReceiveAddress} amount=${item.amount}`);
    const result = await syriatelTransfer(targetReceiveAddress, item.amount, { from: item.sourceEntry.number, apiKey: item.sourceEntry.apiKey });
    if (result.success) {
      console.log(`[SyriatelCashout] consolidate step ${ci + 1} SUCCESS — moved ${item.amount} from ${item.sourceEntry.number} to target`);
    } else {
      console.log(`[SyriatelCashout] consolidate step ${ci + 1} FAILED — from=${item.sourceEntry.number} error=${result.error}. Aborting consolidation.`);
      // Consolidation failed — no money was sent to user, funds moved so far sit safely in target account
      return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'transfer_failed', accountDetails };
    }
  }

  // Consolidation complete — now send the full amount from target to user
  console.log(`[SyriatelCashout] consolidate FINAL SEND: from=${target.entry.number} to=${phone} amount=${amountNewCurrency}`);
  const finalResult = await syriatelTransfer(phone, amountNewCurrency, { from: target.entry.number, apiKey: target.entry.apiKey });
  if (finalResult.success) {
    console.log(`[SyriatelCashout] consolidate FINAL SEND SUCCESS — full amount sent to user`);
    return { success: true, fullSuccess: true, totalSent: amountNewCurrency, remaining: 0, accountDetails };
  }
  if (finalResult.error && finalResult.error.toLowerCase().includes('exceeded account maximum limit')) {
    console.log(`[SyriatelCashout] consolidate FINAL SEND FAILED — recipient_limit_exceeded`);
    return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'recipient_limit_exceeded', accountDetails };
  }
  console.log(`[SyriatelCashout] consolidate FINAL SEND FAILED — error=${finalResult.error}. Money consolidated in target but user didn't receive.`);
  // Final transfer failed — money is consolidated in target but user didn't receive it; admin can retry
  return { success: false, fullSuccess: false, partial: false, totalSent: 0, remaining: amountNewCurrency, failureReason: 'transfer_failed', accountDetails };
}
...
function getShamcashApiUrl(accountAddress, type) {
  ...
  if (type === 'transfer') {
    url = useUrl1
      ? (process.env.SHAMCASH_TRANSFER_API_URL || '').trim()
      : (process.env.SHAMCASH_TRANSFER_API_URL_ALT || '').trim();
    if (!url && !useUrl1) url = (process.env.SHAMCASH_TRANSFER_API_URL || '').trim();
...
/** ShamCash transfer API: send funds from bot wallet to user. Returns { success: true } or { success: false, error: string }. */
async function shamcashTransfer(senderAddress, recipientAddress, currencyId, amount, isHiddenIdentity) {
  const baseUrl = getShamcashApiUrl(senderAddress, 'transfer');
  if (!baseUrl) return { success: false, error: 'ShamCash transfer API not configured' };
  const sep = baseUrl.includes('?') ? '&' : '?';
  let url = `${baseUrl}${sep}account_address=${encodeURIComponent(senderAddress)}&to=${encodeURIComponent(recipientAddress)}&currency_id=${currencyId}&amount=${amount}`;
  if (isHiddenIdentity) url += '&is_hidden_identity=true';
  try {
    if (process.env.SHAMCASH_API_DEBUG) console.log(`[ShamCash] transfer: calling ${url.replace(/api_key=[^&]+/, 'api_key=***')}`);
    const res = await fetch(url);
    const raw = await res.text();
    if (process.env.SHAMCASH_API_DEBUG) console.log(`[ShamCash] transfer: status=${res.status}, body=${raw.slice(0, 500)}`);
    let data;
    try { data = raw ? JSON.parse(raw) : {}; } catch (_) { return { success: false, error: 'Invalid response from server' }; }
    if (data.success) return { success: true };
    return { success: false, error: String(data.error || 'Unknown error') };
  } catch (err) {
    console.error('[ShamCash] transfer error:', err.message);
    return { success: false, error: err.message };
  }
}

/** Attempt automatic ShamCash transfer for a pending withdrawal. Reads bot config, computes amount, calls shamcashTransfer(). Never throws — always returns { success, error? }. */
async function attemptShamcashAutoTransfer(pending) {
  try {
    const senderAddress = (await getConfigValue('SHAM_CASH_DEPOSIT_CODE', '')).trim();
    const isHidden = !!(await getConfigValue('IS_HIDE_SHAMCASH_IDENTITY', true));
    if (!senderAddress) return { success: false, error: 'Bot ShamCash address not configured' };
    ...
    let amount, currencyId;
    if (pending.currency === 'syp') {
      const amountAfterTax = Number(pending.amount_syp) * (1 - taxPercent / 100);
      amount = amountAfterTax / OLD_CURRENCY_MULTIPLE;
      currencyId = 2;
    } else {
      const amountUsd = Number(pending.amount_syp) / exchangeRate;
      amount = amountUsd * (1 - taxPercent / 100);
      currencyId = 1;
    }

    return await shamcashTransfer(senderAddress, pending.client_code, currencyId, amount, isHidden);
  } catch (err) {
    console.error('[ShamCash] attemptShamcashAutoTransfer error:', err.message);
    return { success: false, error: err.message };
  }
}
...
function ichancyAccountKeyboard() {
  ...
  if (FLAG_SEND_BALANCE) {
    rows.push([{ text: '🎁 إهداء رصيد', callback_data: 'send_balance' }]);
  }
  rows.push(
...
function chargeSyriatelErrorKeyboard() {
  ...
  rows.push(...buildSupportButtonsWithFallback());
...
function withdrawSyriatelErrorKeyboard() {
  const rows = [...buildSupportButtonsWithFallback()];
...
function isAdminUser(from) {
  ...
  if (DEBUG_LOGS && ADMIN_USERNAME_RAW && username) {
    debugLog('isAdminUser', { username, userId: from?.id, admins, isAdmin });
  }
  return isAdmin;
}
  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: '🔔 تنبيهات iChancy', callback_data: 'admin_ichancy_alerts' }],
        ...
        ...(process.env.HIDE_BOXES === 'true'
          ? [[{ text: '🎡 جوائز العجلة', callback_data: 'admin_spin_prizes' }]]
...
/** Admin: iChancy alert settings — message text */
function adminIchancyAlertsMessage() {
  return `🔔 إعدادات تنبيهات iChancy

• تنبيه الرصيد المنخفض: ${FLAG_ICHANCY_LOW_BALANCE_ALERT ? '✅ مفعّل' : '❌ معطّل'}
• حد التنبيه: ${formatNumber(ICHANCY_LOW_BALANCE_THRESHOLD)} ل.س

💡 ينبّه في قناة الإشعارات عندما ينخفض رصيد حساب الوكيل عن الحد المحدد.`;
}

/** Admin: iChancy alert settings — keyboard */
function adminIchancyAlertsKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: FLAG_ICHANCY_LOW_BALANCE_ALERT ? '✅ تنبيه الرصيد المنخفض' : '❌ تنبيه الرصيد المنخفض', callback_data: 'admin_ich_alert_toggle' }],
        [{ text: `💰 تعديل حد التنبيه (${formatNumber(ICHANCY_LOW_BALANCE_THRESHOLD)} ل.س)`, callback_data: 'admin_ich_alert_set_threshold' }],
        [{ text: '🔙 العودة للوحة الأدمن', callback_data: 'admin_panel' }],
      ],
    },
  };
...
async function getLuckBoxPrizes() {
  let raw = await getConfigValue('luck_box_prizes');
  // Handle case where JSON column returns as string
  if (typeof raw === 'string') {
    try { raw = JSON.parse(raw); } catch (_) { raw = null; }
  }
  if (Array.isArray(raw) && raw.length >= 3) return raw.slice(0, 3).map(p => ({ amount: Number(p.amount) || 0, weight: Number(p.weight) || 0 }));
  const base = Array.isArray(raw) && raw.length > 0 ? [...raw] : [];
  while (base.length < 3) base.push({ amount: 0, weight: 0 });
  return base.slice(0, 3).map(p => ({ amount: Number(p.amount) || 0, weight: Number(p.weight) || 0 }));
}
...
async function adminUserDetailMessage(telegramUserId) {
  ...
  const n = (v) => { const x = Number(v ?? 0); return Number.isInteger(x) ? x.toLocaleString('en-US') : x.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); };
  ...
  text += `💸 الأرباح المستحقة: ${n(commData.totalCommission)} ل.س\n`;
  text += `💰 الأرباح الموزعة: ${n(distributedAmount)} ل.س\n`;
  ...
  text += `👨‍💼 مجموع الايداعات بواسطة الإدارة : ${n(extStats.adminDeposits)} ل.س\n`;
  text += `👤 مجموع السحوبات بواسطة الإدارة : ${n(extStats.adminWithdrawals)} ل.س\n`;
  if (FLAG_SEND_BALANCE) {
    text += `🎉 الرصيد المستقبل من صديق : ${n(extStats.sendBalanceReceived)} ل.س\n`;
    text += `😉 الرصيد المرسل إلى صديق : ${n(extStats.sendBalanceSent)} ل.س\n`;
  }
...
function buildAdminStatsExcelBuffer(s, opts = {}) {
  ...
  const summaryRows = [
    ...
    ['تاريخ التصدير', opts.exportDateStr || new Date().toLocaleString('sv-SE', { timeZone: getBotTimezone(), year: 'numeric', month: '2-digit', day: '2-digit' })],
    ['التوقيت', new Date().toLocaleString('sv-SE', { timeZone: getBotTimezone(), year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' }).replace(',', '')],
    [],
    ['المستخدمون', 'القيمة'],
  ...
  function formatTime(d) {
    ...
    return x.toLocaleString('sv-SE', { timeZone: getBotTimezone(), year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' }).replace(',', '');
  }
...
/** Clean a WhatsApp number: strip symbols, spaces, leading + and leading 00. */
function cleanWhatsAppNumber(raw) {
  let num = raw.replace(/[\s\-\.\(\)]/g, '').replace(/^\+/, '');
  if (num.startsWith('00')) num = num.slice(2);
  return num;
}

/** Build inline_keyboard rows with enabled support buttons (Telegram + WhatsApp). */
function buildSupportButtons() {
  const rows = [];
  if (SUPPORT_TELEGRAM_ENABLED && SUPPORT_USERNAME) {
    const tgUrl = `https://t.me/${SUPPORT_USERNAME.trim().replace(/^@/, '')}`;
    rows.push([{ text: '📩 تيليغرام', url: tgUrl }]);
  }
  if (SUPPORT_WHATSAPP_ENABLED && SUPPORT_WHATSAPP_NUMBER) {
    const waUrl = `https://wa.me/${SUPPORT_WHATSAPP_NUMBER}`;
    rows.push([{ text: '📱 واتساب', url: waUrl }]);
  }
  return rows;
}

/** Build inline_keyboard rows with enabled support buttons using BOT_SUPPORT_USERNAME as fallback. */
function buildSupportButtonsWithFallback() {
  const rows = [];
  const tgUsername = (SUPPORT_USERNAME || BOT_SUPPORT_USERNAME || '').trim().replace(/^@/, '');
  if (SUPPORT_TELEGRAM_ENABLED && tgUsername) {
    rows.push([{ text: '📩 تيليغرام', url: `https://t.me/${tgUsername}` }]);
  }
  if (SUPPORT_WHATSAPP_ENABLED && SUPPORT_WHATSAPP_NUMBER) {
    rows.push([{ text: '📱 واتساب', url: `https://wa.me/${SUPPORT_WHATSAPP_NUMBER}` }]);
  }
  return rows;
}

// --- Admin support settings (حساب الدعم) ---

function adminSupportMainMessage() {
  const tgStatus = SUPPORT_TELEGRAM_ENABLED ? '✅ مفعل' : '❌ معطل';
  const waStatus = SUPPORT_WHATSAPP_ENABLED ? '✅ مفعل' : '❌ معطل';
  const tgUser = (SUPPORT_USERNAME || '').trim().replace(/^@/, '') || 'لم يُضبط';
  const waNum = SUPPORT_WHATSAPP_NUMBER || 'لم يُضبط';
  return `🛠 إعدادات حساب الدعم

📩 تيليغرام: ${tgStatus}
👤 المستخدم: <code>${escapeHtml(tgUser)}</code>

📱 واتساب: ${waStatus}
📞 الرقم: <code>${escapeHtml(waNum)}</code>`;
}

function adminSupportMainKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: '📩 تيليغرام', callback_data: 'admin_support_telegram' }],
        [{ text: '📱 واتساب', callback_data: 'admin_support_whatsapp' }],
        [{ text: '🔙 العودة للوحة الأدمن', callback_data: 'admin_panel' }],
      ],
    },
  };
...
function adminSupportTelegramMessage() {
  const current = (SUPPORT_USERNAME || '').trim().replace(/^@/, '') || 'لم يُضبط';
  const status = SUPPORT_TELEGRAM_ENABLED ? '✅ مفعل' : '❌ معطل';
  return `📩 إعدادات دعم تيليغرام

الحالة: ${status}
👤 اسم المستخدم: <code>${escapeHtml(current)}</code>`;
}

function adminSupportTelegramKeyboard() {
  const toggleText = SUPPORT_TELEGRAM_ENABLED ? '❌ تعطيل تيليغرام' : '✅ تفعيل تيليغرام';
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: '✏️ تغيير اسم مستخدم الدعم', callback_data: 'admin_support_change_username' }],
        [{ text: toggleText, callback_data: 'admin_support_toggle_telegram' }],
        [{ text: '🔙 العودة', callback_data: 'admin_support_account' }],
      ],
    },
  };
}

function adminSupportWhatsappMessage() {
  const current = SUPPORT_WHATSAPP_NUMBER || 'لم يُضبط';
  const status = SUPPORT_WHATSAPP_ENABLED ? '✅ مفعل' : '❌ معطل';
  return `📱 إعدادات دعم واتساب

الحالة: ${status}
📞 الرقم: <code>${escapeHtml(current)}</code>`;
}

function adminSupportWhatsappKeyboard() {
  const toggleText = SUPPORT_WHATSAPP_ENABLED ? '❌ تعطيل واتساب' : '✅ تفعيل واتساب';
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: '✏️ تغيير رقم واتساب الدعم', callback_data: 'admin_support_change_whatsapp' }],
        [{ text: toggleText, callback_data: 'admin_support_toggle_whatsapp' }],
        [{ text: '🔙 العودة', callback_data: 'admin_support_account' }],
      ],
    },
  };
...
async function adminExchangeRateSettingsMessage() {
  const current = await getConfigValue('EXCHANGE_RATE_SYP_PER_USD', 11500);
...
async function adminManageRatesMessage() {
  ...
  const oxaEnabled = FLAG_DEPOSIT_OXAPAY || FLAG_WITHDRAW_OXAPAY;
  return `⚙️ الإعدادات الحالية:

💰 الحد الأقصى للسحب المباشر:
• سيريتيل كاش: ${fmt(syr.max_cashout_syp)}
• شام كاش: ${fmt(sham.max_cashout_syp)} ل.س | $${sham.max_cashout_usd != null ? sham.max_cashout_usd : '—'} USD${oxaEnabled ? `\n• USDT (كريبتو): ${fmtUsd(oxa.max_cashout_syp)}` : ''}

💵 الحد الأدنى للإيداع:
• سيريتيل كاش: ${fmt(syr.min_deposit_syp)}
• شام كاش: ${fmt(sham.min_deposit_syp)} ل.س | $${sham.min_deposit_usd != null ? sham.min_deposit_usd : '—'} USD${oxaEnabled ? `\n• USDT (كريبتو): ${fmtUsd(oxa.min_deposit_syp)}` : ''}

🏧 الحد الأدنى للسحب:
• سيريتيل كاش: ${fmt(syr.min_cashout_syp)}
• شام كاش: ${fmt(sham.min_cashout_syp)} ل.س | $${sham.min_cashout_usd != null ? sham.min_cashout_usd : '—'} USD${oxaEnabled ? `\n• USDT (كريبتو): ${fmtUsd(oxa.min_cashout_syp)}` : ''}

💸 نسبة خصم السحب:
• سيريتيل كاش: ${pct(syr.cashout_tax_percent)}
• شام كاش: ${pct(sham.cashout_tax_percent)}${oxaEnabled ? `\n• USDT (كريبتو): ${pct(oxa.cashout_tax_percent)}` : ''}

🎁 نسبة البونص:
• سيريتيل كاش: ${pct(syr.deposit_bonus_percent)}
• شام كاش: ${pct(sham.deposit_bonus_percent)}${oxaEnabled ? `\n• USDT (كريبتو): ${pct(oxa.deposit_bonus_percent)}` : ''}`;
}
...
const RATES_EDIT_FIELDS = {
  ...
  min_deposit_usd: 'الحد الأدنى للإيداع (USD)',
  max_cashout_usd: 'الحد الأقصى للسحب المباشر (USD)',
  min_cashout_usd: 'الحد الأدنى للسحب (USD)',
};
...
function adminRatesChooseProviderKeyboard() {
  ...
  if (FLAG_DEPOSIT_OXAPAY || FLAG_WITHDRAW_OXAPAY) {
    rows.push([{ text: '💎 USDT (كريبتو)', callback_data: 'admin_rates_pick_oxapay' }]);
  }
  rows.push([{ text: '🔙 العودة', callback_data: 'admin_rates_edit_back' }]);
  return {
    reply_markup: {
...
async function adminReferralPendingStatsMessage() {
  ...
  if (stats.lastDistributionAt) {
    try {
      ...
      lastDist = d.toLocaleString('sv-SE', { timeZone: getBotTimezone(), year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' }).replace(',', '');
    }
  }
  return `${REFERRAL_STATS_TITLE} 📊
...
function adminReferralPendingStatsKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: '🎯 توزيع جميع العمولات المستحقة', callback_data: 'admin_referral_distribute_all' }],
...
function loggedInMainKeyboard(isAdmin = false) {
  const rows = [
    ...
    ...((process.env.ICHANCY_QUICK_BUTTONS_FOR || '').split(',').map(s => s.trim()).filter(Boolean).includes(BOT_ID)
      ? [[{ text: 'شحن حساب آيتشانسي 📥', callback_data: 'transfer_full_to_ichancy' }, { text: 'سحب من آيتشانسي 📤', callback_data: 'withdraw_ichancy' }]]
      : []),
    [{ text: '👤 معلومات الملف الشخصي', callback_data: 'profile' }],
    [{ text: '🎁 كود هدية', callback_data: 'gift_code' }],
    // [{ text: '🎰 الجاك بوت', callback_data: 'jackpot' }],
    ...((process.env.HIDE_WALLET_BUTTON || '').split(',').map(s => s.trim()).filter(Boolean).includes(BOT_ID)
      ? []
    ...
    ...((process.env.SINGLE_ROW_MENU_FOR || '').split(',').map(s => s.trim()).filter(Boolean).includes(BOT_ID)
      ? [[{ text: '💸 استرداد آخر طلب سحب', callback_data: 'redeem_withdrawal' }, { text: 'Golden Tree ↗', url: GOLDEN_TREE_URL }]]
      : [[{ text: 'Golden Tree ↗', url: GOLDEN_TREE_URL }], [{ text: '💸 استرداد آخر طلب سحب', callback_data: 'redeem_withdrawal' }]]),
...
bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
      ...
      try {
        ...
        if (existingForRef) {
          ...
          if (saved) {
            ...
            notifyReferralChain(userId, existingForRef.ichancy_login || '');
          }
        } else {
  ...
  const _hideSpinFor = (process.env.HIDE_SPIN_FOR || '').split(',').map(s => s.trim()).filter(Boolean);
  if (hasAccount && SPIN_BASE_URL && !_hideSpinFor.includes(BOT_ID)) {
    try {
  ...
  } else if (hasAccount && _hideSpinFor.includes(BOT_ID)) {
    // Remove any leftover spin keyboard from previous sessions
    try {
...
bot.on('callback_query', async (query) => {
  try {
    ...
    // --- Toggle support: must answer BEFORE the blanket answerCallbackQuery so popup alerts work ---
    if (data === 'admin_support_toggle_telegram') {
      if (!isAdminUser(query.from)) return;
      if (!SUPPORT_TELEGRAM_ENABLED && !(SUPPORT_USERNAME || '').trim()) {
        await bot.answerCallbackQuery(query.id, { text: '⚠️ يجب إضافة حساب تيليغرام أولاً قبل التفعيل', show_alert: true });
        return;
      }
      SUPPORT_TELEGRAM_ENABLED = !SUPPORT_TELEGRAM_ENABLED;
      await setConfigValue('SUPPORT_TELEGRAM_ENABLED', SUPPORT_TELEGRAM_ENABLED ? 'true' : 'false');
      await bot.answerCallbackQuery(query.id, { text: SUPPORT_TELEGRAM_ENABLED ? '✅ تم تفعيل تيليغرام' : '❌ تم تعطيل تيليغرام' });
      await bot.editMessageText(adminSupportTelegramMessage(), {
        chat_id: chatId,
        message_id: messageId,
        parse_mode: 'HTML',
        ...adminSupportTelegramKeyboard(),
      });
      return;
    }

    if (data === 'admin_support_toggle_whatsapp') {
      if (!isAdminUser(query.from)) return;
      if (!SUPPORT_WHATSAPP_ENABLED && !(SUPPORT_WHATSAPP_NUMBER || '').trim()) {
        await bot.answerCallbackQuery(query.id, { text: '⚠️ يجب إضافة رقم واتساب أولاً قبل التفعيل', show_alert: true });
        return;
      }
      SUPPORT_WHATSAPP_ENABLED = !SUPPORT_WHATSAPP_ENABLED;
      await setConfigValue('SUPPORT_WHATSAPP_ENABLED', SUPPORT_WHATSAPP_ENABLED ? 'true' : 'false');
      await bot.answerCallbackQuery(query.id, { text: SUPPORT_WHATSAPP_ENABLED ? '✅ تم تفعيل واتساب' : '❌ تم تعطيل واتساب' });
      await bot.editMessageText(adminSupportWhatsappMessage(), {
        chat_id: chatId,
        message_id: messageId,
        parse_mode: 'HTML',
        ...adminSupportWhatsappKeyboard(),
      });
      return;
    }
  ...
  if (data === 'create_account') {
    ...
    const skipOtpList = (process.env.SKIP_USER_CREATION_OTP_FOR || '').split(',').map(s => s.trim()).filter(Boolean);
    const skipOtp = skipOtpList.includes(BOT_ID);
    if (skipOtp) {
      userState[chatId] = { step: 'await_username' };
      await bot.editMessageText('إنشاء حساب أيشانسي ➕', {
        chat_id: chatId,
        message_id: messageId,
        reply_markup: { inline_keyboard: [] },
      });
      await bot.sendMessage(chatId, MSG_ASK_USERNAME);
    } else {
    ...
    if (data === 'terms') {
      ...
      const useAltPolicy = (process.env.USE_ALT_POLICY_FOR || '').split(',').map(s => s.trim()).filter(Boolean).includes(BOT_ID);
      const termsDepositRequired = await cfgInt('deposit_required_ls', 50000);
      const termsReferralsRequired = await cfgInt('active_referrals_required', 5);
      await bot.editMessageText(useAltPolicy ? ALT_TERMS_TEXT : getTermsText(termsDepositRequired, termsReferralsRequired), {
        chat_id: chatId,
        message_id: messageId,
        ...termsKeyboard(),
      });
    ...
    // --- Admin support account: main menu ---
    if (data === 'admin_support_account') {
      ...
      delete userState[chatId];
      await bot.editMessageText(adminSupportMainMessage(), {
        chat_id: chatId,
        message_id: messageId,
        parse_mode: 'HTML',
        ...adminSupportMainKeyboard(),
      });
      return;
    }

    // --- Admin support: Telegram sub-menu ---
    if (data === 'admin_support_telegram') {
      if (!isAdminUser(query.from)) return;
      delete userState[chatId];
      await bot.editMessageText(adminSupportTelegramMessage(), {
        chat_id: chatId,
    ...
    // --- Admin support: WhatsApp sub-menu ---
    if (data === 'admin_support_whatsapp') {
      if (!isAdminUser(query.from)) return;
      delete userState[chatId];
      await bot.editMessageText(adminSupportWhatsappMessage(), {
        chat_id: chatId,
    ...
    // --- Edit Telegram support username ---
    if (data === 'admin_support_change_username') {
      ...
      await bot.editMessageText(
        `🛠️ الرجاء إرسال اسم مستخدم الدعم الجديد (مثال: @${exampleUser})\n\nملاحظة: يمكنك الضغط على 'العودة' للإلغاء.`,
        {
          ...
          message_id: messageId,
          reply_markup: {
            inline_keyboard: [[{ text: '🔙 العودة', callback_data: 'admin_support_telegram' }]],
          },
        }
      );
      return;
    ...
    // --- Edit WhatsApp support number ---
    if (data === 'admin_support_change_whatsapp') {
      if (!isAdminUser(query.from)) return;
      const exampleNum = SUPPORT_WHATSAPP_NUMBER || '963XXXXXXXX';
      userState[chatId] = { step: 'await_admin_support_whatsapp' };
      await bot.editMessageText(
        `📱 الرجاء إرسال رقم واتساب الدعم مع رمز الدولة (مثال: ${exampleNum})\n\nيمكنك الضغط على 'العودة' للإلغاء.`,
        {
          chat_id: chatId,
          message_id: messageId,
          reply_markup: {
            inline_keyboard: [[{ text: '🔙 العودة', callback_data: 'admin_support_whatsapp' }]],
          },
        }
      );
      return;
    ...
    if (data === 'admin_rates_pick_syriatel' || data === 'admin_rates_pick_shamcash' || data === 'admin_rates_pick_oxapay') {
      ...
      // ShamCash + min_deposit, max_cashout, or min_cashout → show SYP/USD sub-choice
      if (provider === 'shamcash' && (state.ratesField === 'min_deposit_syp' || state.ratesField === 'max_cashout_syp' || state.ratesField === 'min_cashout_syp')) {
        const usdField = state.ratesField === 'min_deposit_syp' ? 'min_deposit_usd' : state.ratesField === 'max_cashout_syp' ? 'max_cashout_usd' : 'min_cashout_usd';
        const shamCfg = await getProviderConfig('shamcash');
        const currentSyp = shamCfg[state.ratesField];
        const currentUsd = shamCfg[usdField];
        const fmtSyp = currentSyp != null && Number.isFinite(Number(currentSyp)) ? formatNumber(Number(currentSyp)) : '—';
        const fmtUsd = currentUsd != null ? currentUsd : '—';
        userState[chatId] = { step: 'admin_rates_sham_currency_choice', ratesField: state.ratesField, usdField, provider, messageId: state.messageId };
        await bot.editMessageText(
          `✏️ تعديل <b>${fieldLabel}</b> — شام كاش\n\n` +
          `القيم الحالية:\n• ل.س (SYP): ${fmtSyp}\n• دولار (USD): $${fmtUsd}\n\nاختر العملة:`,
          {
            chat_id: chatId,
            message_id: messageId,
            ...
            reply_markup: {
              inline_keyboard: [
                [{ text: '🇸🇾 ليرة سورية (SYP)', callback_data: 'admin_rates_sham_pick_syp' }],
                [{ text: '💵 دولار أمريكي (USD)', callback_data: 'admin_rates_sham_pick_usd' }],
    ...
    if (data === 'admin_rates_sham_pick_syp' || data === 'admin_rates_sham_pick_usd') {
      if (!isAdminUser(query.from)) return;
      const state = userState[chatId];
      if (state?.step !== 'admin_rates_sham_currency_choice') return;
      const isUsd = data === 'admin_rates_sham_pick_usd';
      const field = isUsd ? state.usdField : state.ratesField;
      const fieldLabel = RATES_EDIT_FIELDS[field] || field;
      const hint = isUsd ? ' (مبلغ بالدولار)' : ' (رقم صحيح بالليرة)';
      userState[chatId] = { step: 'await_admin_rates_single', field, provider: 'shamcash', messageId: state.messageId };
      await bot.editMessageText(
        `✏️ تعديل <b>${fieldLabel}</b> — شام كاش\n\nأرسل القيمة الجديدة${hint}:`,
        {
          chat_id: chatId,
    ...
    if (data === 'admin_referral_distribute_all') {
      ...
      try {
        ...
        const feedbackMsg = userCount > 0
          ...
          : `ℹ️ لا توجد عمولات مستحقة لتوزيعها.`;
    ...
    if (data === 'admin_referral_view_details') {
      ...
      const lines = rows.map((r) => {
        let dateStr = '—';
        if (r.distributed_at) {
          try { dateStr = formatInBotTz(r.distributed_at); } catch (_) { dateStr = new Date(r.distributed_at).toLocaleString('sv-SE', { timeZone: getBotTimezone(), year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }).replace(',', ''); }
        }
    ...
    if (data === 'gift_view_all' || data.startsWith('gift_view_all:')) {
      if (!isAdminUser(query.from)) return;
      const pageArg = data.startsWith('gift_view_all:') ? parseInt(data.slice('gift_view_all:'.length), 10) : 0;
      const page = Number.isFinite(pageArg) && pageArg >= 0 ? pageArg : 0;
      const codes = await listGiftCodes({});
      const tz = getBotTimezone();
      const now = new Date();
      ...
      const navRow = [];
      if (safePage > 0) navRow.push({ text: '◀️ السابق', callback_data: `gift_view_all:${safePage - 1}` });
      if (safePage < totalPages - 1) navRow.push({ text: 'التالي ▶️', callback_data: `gift_view_all:${safePage + 1}` });
      const keyboardRows = [];
      if (navRow.length > 0) keyboardRows.push(navRow);
      keyboardRows.push([{ text: '🔙 العودة', callback_data: 'gift_back' }]);
      await bot.editMessageText(text, { chat_id: chatId, message_id: messageId, parse_mode: 'HTML', reply_markup: { inline_keyboard: keyboardRows } });
      return;
    }
    ...
    if (data === 'admin_stats') {
      ...
      try {
        ...
        silentCheckIchancyBalance(cookies, agentWalletBalance).catch(() => {});
      } catch (err) {
    ...
    if (data === 'admin_stats_export') {
      ...
      try {
        const buffer = buildAdminStatsExcelBuffer(stats, {
          ...
          weekly,
          all,
          exportDateStr: bounds.exportDateStr || new Date().toLocaleString('sv-SE', { timeZone: getBotTimezone(), year: 'numeric', month: '2-digit', day: '2-digit' }),
        });
        const filename = `الإحصائيات_${bounds.exportDateStr || new Date().toLocaleString('sv-SE', { timeZone: getBotTimezone(), year: 'numeric', month: '2-digit', day: '2-digit' })}.xlsx`;
    ...
    // Admin: رصيد شام كاش — current balance from API only (history is a separate screen)
    if (data === 'admin_sham_balance' && isAdminUser(query.from)) {
      ...
      await bot.editMessageText(msg, {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: '📋 سجل طلبات السحب', callback_data: 'admin_sham_history:0' }],
            [{ text: '🔙 العودة للوحة الأدمن', callback_data: 'admin_panel' }],
          ],
        },
      });
    ...
    // Admin: سجل طلبات السحب (شام كاش) — paginated history
    if (data.startsWith('admin_sham_history:') && isAdminUser(query.from)) {
      await bot.answerCallbackQuery(query.id);
      const pageArg = parseInt(data.slice('admin_sham_history:'.length), 10);
      const page = Number.isFinite(pageArg) && pageArg >= 0 ? pageArg : 0;
      ...
      const formatDate = (d) => {
        try {
          ...
          return new Date(d).toLocaleString('sv-SE', { timeZone: getBotTimezone() });
        }
      };
      ...
      let msg = `📋 سجل طلبات السحب (شام كاش) — آخر ${history.length} طلب — صفحة ${safePage + 1}/${totalPages}:\n\n`;
      if (history.length === 0) {
      ...
      const navRow = [];
      if (safePage > 0) navRow.push({ text: '◀️ السابق', callback_data: `admin_sham_history:${safePage - 1}` });
      if (safePage < totalPages - 1) navRow.push({ text: 'التالي ▶️', callback_data: `admin_sham_history:${safePage + 1}` });
      const keyboard = [];
      if (navRow.length > 0) keyboard.push(navRow);
      keyboard.push([{ text: '🔙 العودة للرصيد', callback_data: 'admin_sham_balance' }]);
      await bot.editMessageText(msg, {
        chat_id: chatId,
        message_id: messageId,
        parse_mode: 'HTML',
    ...
    // Admin: list all pending ShamCash withdrawals — accept / reject / message user per row (paginated)
    if ((data === 'admin_sham_pending_list' || data.startsWith('admin_sham_pending_list:')) && isAdminUser(query.from)) {
      const pageArg = data.startsWith('admin_sham_pending_list:') ? parseInt(data.slice('admin_sham_pending_list:'.length), 10) : 0;
      const page = Number.isFinite(pageArg) && pageArg >= 0 ? pageArg : 0;
      const list = await getAllShamcashPending();
      const timezone = (await getConfigValue('timezone', 'Asia/Damascus')) || 'Asia/Damascus';
      const formatDate = (d) => {
      ...
      const userIds = [...new Set(pageItems.map((p) => p.telegram_user_id))];
      const displayMap = await getUsersDisplayMap(userIds);
      let text = `📋 كل طلبات السحب المعلقة (شام كاش) — صفحة ${safePage + 1}/${totalPages} (إجمالي ${list.length}):\n\n`;
      const rows = [];
      for (const p of pageItems) {
        const curLabel = p.currency === 'usd' ? ' USD' : ' ل.س';
        const botName = displayMap[String(p.telegram_user_id)] || String(p.telegram_user_id);
        let syrWarn = '';
        try {
          const syrCount = await getSyriatelDepositCountInLastN(p.telegram_user_id, 3);
          if (syrCount > 0) syrWarn = `\n🟥🟥🟥🟥🟥\n📣📣 محاولة احتيال 📣📣\nبناء على عمليات التعبئة الأخيرة لهذا الزبون على البوت قام بالشحن عبر سريتل وهو الآن يطلب السحب عبر شام كاش يرجى التدقيق قبل الموافقة .\n🟥🟥🟥🟥🟥\n`;
        } catch (_) {}
        text += `• ${escapeHtml(botName)} — <code>${escapeHtml(p.client_code)}</code> — ${escapeHtml(p.amount_display)}${curLabel} — ${formatDate(p.created_at)}${syrWarn}\n`;
        rows.push([
      ...
      const navRow = [];
      if (safePage > 0) navRow.push({ text: '◀️ السابق', callback_data: `admin_sham_pending_list:${safePage - 1}` });
      if (safePage < totalPages - 1) navRow.push({ text: 'التالي ▶️', callback_data: `admin_sham_pending_list:${safePage + 1}` });
    ...
    // Admin: list all pending OxaPay withdrawals — accept / reject per row (paginated)
    if ((data === 'admin_oxapay_pending_list' || data.startsWith('admin_oxapay_pending_list:')) && isAdminUser(query.from)) {
      const pageArg = data.startsWith('admin_oxapay_pending_list:') ? parseInt(data.slice('admin_oxapay_pending_list:'.length), 10) : 0;
      const page = Number.isFinite(pageArg) && pageArg >= 0 ? pageArg : 0;
      ...
      const userIds = [...new Set(pageItems.map((p) => p.telegram_user_id))];
      const displayMap = await getUsersDisplayMap(userIds);
      let text = `📋 طلبات السحب المعلقة (USDT / كريبتو) — صفحة ${safePage + 1}/${totalPages} (إجمالي ${list.length}):\n\n`;
      const rows = [];
      for (const p of pageItems) {
        const botName = displayMap[String(p.telegram_user_id)] || String(p.telegram_user_id);
      ...
      const navRow = [];
      if (safePage > 0) navRow.push({ text: '◀️ السابق', callback_data: `admin_oxapay_pending_list:${safePage - 1}` });
      if (safePage < totalPages - 1) navRow.push({ text: 'التالي ▶️', callback_data: `admin_oxapay_pending_list:${safePage + 1}` });
      if (navRow.length > 0) rows.push(navRow);
      rows.push([{ text: '🔙 العودة للوحة الأدمن', callback_data: 'admin_panel' }]);
      await bot.editMessageText(text, {
        chat_id: chatId,
    ...
    if (data === 'admin_sham_msg_cancel' && isAdminUser(query.from)) {
      delete userState[chatId];
      await bot.answerCallbackQuery(query.id, { text: 'تم الإلغاء.' }).catch(() => {});
      // Navigate directly back to the pending list
      const _cancelList = await getAllShamcashPending();
      const _cancelTz = (await getConfigValue('timezone', 'Asia/Damascus')) || 'Asia/Damascus';
      const _cancelFmtDate = (d) => { try { return new Date(d).toLocaleString('ar-SY', { timeZone: _cancelTz, dateStyle: 'short', timeStyle: 'short' }); } catch (_) { return new Date(d).toLocaleString('sv-SE', { timeZone: getBotTimezone() }); } };
      if (!_cancelList || _cancelList.length === 0) {
        await bot.editMessageText('لا توجد طلبات سحب معلقة.', {
          chat_id: chatId, message_id: messageId,
          reply_markup: { inline_keyboard: [[{ text: '🔙 العودة', callback_data: 'admin_pending_withdrawals' }]] },
        });
      ...
      const _cancelUserIds = [...new Set(_cancelList.map((p) => p.telegram_user_id))];
      const _cancelDisplayMap = await getUsersDisplayMap(_cancelUserIds);
      let _cancelText = '📋 كل طلبات السحب المعلقة (شام كاش):\n\n';
      const _cancelRows = [];
      for (const p of _cancelList) {
        const curLabel = p.currency === 'usd' ? ' USD' : ' ل.س';
        const botName = _cancelDisplayMap[String(p.telegram_user_id)] || String(p.telegram_user_id);
        _cancelText += `• ${escapeHtml(botName)} — <code>${escapeHtml(p.client_code)}</code> — ${escapeHtml(p.amount_display)}${curLabel} — ${_cancelFmtDate(p.created_at)}\n`;
        _cancelRows.push([
          { text: `✅ قبول #${p.id}`, callback_data: `sham_accept_${p.id}` },
          { text: `❌ رفض #${p.id}`, callback_data: `sham_reject_${p.id}` },
          { text: `💬 مراسلة`, callback_data: `admin_sham_msg_${p.id}` },
      ...
      _cancelRows.push([{ text: '🔙 العودة', callback_data: 'admin_pending_withdrawals' }]);
      await bot.editMessageText(_cancelText, {
        chat_id: chatId, message_id: messageId, parse_mode: 'HTML',
        reply_markup: { inline_keyboard: _cancelRows },
      });
      return;
    }

    // Helper: build user acceptance message + display amounts for ShamCash withdrawal
    async function buildShamAcceptUserMsg(pending) {
      const rates = await getRatesForPayment();
      const taxPercent = Number(rates.shamcash.cashout_tax_percent ?? 0);
      const exchangeRate = rates.exchangeRate;
      let amountTransferredDisplay, curLabel;
      if (pending.currency === 'syp') {
        const amountAfterTax = Number(pending.amount_syp) * (1 - taxPercent / 100);
      ...
      return `✅ تمت الموافقة على طلب السحب.\n\nيجري الآن تحويل الرصيد إلى حسابك في شام كاش.\n\n• رمز العميل: <code>${escapeHtml(pending.client_code)}</code>\n• المبلغ المحوّل: ${amountTransferredDisplay}${curLabel}\n\nيرجى التحقق من وصول الرصيد وفي حال التأخير مراسلة الدعم ✅️`;
    }

    // Helper: complete ShamCash accept — update DB, notify user, edit admin message. skipAnswer=true when caller already answered the callback.
    async function completeShamAccept(pending, pendingId, query, resolvedBy, channelStatusText, skipAnswer) {
      try {
        if (pending.transaction_id) await updateTransactionStatus(pending.transaction_id, 'confirmed');
        await updateShamcashPendingStatus(pendingId, 'accepted', resolvedBy);
      } catch (err) {
        console.warn('sham_accept:', err.message);
        if (!skipAnswer) {
          await bot.answerCallbackQuery(query.id, { text: 'حدث خطأ.' }).catch(() => {});
        } else {
          // skipAnswer=true means caller already edited the message to processing state.
          // Edit it to show the error + back button so admin isn't stuck.
          const _errMsgText = query.message.text || query.message.caption || '';
          if (!_errMsgText.startsWith('📤 طلب سحب شام كاش')) {
            await bot.editMessageText(`⚠️ حدث خطأ أثناء المعالجة.\n\n🆔 طلب #${pendingId}`, {
              chat_id: query.message.chat.id,
              message_id: query.message.message_id,
              reply_markup: { inline_keyboard: [[{ text: '🔙 العودة للطلبات', callback_data: 'admin_sham_pending_list' }]] },
            }).catch(() => {});
          }
        }
        return false;
      ...
      let userMsg;
      try {
        userMsg = await buildShamAcceptUserMsg(pending);
      } catch (err) {
        console.warn('sham_accept buildMsg:', err.message);
        userMsg = `✅ تمت الموافقة على طلب السحب.\n\nيجري الآن تحويل الرصيد إلى حسابك في شام كاش.\n• رمز العميل: <code>${escapeHtml(pending.client_code)}</code>\n\nيرجى التحقق من وصول الرصيد وفي حال التأخير مراسلة الدعم ✅️`;
      }
      ...
      if (!skipAnswer) await bot.answerCallbackQuery(query.id, { text: 'تمت الموافقة وإشعار المستخدم.' }).catch(() => {});
      const msgText = query.message.text || query.message.caption || '';
      if (msgText.startsWith('📤 طلب سحب شام كاش')) {
        try {
          const updatedText = reApplyShamcashChannelHtml(msgText.replace(/⏳ في انتظار الموافقة\.|⚠️ فشل التحويل التلقائي:.*/g, channelStatusText));
          await bot.editMessageText(updatedText, { chat_id: query.message.chat.id, message_id: query.message.message_id, parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: '✅ تمت الموافقة', callback_data: 'sham_done' }]] } });
        } catch (_) {}
      } else {
        // In-bot admin panel — edit same message with status + back button
        try {
    ...
    // Helper: handle ShamCash auto-transfer failure — edit admin message with error + 3 buttons
    function handleShamAutoFailure(error, pendingId, query) {
      const errorLine = error.includes('Insufficient balance')
        ? '⚠️ فشل التحويل التلقائي: رصيد المحفظة غير كافٍ'
        : error.includes('blocked from operations for 24 hours')
          ? '⚠️ فشل التحويل التلقائي: الجهاز محظور لمدة 24 ساعة'
          : `⚠️ فشل التحويل التلقائي: ${error}`;
      const failureButtons = {
        inline_keyboard: [
          [{ text: '✅ قبول - تم الدفع يدوياً', callback_data: `sham_accept_manual_${pendingId}` }],
          [{ text: '✅ قبول - إعادة المحاولة', callback_data: `sham_retry_${pendingId}` }],
          [{ text: '❌ رفض', callback_data: `sham_reject_${pendingId}` }],
        ],
      };
      const msgText = query.message.text || query.message.caption || '';
      if (msgText.startsWith('📤 طلب سحب شام كاش')) {
        // Channel message — edit in-place with error + 3 buttons
        const updatedText = reApplyShamcashChannelHtml(msgText.replace(/⏳ في انتظار الموافقة\.|⚠️ فشل التحويل التلقائي:.*/g, errorLine));
        bot.editMessageText(updatedText, {
          chat_id: query.message.chat.id,
          message_id: query.message.message_id,
          parse_mode: 'HTML',
          reply_markup: failureButtons,
        }).catch(() => {});
      } else {
        // In-bot admin panel — edit same message with error + 3 buttons + back
        failureButtons.inline_keyboard.push([{ text: '🔙 العودة للطلبات', callback_data: 'admin_sham_pending_list' }]);
        bot.editMessageText(`${errorLine}\n\n🆔 طلب #${pendingId}`, {
          chat_id: query.message.chat.id,
          message_id: query.message.message_id,
          reply_markup: failureButtons,
        }).catch(() => {});
    ...
    // Helper: when admin clicks accept/reject/retry on an already-processed request,
    // show alert + update the channel message (remove action buttons, show final status).
    async function handleShamAlreadyProcessed(pending, query) {
      ...
      if (msgText.startsWith('📤 طلب سحب شام كاش')) {
        ...
        if (pending && pending.status === 'accepted') {
          ...
          buttonText = '✅ تمت الموافقة';
        } else if (pending && pending.status === 'rejected') {
          statusText = '❌ تم الرفض.';
          buttonText = '❌ تم الرفض';
        } else {
          statusText = '⚠️ تمت المعالجة مسبقاً.';
          buttonText = '⚠️ تمت المعالجة';
        }
        try {
          const updatedText = reApplyShamcashChannelHtml(msgText.replace(/⏳ في انتظار الموافقة\.|⚠️ فشل التحويل التلقائي:.*/g, statusText));
          await bot.editMessageText(updatedText, {
            chat_id: query.message.chat.id,
            message_id: query.message.message_id,
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: [[{ text: buttonText, callback_data: 'sham_done' }]] },
          });
        } catch (_) {}
      }
    }

    // Admin: Accept ShamCash withdrawal — manual payment confirmed (after auto-transfer failure)
    if (data.startsWith('sham_accept_manual_')) {
      if (!isAdminUser(query.from)) {
        await bot.answerCallbackQuery(query.id, { text: 'غير مصرح' }).catch(() => {});
        return;
      }
      const pendingId = parseInt(data.replace('sham_accept_manual_', ''), 10);
      if (!Number.isFinite(pendingId)) return;
      const pending = await getShamcashPendingById(pendingId);
      if (!pending || pending.status !== 'pending') {
        await handleShamAlreadyProcessed(pending, query);
        return;
      }
      await completeShamAccept(pending, pendingId, query, 'admin_accept_manual', '✅ تمت الموافقة - تم الدفع يدوياً.');
      return;
    }

    // Admin: Retry automatic ShamCash transfer (after previous auto-transfer failure)
    if (data.startsWith('sham_retry_')) {
      if (!isAdminUser(query.from)) {
        await bot.answerCallbackQuery(query.id, { text: 'غير مصرح' }).catch(() => {});
        return;
      }
      const pendingId = parseInt(data.replace('sham_retry_', ''), 10);
      if (!Number.isFinite(pendingId)) return;
      if (shamcashRetryInProgress.has(pendingId)) {
        await bot.answerCallbackQuery(query.id, { text: 'جاري المعالجة بالفعل.' }).catch(() => {});
        return;
      }
      shamcashRetryInProgress.add(pendingId);
      try {
        const pending = await getShamcashPendingById(pendingId);
        if (!pending || pending.status !== 'pending') {
          await handleShamAlreadyProcessed(pending, query);
        ...
        // In-bot admin: show processing state on same message
        const _retryMsgText = query.message.text || query.message.caption || '';
        if (!_retryMsgText.startsWith('📤 طلب سحب شام كاش')) {
          await bot.editMessageText(`⏳ جاري إعادة المحاولة...\n\n🆔 طلب #${pendingId}`, {
            chat_id: query.message.chat.id, message_id: query.message.message_id,
          }).catch(() => {});
        }
        const result = await attemptShamcashAutoTransfer(pending);
        if (result.success) {
          await completeShamAccept(pending, pendingId, query, 'admin_accept_auto', '✅ تمت الموافقة - تم التحويل التلقائي بنجاح.', true);
        } else {
          handleShamAutoFailure(result.error || 'Unknown error', pendingId, query);
        }
      } finally {
        shamcashRetryInProgress.delete(pendingId);
      }
      return;
    }

    // Admin (or from channel): Accept ShamCash withdrawal — mark done, notify user (with optional semi-automatic transfer)
    if (data.startsWith('sham_accept_')) {
      ...
      // Semi-automatic: attempt API transfer first
      const semiAutomatic = !!(await getConfigValue('SEMI_SHAMCASH_AUTOMATIC', false));
      if (semiAutomatic) {
        if (shamcashRetryInProgress.has(pendingId)) {
          await bot.answerCallbackQuery(query.id, { text: 'جاري المعالجة بالفعل.' }).catch(() => {});
          return;
        ...
        shamcashRetryInProgress.add(pendingId);
        try {
          await bot.answerCallbackQuery(query.id, { text: 'جاري التحويل التلقائي...' }).catch(() => {});
          // In-bot admin: show processing state on same message
          const _acceptMsgText = query.message.text || query.message.caption || '';
          if (!_acceptMsgText.startsWith('📤 طلب سحب شام كاش')) {
            await bot.editMessageText(`⏳ جاري التحويل التلقائي...\n\n🆔 طلب #${pendingId}`, {
              chat_id: query.message.chat.id, message_id: query.message.message_id,
            }).catch(() => {});
          }
          const result = await attemptShamcashAutoTransfer(pending);
      ...
      // Manual accept (semi_shamcash_automatic disabled)
      await completeShamAccept(pending, pendingId, query, 'admin_accept', '✅ تمت الموافقة.');
      return;
    }
    ...
    if (data.startsWith('sham_reject_')) {
      ...
      try {
        const refundAmount = Math.round(amountSyp * 100) / 100;
        ...
        logTransaction({
          ...
          method: 'refund',
          status: 'confirmed',
          transferId: pending.transaction_id ? `ref:${pending.transaction_id}` : null,
        }).catch(e => console.warn('logTransaction sham_refund:', e.message));
      } catch (err) {
      ...
      const _supportBtns = buildSupportButtonsWithFallback();
      const replyMarkup = _supportBtns.length ? { reply_markup: { inline_keyboard: _supportBtns } } : {};
      ...
      if (msgTextRej.startsWith('📤 طلب سحب شام كاش')) {
        try {
          const updatedTextRej = reApplyShamcashChannelHtml(msgTextRej.replace(/⏳ في انتظار الموافقة\.|⚠️ فشل التحويل التلقائي:.*/g, '❌ تم الرفض.'));
          await bot.editMessageText(updatedTextRej, { chat_id: query.message.chat.id, message_id: query.message.message_id, parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: '❌ تم الرفض', callback_data: 'sham_done' }]] } });
        } catch (_) {}
      } else {
        // In-bot admin panel — edit same message with rejection status + back button
        try {
          await bot.editMessageText(`❌ تم الرفض واسترداد الرصيد.\n\n🆔 طلب #${pendingId}`, {
            chat_id: query.message.chat.id,
            message_id: query.message.message_id,
            reply_markup: { inline_keyboard: [[{ text: '🔙 العودة للطلبات', callback_data: 'admin_sham_pending_list' }]] },
          });
    ...
    if (data.startsWith('syr_retry_')) {
      ...
      try {
        ...
        console.log(`[SyriatelCashout] ADMIN RETRY — pendingId=${pendingId} phone=${pending.phone} remaining=${remaining} amountSent=${pending.amount_sent} totalAmount=${pending.amount_new_currency}`);
        const result = await smartSyriatelCashout({ phone: pending.phone, amountNewCurrency: remaining });
        console.log(`[SyriatelCashout] ADMIN RETRY RETURNED — pendingId=${pendingId} ${JSON.stringify({ success: result.success, fullSuccess: result.fullSuccess, partial: result.partial, totalSent: result.totalSent, remaining: result.remaining, failureReason: result.failureReason })}`);
          ...
          if (refundAmount > 0) {
            ...
            if (!refunded) {
              ...
              logTransaction({
                ...
                status: 'confirmed',
                transferId: pending.transaction_id ? `ref:${pending.transaction_id}` : null,
              }).catch(e => console.warn('logTransaction syr_partial_refund:', e.message));
            }
          }
          await updateSyriatelPendingStatus(pendingId, 'completed', 'admin_retry').catch(() => {});
          if (pending.transaction_id) updateTransactionStatus(pending.transaction_id, 'confirmed').catch(() => {});
        ...
        if (refundAmount > 0) {
          ...
          logTransaction({
            ...
            status: 'confirmed',
            transferId: pending.transaction_id ? `ref:${pending.transaction_id}` : null,
          }).catch(e => console.warn('logTransaction syr_retry_refund:', e.message));
        }
      ...
      try {
        ...
        if (refundAmount > 0) {
          ...
          logTransaction({
            ...
            status: 'confirmed',
            transferId: pending.transaction_id ? `ref:${pending.transaction_id}` : null,
          }).catch(e => console.warn('logTransaction syr_ignore_refund:', e.message));
        }
    ...
    // Admin: iChancy alert settings screen
    if (data === 'admin_ichancy_alerts') {
      if (!isAdminUser(query.from)) {
        await bot.answerCallbackQuery(query.id, { text: 'غير مصرح' }).catch(() => {});
        return;
      }
      await loadLocalConfig();
      await bot.editMessageText(adminIchancyAlertsMessage(), {
        chat_id: chatId,
        message_id: messageId,
        ...adminIchancyAlertsKeyboard(),
      });
      return;
    }

    // Admin: Toggle iChancy low balance alert
    if (data === 'admin_ich_alert_toggle') {
      if (!isAdminUser(query.from)) return;
      await setConfigValue('ICHANCY_LOW_BALANCE_ALERT_ENABLED', !FLAG_ICHANCY_LOW_BALANCE_ALERT);
      await loadLocalConfig();
      await bot.editMessageText(adminIchancyAlertsMessage(), {
        chat_id: chatId,
        message_id: messageId,
      ...
      await bot.answerCallbackQuery(query.id, { text: FLAG_ICHANCY_LOW_BALANCE_ALERT ? 'تم التفعيل' : 'تم الإيقاف' }).catch(() => {});
      return;
    }

    // Admin: Set iChancy low balance alert threshold — prompt for input
    if (data === 'admin_ich_alert_set_threshold') {
      if (!isAdminUser(query.from)) return;
      userState[chatId] = { step: 'await_admin_ich_alert_threshold', messageId };
      await bot.editMessageText(
        `💰 تعديل حد تنبيه رصيد iChancy\n\nالحد الحالي: <code>${formatNumber(ICHANCY_LOW_BALANCE_THRESHOLD)}</code> ل.س\n\nأرسل القيمة الجديدة:`,
        {
          chat_id: chatId,
          message_id: messageId,
          parse_mode: 'HTML',
          reply_markup: { inline_keyboard: [[{ text: '❌ إلغاء', callback_data: 'admin_ichancy_alerts' }]] },
        }
      );
      return;
    }
    ...
    if (data.startsWith('admin_user_logs_') && isAdminUser(query.from)) {
      ...
      try {
        ...
        const methodLabels = {
          ...
          admin: '👨‍💼 الإدارة',
          send_balance: '🎁 إهداء رصيد',
          gift_code: '🎟️ كود هدية',
          referral: '👥 عمولة إحالة',
          refund: '🔄 استرداد',
        };
        const typeLabels = {
          deposit: { syriatel: '📥 إيداع', sham_usd: '📥 إيداع', sham_syp: '📥 إيداع', balance_to_site: '💻 إيداع للموقع', site_to_balance: '💻 سحب من الموقع', spin_wheel: '🎡 ربح العجلة', box_game: '🎮 ربح الصندوق', admin: '👨‍💼 إيداع للحساب بواسطة الإدارة', send_balance: '🎁 استلام إهداء رصيد', gift_code: '🎟️ تفعيل كود هدية', referral: '👥 عمولة إحالة', refund: '🔄 استرداد مبلغ' },
          withdrawal: { syriatel: '📤 سحب', sham_usd: '📤 سحب', sham_syp: '📤 سحب', shamcash: '📤 سحب', balance_to_site: '💻 إيداع للموقع', site_to_balance: '💻 سحب من الموقع', admin: '👨‍💼 سحب من الحساب بواسطة الإدارة', send_balance: '🎁 إهداء رصيد' },
        };
        let text = `━━━━━━━━━━━━━━━\n📜 سجل عمليات المستخدم\n━━━━━━━━━━━━━━━\n\n`;
        if (result.rows.length === 0) {
          ...
          for (const tx of result.rows) {
            ...
            let label = typeGroup[tx.method] || (tx.type === 'deposit' ? '📥 إيداع' : '📤 سحب');
            const statusSuffix = tx.status === 'confirmed' ? ' ( ناجح)' : (tx.status === 'rejected' || tx.status === 'cancelled') ? ' (فاشل)' : ' (قيد الانتظار)';
            label += statusSuffix;
            let method = methodLabels[tx.method] || tx.method || '—';
            // Show sent/received details for send_balance
            if (tx.method === 'send_balance' && tx.transfer_id) {
              const match = tx.transfer_id.match(/^(to|from):(.+?)(?:\(\d+\))?$/);
              if (match) {
                method = match[1] === 'to' ? `إرسال إلى ${match[2]}` : `استلام من ${match[2]}`;
              }
            }
            const dateStr = tx.created_at ? new Date(tx.created_at).toLocaleString('sv-SE', { timeZone: getBotTimezone(), year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' }).replace(',', '') : '—';
    ...
    if (data === 'ichancy') {
      ...
      delete userState[chatId]; // Clear any pending send_balance or other state
      ...
      if (user && !user.ichancy_user_id) {
        const resolvedId = await ensureIchancyUserId(user, query.from.id);
        if (resolvedId) user.ichancy_user_id = resolvedId;
      }
    ...
    if (data === 'send_balance') {
      debugLog('callback_query: executing send_balance');
      if (!FLAG_SEND_BALANCE) {
        await bot.answerCallbackQuery(query.id, { text: '❌ هذه الميزة غير مفعّلة حالياً.' });
        return;
      }
      try {
        ...
        userState[chatId] = { step: 'await_send_balance_recipient', messageId };
        await bot.editMessageText(
          `🎁 إهداء رصيد\n\n💰 رصيدك الحالي: <code>${formatNumber(balance)}</code> ل.س\n\nأرسل اسم المستخدم أو رقم تيليغرام الشخص الذي تريد إرسال الرصيد إليه:`,
          {
            chat_id: chatId,
            message_id: messageId,
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: [[{ text: '🔙 العودة', callback_data: 'ichancy' }]] },
          }
        );
      } catch (err) {
        console.warn('send_balance:', err.message);
        await bot.answerCallbackQuery(query.id, { text: 'خطأ، حاول مرة أخرى' });
      }
      return;
    }
    ...
    if (data === 'wallet') {
      if ((process.env.HIDE_WALLET_BUTTON || '').split(',').map(s => s.trim()).filter(Boolean).includes(BOT_ID)) return;
    ...
    if (data === 'transfer_to_ichancy') {
      ...
      if (!user.ichancy_user_id) {
        await bot.editMessageText(LOADING_TEXT, { chat_id: chatId, message_id: messageId, reply_markup: { inline_keyboard: [] } });
        const resolvedId = await ensureIchancyUserId(user, query.from.id);
        if (!resolvedId) {
          await bot.editMessageText('❌ تعذر ربط حسابك بموقع Ichancy. حاول لاحقاً أو تواصل مع الدعم.', {
            chat_id: chatId, message_id: messageId, ...ichancyAccountKeyboard(),
          });
          return;
        }
        user.ichancy_user_id = resolvedId;
      }
    ...
    if (data === 'transfer_full_to_ichancy') {
      ...
      const rawErr = (notif && notif.content) || '';
      if (/Total Available\(FROM\)/i.test(rawErr)) {
        await bot.editMessageText('فشل الشحن بسبب الصيانة ❌️\n\nحاول بعد قليل .. شكراً لتفهمك ♥️', {
          chat_id: chatId,
          message_id: messageId,
          ...ichancyAccountKeyboard(),
        });
      ...
      const errMsg = rawErr || 'فشل التحويل. حاول لاحقاً.';
    ...
    if (data.startsWith('oxapay_wd_reject_') && isAdminUser(query.from)) {
      ...
      // Refund balance
      const oxaRefundAmount = Number(pending.amount_syp);
      await adjustBalance(pending.telegram_user_id, { balanceDelta: oxaRefundAmount });
      ...
      logTransaction({
        telegramUserId: pending.telegram_user_id,
        type: 'deposit',
        amount: oxaRefundAmount,
        method: 'refund',
        status: 'confirmed',
        transferId: pending.transaction_id ? `ref:${pending.transaction_id}` : null,
      }).catch(e => console.warn('logTransaction oxapay_refund:', e.message));
    ...
    if (data === 'referrals') {
      ...
      const uid = query.from.id;
      let commData = { totalCommission: 0, levels: [], referralCount: 0 };
      let hasCustomPercent = false;
      try {
        const userRow = await getUserByTelegramId(uid);
        const effectivePercents = [...REFERRAL_PERCENTS];
        if (userRow?.custom_referral_percent != null) {
          effectivePercents[0] = userRow.custom_referral_percent;
          hasCustomPercent = true;
        }
      ...
      for (const l of commData.levels) {
        if (l.percent > 0) {
          const pctLabel = (l.level === 1 && hasCustomPercent) ? 'نسبة وكيل%' : `${l.percent}%`;
          levelLines += `\n▫️ المستوى ${l.level} (${pctLabel}): ${formatNumber(l.commission)} ل.س`;
        }
      }
    ...
    if (data.startsWith('txlog_')) {
      ...
      if (result.rows.length === 0) {
        ...
        const lines = result.rows.map((tx, i) => {
          ...
          const dateStr = d.toLocaleString('sv-SE', { timeZone: getBotTimezone(), year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }).replace(',', '');
          let method = methodLabel[tx.method] || tx.method;
          // Show sent/received details for send_balance
          if (tx.method === 'send_balance' && tx.transfer_id) {
            const match = tx.transfer_id.match(/^(to|from):(.+?)(?:\(\d+\))?$/);
            if (match) {
              method = match[1] === 'to' ? `🎁 إرسال رصيد إلى ${escapeHtml(match[2])}` : `🎁 استلام رصيد من ${escapeHtml(match[2])}`;
            }
          }
          const txId = (tx.transfer_id && tx.method !== 'send_balance' && tx.method !== 'refund') ? `\n   🔖 رقم العملية: <code>${escapeHtml(tx.transfer_id)}</code>` : '';
          const statusIcon = tx.status === 'confirmed' ? '✅' : tx.status === 'rejected' ? '❌' : '⏳';
          const statusText = tx.status === 'confirmed' ? '( ناجح)' : tx.status === 'rejected' ? '(فاشل)' : '(قيد الانتظار)';
          const txTypeLabel = tx.type === 'deposit' ? 'إيداع' : 'سحب';
          return `${num}. ${statusIcon} ${txTypeLabel} ${statusText} <code>${formatNumber(tx.amount)}</code> ل.س — ${method}\n   📅 ${dateStr}${txId}`;
        });
    ...
    if (data === 'support') {
      ...
      const supportBtns = buildSupportButtons();
      const buttons = [...supportBtns];
      buttons.push([{ text: '🔙 العودة للقائمة الرئيسية', callback_data: 'support_back' }]);
      const msgText = supportBtns.length > 0
        ? 'لأي سؤال أو مشكلة، الرجاء التواصل مع فريق الدعم عبر الضغط على الزر أدناه.'
        : '⚠️ الدعم غير متاح حالياً. يرجى المحاولة لاحقاً.';
    ...
    if (data === 'box_game') {
      await bot.answerCallbackQuery(query.id).catch(() => {});
      if (process.env.HIDE_BOXES === 'true') {
        await bot.sendMessage(chatId, '❌ لعبة الصناديق غير متاحة حالياً.');
        return;
      }
    ...
    if (data === 'box_play_now') {
      ...
      // Weighted random box selection based on percentage weights
      const prizes = await getLuckBoxPrizes();
      ...
      if (totalWeight > 0) {
        ...
        for (let i = 0; i < prizes.length; i++) {
          cumulative += (Number(prizes[i].weight) || 0);
          if (rand < cumulative) {
            boxIndex = i;
            break;
          }
        }
        if (boxIndex === -1) boxIndex = prizes.length - 1;
      } else {
        // If no weights configured, pick randomly
        boxIndex = Math.floor(Math.random() * prizes.length);
      }
      const box = prizes[boxIndex] || { amount: 0, weight: 0 };
      const amount = Number(box.amount) || 0;
      const now = new Date();
      // Set cooldown immediately to prevent re-rolling exploits
      await createOrUpdateUser(userId, { last_box_game_at: now });
      if (amount <= 0) {
        await bot.sendMessage(chatId, `📦 فتحت الصندوق ${boxIndex + 1}... الصندوق فارغ، حظًا أوفر في المرة القادمة!`);
        return;
      }
      ...
      await bot.sendMessage(chatId, `🎉 مبروك! فتحت الصندوق ${boxIndex + 1} وفزت بـ ${amountFormatted}.`);
      return;
    }
    ...
    if (data.startsWith('sham_withdraw_cancel_')) {
      ...
      if ((!shamListR || shamListR.length === 0) && (!syrListR || syrListR.length === 0)) {
        ...
        const timezone = (await getConfigValue('timezone', 'Asia/Damascus')) || 'Asia/Damascus';
        const formatDate = (d) => { try { return new Date(d).toLocaleString('ar-SY', { timeZone: timezone, dateStyle: 'short', timeStyle: 'short' }); } catch (_) { return new Date(d).toLocaleString('sv-SE', { timeZone: getBotTimezone() }); } };
        let msg = '📋 طلبات السحب المعلقة:\n';
        const rows = [];
    ...
    if (e && e.message && e.message.includes('message is not modified')) return;
...
bot.on('message', async (msg) => {
  ...
  if (state.step === 'await_admin_support_username') {
    ...
    try {
      ...
      delete userState[chatId];
      await bot.sendMessage(chatId, '✅ تم تحديث اسم مستخدم الدعم.\n\n' + adminSupportTelegramMessage(), { parse_mode: 'HTML', ...adminSupportTelegramKeyboard() });
    } catch (err) {
  ...
  if (state.step === 'await_admin_support_whatsapp') {
    if (!isAdminUser(msg.from)) {
      delete userState[chatId];
      return;
    }
    const cleaned = cleanWhatsAppNumber(text.trim());
    if (!/^\d{10,15}$/.test(cleaned)) {
      return bot.sendMessage(chatId, '❌ رقم غير صالح. أدخل رقماً صحيحاً مع رمز الدولة (10-15 رقماً). مثال: +963912345678');
    }
    try {
      await setConfigValue('SUPPORT_WHATSAPP_NUMBER', cleaned);
      SUPPORT_WHATSAPP_NUMBER = cleaned;
      delete userState[chatId];
      await bot.sendMessage(chatId, '✅ تم تحديث رقم واتساب الدعم.\n\n' + adminSupportWhatsappMessage(), { parse_mode: 'HTML', ...adminSupportWhatsappKeyboard() });
    } catch (err) {
      console.warn('setConfigValue SUPPORT_WHATSAPP_NUMBER:', err.message);
      await bot.sendMessage(chatId, '❌ حدث خطأ أثناء التحديث.');
    }
    return;
  }
  ...
  if (state.step === 'await_admin_user_add_money') {
    ...
    try {
      ...
      const updated = await adjustBalance(telegramUserId, { balanceDelta: roundedAmt });
      if (!updated) {
        return bot.sendMessage(chatId, '❌ حدث خطأ في تحديث الرصيد.');
      }
      try {
        await logTransaction({ telegramUserId, type: 'deposit', amount: roundedAmt, method: 'admin', transferId: null, status: 'confirmed' });
      } catch (e) { console.warn('logTransaction admin add:', e.message); }
...
✅ تمت العملية بنجاح.`;
  ...
  if (state.step === 'await_admin_user_deduct_money') {
    ...
    try {
      ...
      try {
        await logTransaction({ telegramUserId, type: 'withdrawal', amount: roundedAmount, method: 'admin', transferId: null, status: 'confirmed' });
      } catch (e) { console.warn('logTransaction admin deduct:', e.message); }
      const dateStr = new Date().toLocaleString('sv-SE', { timeZone: getBotTimezone() });
      const amountFormatted = formatCurrencySyp(amount);
      const deductMsg = `🟥 <b>تم سحب رصيد من حسابك</b>
...
✅ تمت العملية بنجاح.`;
      await bot.sendMessage(telegramUserId, deductMsg, { parse_mode: 'HTML' }).catch((err) => console.warn('admin deduct money: send to user:', err?.message));
...
${escapeHtml(adminMessage)}`;
  ...
  if (state.step === 'await_admin_ich_alert_threshold') {
    if (!isAdminUser(msg.from)) { delete userState[chatId]; return; }
    const num = parseFloat(String(text).replace(/,/g, '.').trim());
    if (!Number.isFinite(num) || num <= 0) {
      return bot.sendMessage(chatId, '❌ يرجى إدخال رقم صحيح أكبر من صفر.');
    ...
    try {
      await setConfigValue('ICHANCY_LOW_BALANCE_THRESHOLD', num);
      await loadLocalConfig();
      delete userState[chatId];
      await bot.sendMessage(chatId, `✅ تم تحديث حد تنبيه iChancy إلى ${formatNumber(num)} ل.س`);
      await bot.editMessageText(adminIchancyAlertsMessage(), {
        chat_id: chatId,
        message_id: state.messageId,
        ...adminIchancyAlertsKeyboard(),
      ...
      console.warn('setConfigValue ICHANCY_LOW_BALANCE_THRESHOLD:', err.message);
      await bot.sendMessage(chatId, '❌ حدث خطأ أثناء التحديث.');
    }
    return;
  }
  ...
  if (state.step === 'await_admin_rates_single') {
    ...
    const isUsdField = field === 'min_deposit_usd' || field === 'max_cashout_usd' || field === 'min_cashout_usd';
    const num = (isPercent || isUsdField) ? parseFloat(text) : parseInt(text, 10);
    ...
    if (isUsdField && num <= 0) {
      return bot.sendMessage(chatId, '❌ القيمة يجب أن تكون أكبر من صفر.');
    }
    if (!isPercent && !isUsdField && num < 0) {
      return bot.sendMessage(chatId, '❌ القيمة يجب أن تكون رقماً موجباً.');
    }
    const value = (isPercent || isUsdField) ? num : Math.round(num);
    try {
  ...
  if (state.step === 'await_password') {
    ...
    try {
      ...
      getAgentSession().then(c => silentCheckIchancyBalance(c)).catch(() => {});
      ...
      if (result.registerOk) {
        ...
        if (pendingRef) {
          try {
            ...
            if (saved) {
              debugLog('Applied pending referral after registration', { userId: msg.from.id, referrerId: pendingRef });
              notifyReferralChain(msg.from.id, displayUsername);
            }
          } catch (refErr) {
  ...
  if (state.step === 'await_transfer_amount') {
    ...
    if (!user.ichancy_user_id) {
      await bot.sendMessage(chatId, '⏳ جاري ربط حسابك...');
      const resolvedId = await ensureIchancyUserId(user, msg.from.id);
      if (!resolvedId) {
        delete userState[chatId];
        return bot.sendMessage(chatId, '❌ تعذر ربط حسابك بموقع Ichancy. حاول لاحقاً.');
      }
      user.ichancy_user_id = resolvedId;
    }
    const botBalance = Number(user.balance ?? 0);
    ...
    const rawErr = (notif && notif.content) || '';
    if (/Total Available\(FROM\)/i.test(rawErr)) {
      return bot.sendMessage(chatId, 'فشل الشحن بسبب الصيانة ❌️\n\nحاول بعد قليل .. شكراً لتفهمك ♥️');
    }
    const errMsg = rawErr || 'فشل التحويل. حاول لاحقاً.';
  ...
  // Send balance: user sent recipient identifier
  if (state.step === 'await_send_balance_recipient') {
    debugLog('message: handling await_send_balance_recipient', { text });
    if (!FLAG_SEND_BALANCE) {
      delete userState[chatId];
      return bot.sendMessage(chatId, '❌ هذه الميزة غير مفعّلة حالياً.');
    }
    if (/إلغاء|cancel/i.test(text)) {
      delete userState[chatId];
      return bot.sendMessage(chatId, 'تم إلغاء إهداء الرصيد.');
    }
    let recipient = null;
    try {
      recipient = await findUserForTransfer(text);
    } catch (err) {
      console.warn('findUserForTransfer:', err.message);
    }
    if (!recipient) {
      return bot.sendMessage(chatId, '❌ لم يتم العثور على المستخدم. تأكد من اسم المستخدم أو رقم تيليغرام وحاول مرة أخرى.');
    }
    if (String(recipient.telegram_user_id) === String(msg.from.id)) {
      return bot.sendMessage(chatId, '❌ لا يمكنك إرسال رصيد لنفسك.');
    }
    const recipientBlocked = await isUserBlocked(recipient.telegram_user_id, recipient.telegram_username);
    if (recipientBlocked) {
      return bot.sendMessage(chatId, '❌ لا يمكن إرسال رصيد لهذا المستخدم.');
    }
    const sender = await getUserByTelegramId(msg.from.id);
    const senderBalance = Number(sender?.balance ?? 0);
    // Show the matched identifier the user searched for, not always telegram_username
    const searchInput = String(text).trim().replace(/^@/, '').toLowerCase();
    let recipientDisplayName;
    if (recipient.ichancy_login && recipient.ichancy_login.toLowerCase() === searchInput) {
      recipientDisplayName = recipient.ichancy_login;
    } else if (recipient.telegram_username && recipient.telegram_username.toLowerCase() === searchInput) {
      recipientDisplayName = recipient.telegram_username;
    } else {
      recipientDisplayName = recipient.telegram_username || recipient.ichancy_login || String(recipient.telegram_user_id);
    }
    userState[chatId] = {
      step: 'await_send_balance_amount',
      recipientTelegramId: recipient.telegram_user_id,
      recipientUsername: recipientDisplayName,
      messageId: state.messageId,
    };
    return bot.sendMessage(chatId,
      `🎁 إهداء رصيد إلى: <b>${escapeHtml(recipientDisplayName)}</b>\n\n💰 رصيدك الحالي: <code>${formatNumber(senderBalance)}</code> ل.س\n\nأرسل المبلغ الذي تريد إرساله:`,
      { parse_mode: 'HTML' }
    );
  }

  // Send balance: user sent amount
  if (state.step === 'await_send_balance_amount') {
    debugLog('message: handling await_send_balance_amount', { text });
    if (!FLAG_SEND_BALANCE) {
      delete userState[chatId];
      return bot.sendMessage(chatId, '❌ هذه الميزة غير مفعّلة حالياً.');
    }
    ...
    const senderBalance = Number(sender.balance ?? 0);
    // Round to 2 decimal places to match DB DECIMAL(18,2)
    const roundedAmount = Math.round(amount * 100) / 100;
    if (roundedAmount < 1) {
      return bot.sendMessage(chatId, '❌ الحد الأدنى للتحويل هو 1 ل.س.');
    }
    if (roundedAmount > senderBalance) {
      return bot.sendMessage(chatId, `❌ رصيدك غير كافٍ. رصيدك الحالي: <code>${formatNumber(senderBalance)}</code> ل.س`, { parse_mode: 'HTML' });
    }
    // Deduct from sender atomically
    const updatedSender = await adjustBalance(msg.from.id, { balanceDelta: -roundedAmount });
    if (!updatedSender) {
      return bot.sendMessage(chatId, `❌ رصيدك غير كافٍ لإتمام العملية.`);
    }
    // Add to recipient
    const updatedRecipient = await adjustBalance(state.recipientTelegramId, { balanceDelta: roundedAmount, giftsDelta: roundedAmount });
    if (!updatedRecipient) {
      // Rollback sender deduction
      try {
        await adjustBalance(msg.from.id, { balanceDelta: roundedAmount });
      } catch (rollbackErr) {
        console.error('CRITICAL: send_balance rollback failed! Sender:', msg.from.id, 'Amount:', roundedAmount, 'Error:', rollbackErr.message);
      }
      delete userState[chatId];
      return bot.sendMessage(chatId, '❌ فشل إضافة الرصيد للمستلم. تم إلغاء العملية.');
    }
    const recipientUsername = state.recipientUsername;
    const recipientTelegramId = state.recipientTelegramId;
    const senderDisplayName = sender.ichancy_login || sender.telegram_username || sender.first_name || String(msg.from.id);
    // Build transferId with proper format
    const senderTag = sender.telegram_username ? `@${sender.telegram_username}` : senderDisplayName;
    const recipientTag = recipientUsername.startsWith('@') ? recipientUsername : (recipientUsername.match(/^\d+$/) ? recipientUsername : `@${recipientUsername}`);
    ...
    logTransaction({
      ...
      method: 'send_balance',
      status: 'confirmed',
      transferId: `to:${recipientTag}(${recipientTelegramId})`,
    }).catch(e => console.warn('logTransaction send_balance sender:', e.message));
    logTransaction({
      telegramUserId: recipientTelegramId,
      type: 'deposit',
      amount: roundedAmount,
      method: 'send_balance',
      status: 'confirmed',
      transferId: `from:${senderTag}(${msg.from.id})`,
    }).catch(e => console.warn('logTransaction send_balance recipient:', e.message));
    delete userState[chatId];
    // Notify recipient
    bot.sendMessage(recipientTelegramId,
      `🎁 لقد استلمت <code>${formatNumber(roundedAmount)}</code> ل.س من <b>${escapeHtml(senderDisplayName)}</b>`,
      { parse_mode: 'HTML' }
    ).catch(e => console.warn('send_balance notify recipient:', e.message));
    // Confirm to sender
    return bot.sendMessage(chatId,
      `✅ تم تحويل <code>${formatNumber(roundedAmount)}</code> ل.س بنجاح إلى <b>${escapeHtml(recipientUsername)}</b>`,
      { parse_mode: 'HTML' }
    );
  }
  ...
  if (state.step === 'await_sham_usd_client_code') {
    ...
    const { shamcash: sham } = await getRatesForPayment();
    userState[chatId] = { step: 'await_sham_usd_amount', clientCode: text.replace(/\s+/g, ''), messageId: state.messageId };
  ...
  if (state.step === 'await_sham_syp_client_code') {
    ...
    const sypMinFormatted = formatNumber(sham.min_cashout_syp);
    const sypMaxFormatted = formatNumber(sham.max_cashout_syp);
    userState[chatId] = { step: 'await_sham_syp_amount', clientCode: text.replace(/\s+/g, ''), messageId: state.messageId };
  ...
  if (state.step === 'await_syriatel_amount') {
    ...
    try {
      ...
      if (existingPending.length > 0) {
        console.log(`[SyriatelCashout] BLOCKED — user ${msg.from.id} has ${existingPending.length} existing pending/partial withdrawal(s): ${JSON.stringify(existingPending.map(p => ({ id: p.id, status: p.status, amount: p.amount_new_currency, phone: p.phone, created_at: p.created_at, failure_reason: p.failure_reason })))}`);
        return bot.sendMessage(chatId, '⏳ لديك طلب سحب قيد المعالجة. يرجى الانتظار حتى اكتماله.');
      }
    } catch (pendingErr) {
      console.log(`[SyriatelCashout] getSyriatelPendingByUser ERROR for user ${msg.from.id}: ${pendingErr.message}`);
    }

    // Race condition guard
    if (syriatelCashoutInProgress.has(msg.from.id)) {
      console.log(`[SyriatelCashout] BLOCKED — user ${msg.from.id} already in syriatelCashoutInProgress set (race guard)`);
      return bot.sendMessage(chatId, '⏳ يتم معالجة طلب سحب سابق. يرجى الانتظار.');
    }
    syriatelCashoutInProgress.add(msg.from.id);
    console.log(`[SyriatelCashout] user ${msg.from.id} requesting withdrawal: amount=${amount} SYP, phone=${phone.trim()}, afterTax will be calculated next`);
    ...
    try {
      ...
      // Run smart cashout
      console.log(`[SyriatelCashout] calling smartSyriatelCashout: phone=${phone.trim()} amountToSendViaApi=${amountToSendViaApi} (original=${amount} SYP, afterTax=${afterTax}, OLD_CURRENCY_MULTIPLE=${OLD_CURRENCY_MULTIPLE}) pendingId=${pending.id}`);
      const result = await smartSyriatelCashout({ phone: phone.trim(), amountNewCurrency: amountToSendViaApi });
      console.log(`[SyriatelCashout] smartSyriatelCashout RETURNED: ${JSON.stringify({ success: result.success, fullSuccess: result.fullSuccess, partial: result.partial, totalSent: result.totalSent, remaining: result.remaining, failureReason: result.failureReason })} pendingId=${pending.id}`);
      ...
      console.log(`[SyriatelCashout] TOTAL FAILURE — pendingId=${pending.id} failureReason=${result.failureReason} — sending to admin for manual retry`);
  ...
  if (state.step === 'await_charge_syriatel_transfer_id') {
    ...
    alertTransaction(msg.from, 'deposit', creditAmountBase, 'syriatel', transferId);
  ...
  if (state.step === 'await_gift_code') {
    ...
    if (!result.error) {
      logTransaction({
        ...
        method: 'gift_code',
        status: 'confirmed',
        transferId: code.toUpperCase(),
      }).catch(e => console.warn('logTransaction gift_code:', e.message));
    }
    if (result.error) {
      let errMsg;
      if (result.error === 'empty') {
        ...
        errMsg = '❌ لقد قمت باستخدام هذا الكود مسبقاً.\nلا يمكن استخدام نفس الكود أكثر من مرة.';
      } else if (result.error === 'batch_used') {
        errMsg = '❌ لقد استفدت من كود هدية في هذه الدفعة بالفعل.\nيُسمح لكل مستخدم باستخدام كود واحد فقط من كل دفعة.\nترقّب الدفعة القادمة! 🎁';
      } else {
  ...
  if (state.step === 'await_charge_sham_usd_amount') {
    ...
    return sendShamcashQR(chatId, msg, shamCode, chargeShamUsdTransferCancelKeyboard());
  }
  ...
  if (state.step === 'await_charge_sham_usd_transfer_id') {
    ...
    // Custom alert for sham_usd deposits: show USD amount + exchange rate
    if (ALERT_CHANNEL_TRANSACTIONS) {
      const aName = [msg.from.first_name, msg.from.last_name].filter(Boolean).join(' ') || '—';
      const aMsg = `📥 إيداع شام كاش (ل.س)\n\n👤 ${escapeHtml(aName)} (<code>${msg.from.id}</code>)\n🆔 ${msg.from.username ? `@${escapeHtml(msg.from.username)}` : 'N/A'}\n💰  مبلغ الإيداع بالدولار : ${apiAmount}$\n🟥 سعر الصرف الحالي : ${formatNumber(exchangeRate)}\n💵 مبلغ الايداع بالعملة القديمة : <code>${formatNumber(creditAmountBase)}</code> ل.س\n🔖 رقم العملية: <code>${escapeHtml(transferId)}</code>`;
      bot.sendMessage(ALERT_CHANNEL_TRANSACTIONS, aMsg, { parse_mode: 'HTML' }).catch((err) =>
        console.warn('alertTransaction sham_usd:', err.message)
      );
    }
    const amountFormatted = formatNumber(creditAmount);
    const updatedBalance = formatNumber(Number(updated.balance ?? 0));
  ...
  if (state.step === 'await_charge_sham_syp_amount') {
    ...
    return sendShamcashQR(chatId, msg, shamCode, chargeShamSypTransferCancelKeyboard());
  }
  ...
  if (state.step === 'await_charge_sham_syp_transfer_id') {
    ...
    alertTransaction(msg.from, 'deposit', creditAmountBase, 'sham_syp', transferId);
    const amountFormatted = formatNumber(creditAmount);
    const updatedBalance = formatNumber(Number(updated.balance ?? 0));
    const bonusNote = bonus > 0 ? ` (يشمل بونص إيداع ${sham.deposit_bonus_percent}%)` : '';
    return bot.sendMessage(
...
bot.on('polling_error', (err) => {
  const desc = err?.response?.body?.description || '';
  if (desc.includes('query is too old') || desc.includes('query ID is invalid')) {
    console.warn(`[Bot:${BOT_ID}] Ignoring Telegram stale query polling error`);
    return;
  }
  console.error(`[Bot:${BOT_ID}] Polling error:`, err.message);
});
...
const CONFIG_DEFAULTS = {
  ...
  ICHANCY_PARENT_ID: '',
  EXCHANGE_RATE_SYP_PER_USD: 11500,
...
async function loadLocalConfig() {
  ...
  FLAG_SEND_BALANCE = !!(await getConfigValue('SEND_BALANCE_ENABLED', false));
  ...
  SUPPORT_USERNAME = await getConfigValue('SUPPORT_USERNAME', '');
  SUPPORT_WHATSAPP_NUMBER = await getConfigValue('SUPPORT_WHATSAPP_NUMBER', '');
  SUPPORT_TELEGRAM_ENABLED = (await getConfigValue('SUPPORT_TELEGRAM_ENABLED', 'false')) === 'true';
  SUPPORT_WHATSAPP_ENABLED = (await getConfigValue('SUPPORT_WHATSAPP_ENABLED', 'false')) === 'true';
  // Auto-disable if no account is set
  if (SUPPORT_TELEGRAM_ENABLED && !(SUPPORT_USERNAME || '').trim()) SUPPORT_TELEGRAM_ENABLED = false;
  if (SUPPORT_WHATSAPP_ENABLED && !(SUPPORT_WHATSAPP_NUMBER || '').trim()) SUPPORT_WHATSAPP_ENABLED = false;
  ...
  EXCHANGE_RATE_SYP_PER_USD = await cfgFloat('EXCHANGE_RATE_SYP_PER_USD', 11500);
  ...
  SYRIATEL_LOW_BALANCE_THRESHOLD = Number(await getConfigValue('SYRIATEL_LOW_BALANCE_THRESHOLD', 1500)) || 1500;
  ...
  // iChancy alert settings
  FLAG_ICHANCY_LOW_BALANCE_ALERT = !!(await getConfigValue('ICHANCY_LOW_BALANCE_ALERT_ENABLED', true));
  ICHANCY_LOW_BALANCE_THRESHOLD = Number(await getConfigValue('ICHANCY_LOW_BALANCE_THRESHOLD', 20000)) || 20000;
}
...
async function start(options = {}) {
  ...
  // Debug: log token info and verify with getMe before polling
  const cleanToken = token.trim();
  console.log(`[Bot:${BOT_ID}] Token length: ${cleanToken.length}, value: ${cleanToken.slice(0, 6)}...${cleanToken.slice(-6)}`);
  bot = new TelegramBot(cleanToken, { polling: false });
  try {
    const me = await bot.getMe();
    console.log(`[Bot:${BOT_ID}] getMe OK: @${me.username}`);
  } catch (meErr) {
    console.error(`[Bot:${BOT_ID}] getMe FAILED: ${meErr.message} — token is likely wrong!`);
    return false;
  }
  ...
  if (options.webhookDomain && options.webhookPath) {
    ...
    // Clear any stale webhook before polling — otherwise Telegram returns 404
