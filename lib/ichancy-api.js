...
async function signIn(username, password) {
  ...
  const payloadForLog = { username, password: password ? '***' : '' };
  ...
  const success = res.status === 200 && data && data.status === true;
  ...
  if (_apiDebugLogs) {
    ...
    debugLog('signIn: got response', { status: res.status, success, cookies: cookies ? 'present' : 'none' });
  }
...
function createApiClient(config = {}) {
  ...
  configureApi({ debugLogs, cookieRefreshMinutes: config.cookieRefreshMinutes, agentUsername: config.agentUsername, agentPassword: config.agentPassword, parentId: config.parentId });
