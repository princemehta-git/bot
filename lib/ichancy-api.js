...
async function signIn(username, password) {
  ...
  debugLog('signIn: got response', { status: res.status, success: !!(data && data.status === true), cookies: cookies ? 'present' : 'none' });
 ...
 * @param {string} cookies - From signIn()
 * @param {string} playerLogin - e.g. "playeridtest-Bot"
...
async function depositToPlayer(cookies, playerId, amount, opts = {}) {
  ...
  const success = status === 200 && data && data.status === true && data.result === true;
  debugLog('depositToPlayer: got result', { success, status });
  return {
    success,
    data,
...
async function withdrawFromPlayer(cookies, playerId, amount, opts = {}) {
  ...
  const success = status === 200 && data && data.status === true && data.result === true;
  debugLog('withdrawFromPlayer: got result', { success, status });
  return {
    success,
    data,
    notification: data && data.notification,
