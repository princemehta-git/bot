...
let OxapayPendingInvoice;
let OxapayPendingWithdrawal;
let SyriatelPendingWithdrawal;
...
async function initDb() {
  ...
  User = sequelize.define('User', {
    ...
    spins_granted_today: { type: DataTypes.INTEGER.UNSIGNED, allowNull: false, defaultValue: 0 },
  ...
  GiftCode = sequelize.define('GiftCode', {
    ...
    publish_batch_id: { type: DataTypes.INTEGER.UNSIGNED, allowNull: true, defaultValue: null },
  }, {
  ...
  Transaction = sequelize.define('Transaction', {
    ...
    bonus_amount: { type: DataTypes.DECIMAL(18, 2), allowNull: false, defaultValue: 0 },
  ...
  PaymentProvider = sequelize.define('PaymentProvider', {
    ...
    provider_name: { type: DataTypes.STRING(64), allowNull: false },
    ...
    min_deposit_usd: { type: DataTypes.FLOAT, allowNull: true, defaultValue: null },
    max_cashout_usd: { type: DataTypes.FLOAT, allowNull: true, defaultValue: null },
    min_cashout_usd: { type: DataTypes.FLOAT, allowNull: true, defaultValue: null },
  }, {
  ...
  ShamcashPendingWithdrawal = sequelize.define('ShamcashPendingWithdrawal', {
    ...
    channel_message_id: { type: DataTypes.BIGINT, allowNull: true },
  }, {
  ...
  OxapayPendingInvoice = sequelize.define('OxapayPendingInvoice', {
    id: { type: DataTypes.INTEGER.UNSIGNED, autoIncrement: true, primaryKey: true },
    bot_id: { type: DataTypes.STRING(128), allowNull: false },
    track_id: { type: DataTypes.STRING(128), allowNull: false },
    telegram_user_id: { type: DataTypes.BIGINT, allowNull: false },
    coin: { type: DataTypes.STRING(32), allowNull: false },
    coin_display: { type: DataTypes.STRING(32), allowNull: true },
    amount_usd: { type: DataTypes.DECIMAL(18, 8), allowNull: false },
    amount_coin: { type: DataTypes.DECIMAL(18, 8), allowNull: false },
    amount_syp_credited: { type: DataTypes.DECIMAL(18, 2), allowNull: true, defaultValue: null },
    status: { type: DataTypes.ENUM('pending', 'paid', 'expired', 'cancelled'), allowNull: false, defaultValue: 'pending' },
    polling_count: { type: DataTypes.INTEGER.UNSIGNED, allowNull: false, defaultValue: 0 },
  }, {
    tableName: 'oxapay_pending_invoices',
    timestamps: true,
    updatedAt: false,
    createdAt: 'created_at',
    indexes: [
      { unique: true, fields: ['bot_id', 'track_id'] },
      { fields: ['bot_id', 'telegram_user_id', 'status'] },
    ],
  });

  OxapayPendingWithdrawal = sequelize.define('OxapayPendingWithdrawal', {
    id: { type: DataTypes.INTEGER.UNSIGNED, autoIncrement: true, primaryKey: true },
    bot_id: { type: DataTypes.STRING(128), allowNull: false },
    telegram_user_id: { type: DataTypes.BIGINT, allowNull: false },
    amount_syp: { type: DataTypes.DECIMAL(18, 2), allowNull: false },
    ...
    tableName: 'oxapay_pending_withdrawals',
    timestamps: true,
    updatedAt: false,
    createdAt: 'created_at',
    indexes: [
  ...
  SyriatelPendingWithdrawal = sequelize.define('SyriatelPendingWithdrawal', {
    id: { type: DataTypes.INTEGER.UNSIGNED, autoIncrement: true, primaryKey: true },
    bot_id: { type: DataTypes.STRING(128), allowNull: false },
    telegram_user_id: { type: DataTypes.BIGINT, allowNull: false },
    amount_syp: { type: DataTypes.DECIMAL(18, 2), allowNull: false },
    amount_new_currency: { type: DataTypes.DECIMAL(18, 2), allowNull: false },
    phone: { type: DataTypes.STRING(64), allowNull: false },
    amount_sent: { type: DataTypes.DECIMAL(18, 2), allowNull: false, defaultValue: 0 },
    status: { type: DataTypes.ENUM('pending', 'completed', 'partial', 'failed', 'ignored', 'cancelled'), allowNull: false, defaultValue: 'pending' },
    failure_reason: { type: DataTypes.STRING(64), allowNull: true },
    admin_message_id: { type: DataTypes.STRING(64), allowNull: true },
    admin_chat_id: { type: DataTypes.STRING(64), allowNull: true },
    transaction_id: { type: DataTypes.INTEGER.UNSIGNED, allowNull: true },
    resolved_at: { type: DataTypes.DATE, allowNull: true },
    resolved_by: { type: DataTypes.STRING(64), allowNull: true },
  }, {
    tableName: 'syriatel_pending_withdrawals',
    timestamps: true,
    updatedAt: false,
    createdAt: 'created_at',
    indexes: [
  ...
  Bot = sequelize.define('Bot', {
    ...
    exchange_rate_syp_per_usd: { type: DataTypes.FLOAT, allowNull: false, defaultValue: 11500 },
    ...
    support_whatsapp_number: { type: DataTypes.STRING(32), allowNull: true },
    support_telegram_enabled: { type: DataTypes.STRING(8), allowNull: true, defaultValue: 'false' },
    support_whatsapp_enabled: { type: DataTypes.STRING(8), allowNull: true, defaultValue: 'false' },
    ...
    oxapay_merchant_api_key: { type: DataTypes.STRING(255), allowNull: true },
    deposit_oxapay_enabled: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
    withdraw_oxapay_enabled: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
    blocked_users: { type: DataTypes.JSON, allowNull: true, defaultValue: [] },
    spin_prizes: { type: DataTypes.JSON, allowNull: true, defaultValue: [{ text: 'حظ أوفر', weight: 83 }, { text: '💰 50000', weight: 1 }, { text: '💎 10000', weight: 8 }, { text: '💲 5000', weight: 8 }] },
    luck_box_prizes: { type: DataTypes.JSON, allowNull: true, defaultValue: [{ amount: 0, weight: 0 }, { amount: 0, weight: 0 }, { amount: 0, weight: 0 }] },
    syriatel_low_balance_alert_enabled: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
    syriatel_low_balance_threshold: { type: DataTypes.DECIMAL(18, 2), allowNull: false, defaultValue: 1500 },
    syriatel_tx_fail_balance_alert_enabled: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: true },
    ichancy_low_balance_alert_enabled: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: true },
    ichancy_low_balance_threshold: { type: DataTypes.DECIMAL(18, 2), allowNull: false, defaultValue: 20000 },
    semi_shamcash_automatic: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
    is_hide_shamcash_identity: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: true },
    send_balance_enabled: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
  }, { tableName: 'bots', indexes: [{ unique: true, fields: ['bot_id'] }] });

  // ── Migrations ──────────────────────────────────────────────────────

  await migratePaymentProvidersFromBotsIfNeeded();
  await migrateProviderNameEnumToVarchar();
...
/**
 * One-time: convert payment_providers.provider_name from ENUM to VARCHAR(64)
 * so that 'oxapay' can be inserted without altering the ENUM definition.
 */
async function migrateProviderNameEnumToVarchar() {
  try {
    const [cols] = await sequelize.query(
      "SELECT DATA_TYPE, COLUMN_TYPE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'payment_providers' AND COLUMN_NAME = 'provider_name'",
      { replacements: [database] }
    );
    if (!cols || cols.length === 0) return; // table not created yet
    const dataType = (cols[0].DATA_TYPE || '').toLowerCase();
    if (dataType === 'varchar') return; // already varchar, nothing to do
    // Change ENUM to VARCHAR(64)
    await sequelize.query(
      'ALTER TABLE payment_providers MODIFY COLUMN provider_name VARCHAR(64) NOT NULL'
    );
  } catch (err) {
    console.warn('migrateProviderNameEnumToVarchar:', err.message);
  }
}

/**
 * One-time: drop bots columns that were removed from the model (golden_tree_url,
 * ichancy_site_url, cookie_refresh_interval_minutes). These are now hardcoded defaults.
 */
async function dropRemovedBotColumns() {
  const removed = ['golden_tree_url', 'ichancy_site_url', 'cookie_refresh_interval_minutes'];
  try {
    const [cols] = await sequelize.query(
      `SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'bots' AND COLUMN_NAME IN (${removed.map(() => '?').join(',')})`,
      { replacements: [database, ...removed] }
    );
    for (const { COLUMN_NAME } of (cols || [])) {
      await sequelize.query(`ALTER TABLE bots DROP COLUMN \`${COLUMN_NAME}\``).catch(() => {});
      console.log(`Dropped obsolete bots column: ${COLUMN_NAME}`);
    }
  } catch (err) {
    console.warn('dropRemovedBotColumns:', err.message);
  }
}
...
async function migrateFromBotConfigTable() {
  try {
    ...
    for (const bid of botIds) {
      ...
      const intFields = ['deposit_required_ls', 'active_referrals_required'];
...
function getMidnightInTimezone(year, month, day, timeZone) {
  const tz = (timeZone || '').trim() || 'Asia/Damascus';
  // Asia/Damascus is UTC+3 year-round; use direct formula for reliability
  if (tz === 'Asia/Damascus') {
    return new Date(Date.UTC(year, month - 1, day, 0, 0, 0) - 3 * 60 * 60 * 1000);
  }
  const pad = (n) => String(n).padStart(2, '0');
  const targetMonth = pad(month);
  const targetDay = pad(day);
  ...
  for (let t = start; t <= end; t += 900000) {
    ...
    const y = get('year');
    const m = get('month');
    const dayVal = get('day');
    const h = get('hour');
    const min = get('minute');
    if (y === String(year) && m === targetMonth && dayVal === targetDay && h === '00' && min === '00') return d;
  }
  return new Date(Date.UTC(year, month - 1, day, 0, 0, 0));
}

/**
 * Format a Date as 'YYYY-MM-DD HH:mm:ss' in UTC for MySQL.
 * Ensures correct comparison regardless of Node.js process timezone.
 */
function dateToUtcStringForMysql(d) {
...
function createBotDb(bid) {
  ...
  async function findUserForTransfer(identifier) {
    if (!User) throw new Error('DB not initialized');
    const trimmed = String(identifier).trim().replace(/^@/, '');
    if (!trimmed) return null;
    if (/^\d+$/.test(trimmed)) {
      const row = await User.findOne({ where: { bot_id: bid, telegram_user_id: trimmed } });
      return toPlainSnake(row);
    }
    // Search by telegram_username OR ichancy_login (case-insensitive)
    const lowerVal = trimmed.toLowerCase();
    const row = await User.findOne({
      where: {
        bot_id: bid,
        [Op.or]: [
          sequelize.where(sequelize.fn('LOWER', sequelize.col('telegram_username')), lowerVal),
          sequelize.where(sequelize.fn('LOWER', sequelize.col('ichancy_login')), lowerVal),
        ],
      },
    });
    return toPlainSnake(row);
  ...
  /** Sum of confirmed real-money deposits in the last 24 hours for a user (for daily spin eligibility). Excludes spin_wheel, box_game, balance_to_site, site_to_balance. */
  async function getDepositSumLast24h(telegramUserId) {
    if (!Transaction) return 0;
    const since = new Date(Date.now() - 24 * 60 * 60 * 1000);
    const sinceUtc = dateToUtcStringForMysql(since);
    if (!sinceUtc) return 0;
    const sum = await Transaction.sum('amount', {
      where: {
        ...
        type: 'deposit',
        status: 'confirmed',
        method: { [Op.in]: ['syriatel', 'sham_syp', 'sham_usd', 'oxapay'] },
        created_at: { [Op.gte]: sinceUtc },
      },
    });
    return Number(sum) || 0;
  }

  /** Count of direct referrals who had at least one confirmed real-money deposit in the last 24 hours. Excludes spin_wheel, box_game, etc. */
  async function getActiveReferralsCountLast24h(telegramUserId) {
    ...
    const [rows] = await sequelize.query(
      `SELECT COUNT(DISTINCT t.telegram_user_id) AS cnt
       ...
       WHERE t.bot_id = ? AND t.type = 'deposit' AND t.status = 'confirmed' AND t.method IN ('syriatel', 'sham_syp', 'sham_usd', 'oxapay') AND t.created_at >= ?`,
      { replacements: [telegramUserId, bid, sinceUtc] }
    );
    return Number(rows?.[0]?.cnt) || 0;
  }

  /**
   * Grant daily spins based on eligibility:
   * - 1 spin if EITHER deposit >= deposit_required_ls OR active referrals >= active_referrals_required (last 24h)
   * - 2 spins if BOTH conditions are met
   * Uses bot timezone for "today". Resets wheel_spins_available_today on a new day.
   * Uses spins_granted_today to track how many spins were already issued today, allowing
   * a user who initially met one condition to receive a second spin later in the same day.
   */
  async function ensureDailySpinEligibility(telegramUserId) {
    ...
    const [depositSum, activeRefs, userRow] = await Promise.all([
      ...
      User.findOne({ where: { bot_id: bid, telegram_user_id: telegramUserId }, attributes: ['id', 'wheel_spins_available_today', 'spins_granted_today', 'last_spin_grant_date'] }),
    ]);
    if (!userRow) return;
    ...
    // New day: reset both spin counters
    if (lastGrant && lastGrant < todayStr) {
      await userRow.update({ wheel_spins_available_today: 0, spins_granted_today: 0, last_spin_grant_date: null });
      plain.wheel_spins_available_today = 0;
      plain.spins_granted_today = 0;
    }
    const depositEligible = depositSum >= depositRequired;
    const refsEligible = activeRefs >= referralsRequired;
    const spinTarget = (depositEligible ? 1 : 0) + (refsEligible ? 1 : 0);
    if (spinTarget === 0) return;
    const alreadyGranted = Number(plain.spins_granted_today ?? 0);
    const toGrant = spinTarget - alreadyGranted;
    if (toGrant <= 0) return;
    await User.update(
      {
        wheel_spins_available_today: sequelize.literal(`wheel_spins_available_today + ${toGrant}`),
        spins_granted_today: spinTarget,
        last_spin_grant_date: todayStr,
      },
  ...
  async function redeemGiftCode(code, telegramUserId) {
    ...
    try {
      ...
      if (giftCode.publish_batch_id != null) {
        const [[batchCheck]] = await sequelize.query(
          `SELECT 1 AS found FROM gift_code_redemptions r
           JOIN gift_codes g ON r.gift_code_id = g.id
           WHERE g.publish_batch_id = ? AND r.telegram_user_id = ? AND g.bot_id = ?
           LIMIT 1`,
          { replacements: [giftCode.publish_batch_id, telegramUserId, bid], transaction: t }
        );
        if (batchCheck) { await t.rollback(); return { error: 'batch_used' }; }
      }
  ...
  async function deleteExpiredGiftCodes() {
    ...
    const nowUtc = dateToUtcStringForMysql(new Date());
    if (!nowUtc) return 0;
    return GiftCode.destroy({ where: { bot_id: bid, expiry_date: { [Op.lt]: nowUtc } } });
  }
  ...
  async function listGiftCodes(opts = {}) {
    ...
    const codes = await GiftCode.findAll({ where, order: [['id', 'DESC']], attributes: ['id', 'code', 'amount', 'expiry_date', 'max_redemptions', 'is_active', 'published', 'publish_batch_id'] });
    const result = [];
  ...
  async function markGiftCodesPublished(ids) {
    ...
    const [[result]] = await sequelize.query(
      'SELECT COALESCE(MAX(publish_batch_id), 0) + 1 AS next_batch FROM gift_codes WHERE bot_id = ?',
      { replacements: [bid] }
    );
    const batchId = result.next_batch;
    await GiftCode.update(
      { published: true, publish_batch_id: batchId },
      { where: { id: { [Op.in]: ids }, bot_id: bid } }
    );
  }
  ...
  async function logTransaction(opts) {
    ...
    const row = await Transaction.create({
      ...
      bonus_amount: opts.bonusAmount || 0,
      tax_amount: opts.taxAmount || 0,
    });
    return row.get ? row.get({ plain: true }) : row;
  }
  ...
  async function updateShamcashChannelMessageId(id, channelMessageId) {
    if (!ShamcashPendingWithdrawal) throw new Error('DB not initialized');
    const row = await ShamcashPendingWithdrawal.findOne({ where: { id, bot_id: bid } });
    if (!row) return false;
    await row.update({ channel_message_id: channelMessageId });
    return true;
  }
  ...
  // ── Syriatel Pending Withdrawal tracking ────────────────────────────────

  async function createSyriatelPendingWithdrawal(opts) {
    if (!SyriatelPendingWithdrawal) throw new Error('DB not initialized');
    const row = await SyriatelPendingWithdrawal.create({
      bot_id: bid,
      telegram_user_id: opts.telegramUserId,
      amount_syp: opts.amountSyp,
      amount_new_currency: opts.amountNewCurrency,
  ...
  async function getSyriatelPendingById(id) {
    if (!SyriatelPendingWithdrawal) throw new Error('DB not initialized');
    const row = await SyriatelPendingWithdrawal.findOne({ where: { id, bot_id: bid } });
    return row ? (row.get ? row.get({ plain: true }) : row) : null;
  }

  async function getSyriatelPendingByUser(telegramUserId) {
    if (!SyriatelPendingWithdrawal) throw new Error('DB not initialized');
    const rows = await SyriatelPendingWithdrawal.findAll({
      where: { bot_id: bid, telegram_user_id: telegramUserId, status: ['pending', 'partial'] },
      order: [['created_at', 'DESC']],
    });
    return rows.map(r => r.get ? r.get({ plain: true }) : r);
  }

  async function updateSyriatelPendingStatus(id, status, resolvedBy) {
    if (!SyriatelPendingWithdrawal) throw new Error('DB not initialized');
    const row = await SyriatelPendingWithdrawal.findOne({ where: { id, bot_id: bid } });
    if (!row) return false;
    const updates = { status };
    if (resolvedBy) { updates.resolved_at = new Date(); updates.resolved_by = resolvedBy; }
    await row.update(updates);
    return true;
  }

  async function updateSyriatelPendingAmountSent(id, amountSent) {
    if (!SyriatelPendingWithdrawal) throw new Error('DB not initialized');
    const [n] = await SyriatelPendingWithdrawal.update(
  ...
  async function updateSyriatelPendingFailureReason(id, failureReason) {
    if (!SyriatelPendingWithdrawal) throw new Error('DB not initialized');
    const [n] = await SyriatelPendingWithdrawal.update(
  ...
  async function updateSyriatelPendingAdminMessage(id, adminMessageId, adminChatId) {
    if (!SyriatelPendingWithdrawal) throw new Error('DB not initialized');
    const [n] = await SyriatelPendingWithdrawal.update(
      { admin_message_id: String(adminMessageId), admin_chat_id: String(adminChatId) },
      { where: { id, bot_id: bid } }
    );
    return n > 0;
  }

  // ── OxaPay Invoice tracking ──────────────────────────────────────────────

  async function createOxapayPendingInvoice(opts) {
    if (!OxapayPendingInvoice) throw new Error('DB not initialized');
    const row = await OxapayPendingInvoice.create({
      bot_id: bid,
      track_id: String(opts.trackId),
      telegram_user_id: opts.telegramUserId,
      coin: String(opts.coin),
      coin_display: String(opts.coinDisplay || opts.coin),
      amount_usd: opts.amountUsd,
      amount_coin: opts.amountCoin,
      status: 'pending',
      polling_count: 0,
    });
    return row.get ? row.get({ plain: true }) : row;
  ...
  async function getOxapayPendingInvoiceByTrackId(trackId) {
    if (!OxapayPendingInvoice) throw new Error('DB not initialized');
    const row = await OxapayPendingInvoice.findOne({ where: { bot_id: bid, track_id: String(trackId) } });
    return row ? (row.get ? row.get({ plain: true }) : row) : null;
  }

  async function getOxapayPendingInvoicesByUser(telegramUserId) {
    if (!OxapayPendingInvoice) throw new Error('DB not initialized');
    const rows = await OxapayPendingInvoice.findAll({
      where: { bot_id: bid, telegram_user_id: telegramUserId, status: 'pending' },
      order: [['created_at', 'DESC']],
    });
    return rows.map(r => r.get ? r.get({ plain: true }) : r);
  ...
  /**
   * Atomically mark invoice as paid. Returns true if this call claimed it (first time),
   * false if it was already processed (double-credit prevention).
   */
  async function claimOxapayInvoicePaid(trackId, amountSypCredited) {
    if (!OxapayPendingInvoice) throw new Error('DB not initialized');
    const [affectedCount] = await OxapayPendingInvoice.update(
      { status: 'paid', amount_syp_credited: amountSypCredited },
      { where: { bot_id: bid, track_id: String(trackId), status: 'pending' } }
    );
    return affectedCount > 0;
  }

  async function updateOxapayPendingInvoiceStatus(trackId, status) {
    if (!OxapayPendingInvoice) throw new Error('DB not initialized');
    await OxapayPendingInvoice.update(
      { status },
      { where: { bot_id: bid, track_id: String(trackId) } }
    );
  }

  async function incrementOxapayPollCount(trackId) {
    if (!OxapayPendingInvoice) throw new Error('DB not initialized');
    await OxapayPendingInvoice.increment('polling_count', { where: { bot_id: bid, track_id: String(trackId) } });
  }

  // ── OxaPay Pending Withdrawals ────────────────────────────────────────────

  async function createOxapayPendingWithdrawal(opts) {
    if (!OxapayPendingWithdrawal) throw new Error('DB not initialized');
    const row = await OxapayPendingWithdrawal.create({
      bot_id: bid,
      telegram_user_id: opts.telegramUserId,
      amount_syp: opts.amountSyp,
      amount_usd: opts.amountUsd,
      coin: String(opts.coin),
      coin_display: String(opts.coinDisplay || opts.coin),
      wallet_address: String(opts.walletAddress || '').trim(),
      transaction_id: opts.transactionId || null,
      status: 'pending',
    });
    return row.get ? row.get({ plain: true }) : row;
  ...
  async function getOxapayPendingWithdrawalById(id) {
    if (!OxapayPendingWithdrawal) throw new Error('DB not initialized');
    const row = await OxapayPendingWithdrawal.findOne({ where: { id, bot_id: bid } });
    return row ? (row.get ? row.get({ plain: true }) : row) : null;
  }

  async function getOxapayPendingWithdrawalsByUser(telegramUserId) {
    if (!OxapayPendingWithdrawal) throw new Error('DB not initialized');
    const rows = await OxapayPendingWithdrawal.findAll({
      where: { bot_id: bid, telegram_user_id: telegramUserId, status: 'pending' },
      order: [['created_at', 'DESC']],
    });
    return rows.map(r => r.get ? r.get({ plain: true }) : r);
  ...
  async function getAllOxapayPendingWithdrawals() {
    if (!OxapayPendingWithdrawal) throw new Error('DB not initialized');
    const rows = await OxapayPendingWithdrawal.findAll({
      where: { bot_id: bid, status: 'pending' },
      order: [['created_at', 'ASC']],
  ...
  async function updateOxapayWithdrawalStatus(id, status, resolvedBy) {
    if (!OxapayPendingWithdrawal) throw new Error('DB not initialized');
    const row = await OxapayPendingWithdrawal.findOne({ where: { id, bot_id: bid } });
    if (!row) return false;
    await row.update({ status, resolved_at: new Date(), resolved_by: resolvedBy || null });
    return true;
  }
  ...
  async function cleanupSyriatelUsedTransactionsOlderThan(days) {
    ...
    const cutoffUtc = dateToUtcStringForMysql(cutoff);
    if (!cutoffUtc) return 0;
    const deleted = await SyriatelUsedTransaction.destroy({
      where: { bot_id: bid, used_at: { [Op.lt]: cutoffUtc } },
    });
    return deleted || 0;
  }
  ...
  async function getTotalDepositsForUser(telegramUserId) {
    if (!Transaction) throw new Error('DB not initialized');
    const sum = await Transaction.sum('amount', {
      where: { bot_id: bid, telegram_user_id: telegramUserId, type: 'deposit', status: 'confirmed', method: { [Op.in]: ['syriatel', 'sham_syp', 'sham_usd'] } },
    });
    return Number(sum) || 0;
  }
  ...
  async function getWithdrawalCountForUser(telegramUserId) {
    ...
    return Transaction.count({
      where: { bot_id: bid, telegram_user_id: telegramUserId, type: 'withdrawal', status: 'confirmed', method: { [Op.in]: ['syriatel', 'sham_syp', 'sham_usd'] } },
    });
  }

  async function getSyriatelDepositCountInLastN(telegramUserId, n = 3) {
    if (!Transaction) throw new Error('DB not initialized');
    const lastDeposits = await Transaction.findAll({
      where: { bot_id: bid, telegram_user_id: telegramUserId, type: 'deposit', status: 'confirmed' },
      order: [['created_at', 'DESC']],
      limit: n,
      attributes: ['method'],
    });
    return lastDeposits.filter(r => (r.get ? r.get('method') : r.method) === 'syriatel').length;
  }

  async function getUserExtendedStats(telegramUserId) {
    if (!Transaction) throw new Error('DB not initialized');
    const [rows] = await sequelize.query(
      `SELECT
        COALESCE(SUM(CASE WHEN type = 'deposit' AND status = 'confirmed' AND method = 'spin_wheel' THEN amount ELSE 0 END), 0) AS spin_profits,
        COALESCE(SUM(CASE WHEN type = 'deposit' AND status = 'confirmed' AND method = 'box_game' THEN amount ELSE 0 END), 0) AS box_profits,
        COALESCE(SUM(CASE WHEN type = 'deposit' AND status = 'confirmed' THEN bonus_amount ELSE 0 END), 0) AS total_bonuses,
        COALESCE(SUM(CASE WHEN type = 'withdrawal' AND status = 'confirmed' AND method IN ('syriatel', 'sham_syp', 'sham_usd') THEN amount ELSE 0 END), 0) AS total_withdrawals,
        COALESCE(SUM(CASE WHEN type = 'withdrawal' AND status = 'confirmed' AND method IN ('syriatel', 'sham_syp', 'sham_usd') THEN tax_amount ELSE 0 END), 0) AS total_taxes,
        COALESCE(SUM(CASE WHEN type = 'deposit' AND status = 'confirmed' AND method = 'admin' THEN amount ELSE 0 END), 0) AS admin_deposits,
        COALESCE(SUM(CASE WHEN type = 'withdrawal' AND status = 'confirmed' AND method = 'admin' THEN amount ELSE 0 END), 0) AS admin_withdrawals,
        COALESCE(SUM(CASE WHEN type = 'withdrawal' AND status = 'confirmed' AND method = 'send_balance' THEN amount ELSE 0 END), 0) AS send_balance_sent,
        COALESCE(SUM(CASE WHEN type = 'deposit' AND status = 'confirmed' AND method = 'send_balance' THEN amount ELSE 0 END), 0) AS send_balance_received
      FROM transactions WHERE bot_id = ? AND telegram_user_id = ?`,
      { replacements: [bid, telegramUserId] }
    );
    const r = rows?.[0] ?? {};
    return {
      spinProfits: Number(r.spin_profits ?? 0),
      boxProfits: Number(r.box_profits ?? 0),
      totalBonuses: Number(r.total_bonuses ?? 0),
      totalWithdrawals: Number(r.total_withdrawals ?? 0),
      totalTaxes: Number(r.total_taxes ?? 0),
      adminDeposits: Number(r.admin_deposits ?? 0),
      adminWithdrawals: Number(r.admin_withdrawals ?? 0),
      sendBalanceSent: Number(r.send_balance_sent ?? 0),
      sendBalanceReceived: Number(r.send_balance_received ?? 0),
    };
  }
  ...
  async function getAdminStats(opts = {}) {
    ...
    const activeCutoffUtc = dateToUtcStringForMysql(activeCutoff);

    const [usersTotal, usersActive, usersDeleted, totalUserBalances] = await Promise.all([
      User.count({ where: bw }),
      User.count({ where: { ...bw, ...(activeCutoffUtc && { updated_at: { [Op.gte]: activeCutoffUtc } }) } }),
      DeletedUser.count({ where: bw }),
      User.sum('balance', { where: bw }),
    ]);

    const realPaymentMethods = ['syriatel', 'sham_syp', 'sham_usd', 'oxapay'];
    const realWithdrawalMethods = ['syriatel', 'sham_syp', 'sham_usd', 'oxapay'];
    const methodPlaceholders = realPaymentMethods.map(() => '?').join(',');
    const wMethodPlaceholders = realWithdrawalMethods.map(() => '?').join(',');
    const monthDepositWhere = monthEnd
      ? `WHERE bot_id = ? AND type = 'deposit' AND status = 'confirmed' AND method IN (${methodPlaceholders}) AND created_at >= ? AND created_at <= ?`
      : `WHERE bot_id = ? AND type = 'deposit' AND status = 'confirmed' AND method IN (${methodPlaceholders}) AND created_at >= ?`;
    const monthDepositReplacements = monthEnd
      ? [bid, ...realPaymentMethods, dateToUtcStringForMysql(monthStart), dateToUtcStringForMysql(monthEnd)]
      : [bid, ...realPaymentMethods, dateToUtcStringForMysql(monthStart)];

    const [monthlyDepositSum] = await sequelize.query(
      `SELECT COALESCE(SUM(amount), 0) AS total FROM transactions ${monthDepositWhere}`,
      { replacements: monthDepositReplacements }
    );
    const totalDeposits = Number(monthlyDepositSum?.[0]?.total ?? 0);

    const monthWithdrawWhere = monthEnd
      ? `WHERE bot_id = ? AND type = 'withdrawal' AND status = 'confirmed' AND method IN (${wMethodPlaceholders}) AND created_at >= ? AND created_at <= ?`
      : `WHERE bot_id = ? AND type = 'withdrawal' AND status = 'confirmed' AND method IN (${wMethodPlaceholders}) AND created_at >= ?`;
    const monthWithdrawReplacements = monthEnd
      ? [bid, ...realWithdrawalMethods, dateToUtcStringForMysql(monthStart), dateToUtcStringForMysql(monthEnd)]
      : [bid, ...realWithdrawalMethods, dateToUtcStringForMysql(monthStart)];

    const [monthlyWithdrawRows] = await sequelize.query(
      `SELECT method, amount, tax_amount FROM transactions ${monthWithdrawWhere}`,
      { replacements: monthWithdrawReplacements }
    );
    let totalWithdrawals = 0;
    let totalTaxes = 0;
    for (const r of monthlyWithdrawRows || []) {
      totalWithdrawals += Number(r.amount ?? 0);
      totalTaxes += Number(r.tax_amount ?? 0);
    }

    const [pendingRows] = await sequelize.query(
      `SELECT method, amount FROM transactions WHERE bot_id = ? AND type = 'withdrawal' AND status = 'pending' AND method IN (${wMethodPlaceholders})`,
      { replacements: [bid, ...realWithdrawalMethods] }
    );
    let pendingWithdrawalsSum = 0;
    for (const r of pendingRows || []) {
      pendingWithdrawalsSum += Number(r.amount ?? 0);
    }
    ...
    const todayStartUtc = todayStart ? dateToUtcStringForMysql(todayStart) : null;
    const weekStartUtc = weekStart ? dateToUtcStringForMysql(weekStart) : null;
    if (!isPreviousMonth && todayStartUtc && weekStartUtc) {
      const [todayDepositRows] = await sequelize.query(
        `SELECT COALESCE(SUM(amount), 0) AS total FROM transactions WHERE bot_id = ? AND type = 'deposit' AND status = 'confirmed' AND method IN (${methodPlaceholders}) AND created_at >= ?`,
        { replacements: [bid, ...realPaymentMethods, todayStartUtc] }
      );
      const todayDepositRow = todayDepositRows?.[0];
      todayDeposits = Number(todayDepositRow?.total ?? todayDepositRow?.TOTAL ?? 0);
      const [todayWithdrawRows] = await sequelize.query(
        `SELECT method, amount, tax_amount FROM transactions WHERE bot_id = ? AND type = 'withdrawal' AND status = 'confirmed' AND method IN (${wMethodPlaceholders}) AND created_at >= ?`,
        { replacements: [bid, ...realWithdrawalMethods, todayStartUtc] }
      );
      for (const r of todayWithdrawRows || []) {
        todayWithdrawals += Number(r.amount ?? 0);
      }
      const [weekDepositRows] = await sequelize.query(
        `SELECT COALESCE(SUM(amount), 0) AS total FROM transactions WHERE bot_id = ? AND type = 'deposit' AND status = 'confirmed' AND method IN (${methodPlaceholders}) AND created_at >= ?`,
        { replacements: [bid, ...realPaymentMethods, weekStartUtc] }
      );
      const weekDepositRow = weekDepositRows?.[0];
      weekDeposits = Number(weekDepositRow?.total ?? weekDepositRow?.TOTAL ?? 0);
      const [weekWithdrawRows] = await sequelize.query(
        `SELECT method, amount, tax_amount FROM transactions WHERE bot_id = ? AND type = 'withdrawal' AND status = 'confirmed' AND method IN (${wMethodPlaceholders}) AND created_at >= ?`,
        { replacements: [bid, ...realWithdrawalMethods, weekStartUtc] }
      );
      for (const r of weekWithdrawRows || []) {
        weekWithdrawals += Number(r.amount ?? 0);
      }
    }

    const [bonusRows] = await sequelize.query(
      `SELECT COALESCE(SUM(bonus_amount), 0) AS total FROM transactions ${monthDepositWhere}`,
      { replacements: monthDepositReplacements }
    );
    let totalBonuses = Number(bonusRows?.[0]?.total ?? bonusRows?.[0]?.TOTAL ?? 0);

    let wheelProfits = 0;
    let boxProfits = 0;
    try {
      const gameDateReplacements = monthEnd ? [bid, dateToUtcStringForMysql(monthStart), dateToUtcStringForMysql(monthEnd)] : [bid, dateToUtcStringForMysql(monthStart)];
      const gameDateClause = monthEnd
        ? `bot_id = ? AND type = 'deposit' AND status = 'confirmed' AND created_at >= ? AND created_at <= ?`
        : `bot_id = ? AND type = 'deposit' AND status = 'confirmed' AND created_at >= ?`;
      const [gameRows] = await sequelize.query(
        `SELECT method, COALESCE(SUM(amount), 0) AS total FROM transactions WHERE ${gameDateClause} AND method IN ('spin_wheel', 'box_game') GROUP BY method`,
        { replacements: gameDateReplacements }
      );
      for (const r of gameRows || []) {
        if (r.method === 'spin_wheel') wheelProfits = Number(r.total ?? 0);
        if (r.method === 'box_game') boxProfits = Number(r.total ?? 0);
      }
    } catch (_) {}

    const referralDistWhere = monthEnd
      ? { bot_id: bid, distributed_at: { [Op.gte]: dateToUtcStringForMysql(monthStart), [Op.lte]: dateToUtcStringForMysql(monthEnd) } }
      : { bot_id: bid, distributed_at: { [Op.gte]: dateToUtcStringForMysql(monthStart) } };
    ...
    let codeProfits = 0;
    try {
      const codeReplacements = monthEnd ? [bid, dateToUtcStringForMysql(monthStart), dateToUtcStringForMysql(monthEnd)] : [bid, dateToUtcStringForMysql(monthStart)];
  ...
  async function getTopUsersByNetDeposits(opts = {}) {
    ...
    const startStr = startDate ? dateToUtcStringForMysql(startDate) : null;
    const endStr = endDate ? dateToUtcStringForMysql(endDate) : null;
    const [rows] = await sequelize.query(
      `SELECT t.telegram_user_id, u.telegram_username, u.first_name, u.balance AS current_balance,
        COALESCE(SUM(CASE WHEN t.type = 'deposit' AND t.status = 'confirmed' AND t.method IN ('syriatel', 'sham_syp', 'sham_usd') THEN t.amount ELSE 0 END), 0) AS confirmed_deposits,
        COALESCE(SUM(CASE WHEN t.type = 'withdrawal' AND t.status = 'confirmed' AND t.method IN ('syriatel', 'sham_syp', 'sham_usd') THEN t.amount ELSE 0 END), 0) AS confirmed_withdrawals
      ...
      { replacements: [bid, startStr, startStr, endStr, endStr] }
    );
  ...
  async function getProviderConfig(providerName) {
    ...
    const defaults = { min_deposit_syp: 50, min_cashout_syp: 25000, max_cashout_syp: 250000, cashout_tax_percent: 0, deposit_bonus_percent: 0, min_deposit_usd: null, max_cashout_usd: null, min_cashout_usd: null };
    ...
    if (cached) {
      return {
        ...
        min_deposit_usd: cached.min_deposit_usd ?? defaults.min_deposit_usd,
        max_cashout_usd: cached.max_cashout_usd ?? defaults.max_cashout_usd,
        min_cashout_usd: cached.min_cashout_usd ?? defaults.min_cashout_usd,
      };
    }
    ...
    return {
      ...
      min_deposit_usd: p.min_deposit_usd ?? defaults.min_deposit_usd,
      max_cashout_usd: p.max_cashout_usd ?? defaults.max_cashout_usd,
      min_cashout_usd: p.min_cashout_usd ?? defaults.min_cashout_usd,
    };
  }
  ...
  async function setProviderConfig(providerName, fields) {
    ...
    const allowed = ['min_deposit_syp', 'min_cashout_syp', 'max_cashout_syp', 'cashout_tax_percent', 'deposit_bonus_percent', 'min_deposit_usd', 'max_cashout_usd', 'min_cashout_usd'];
    const updates = {};
  ...
  async function seedConfigDefaults(defaults) {
    ...
    if (!created && row.admin_username == null) {
      await row.update({ admin_username: cols.admin_username || '' });
      await row.reload();
    }
    botConfigRow = row.get({ plain: true });
  ...
  async function seedPaymentProviders() {
    ...
    if (count > 0) {
      // Ensure oxapay row exists even if others already seeded
      await PaymentProvider.findOrCreate({
        where: { bot_id: bid, provider_name: 'oxapay' },
        defaults: { bot_id: bid, provider_name: 'oxapay', min_deposit_syp: 50, min_cashout_syp: 25000, max_cashout_syp: 250000, cashout_tax_percent: 0, deposit_bonus_percent: 0 },
      });
      // Backfill shamcash USD columns for existing rows
      const shamRow = await PaymentProvider.findOne({ where: { bot_id: bid, provider_name: 'shamcash' } });
      if (shamRow) {
        const updates = {};
        if (shamRow.min_deposit_usd == null) updates.min_deposit_usd = 1;
        if (shamRow.max_cashout_usd == null) updates.max_cashout_usd = 200;
        if (shamRow.min_cashout_usd == null) updates.min_cashout_usd = 5;
        if (Object.keys(updates).length) await shamRow.update(updates);
      }
      await loadProviderConfigs();
      return;
    }
    await PaymentProvider.bulkCreate([
      ...
      { bot_id: bid, provider_name: 'shamcash', min_deposit_syp: 50, min_cashout_syp: 100000, max_cashout_syp: 2500000, cashout_tax_percent: 0, deposit_bonus_percent: 0, min_deposit_usd: 1, max_cashout_usd: 200, min_cashout_usd: 5 },
      { bot_id: bid, provider_name: 'oxapay', min_deposit_syp: 50, min_cashout_syp: 25000, max_cashout_syp: 250000, cashout_tax_percent: 0, deposit_bonus_percent: 0 },
    ]);
    await loadProviderConfigs();
  }
  ...
  async function getTransactionsForExport(startDate, endDate) {
    ...
    const startUtc = startDate ? dateToUtcStringForMysql(startDate) : null;
    const endUtc = endDate ? dateToUtcStringForMysql(endDate) : null;
    if (startUtc) {
      where.push('t.created_at >= ?');
      replacements.push(startUtc);
    }
    if (endUtc) {
      where.push('t.created_at <= ?');
      replacements.push(endUtc);
    }
    const [rows] = await sequelize.query(
    ...
    const methodMap = { syriatel: 'SYRIATEL_CASH', sham_syp: 'sham_cash', sham_usd: 'sham_cash', oxapay: 'OxaPay', spin_wheel: 'عجلة الحظ', box_game: 'لعبة الصناديق' };
  ...
  return {
    getUserByTelegramId, findUserForTransfer, createOrUpdateUser, moveUserToDeletedUsers,
