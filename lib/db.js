...
function createBotDb(bid) {
  ...
  async function getAdminStats(opts = {}) {
    ...
    const referralWhere = monthEnd
      ? { ...bw, created_at: { [Op.gte]: monthStart, [Op.lte]: monthEnd } }
      : { ...bw, created_at: { [Op.gte]: monthStart } };
    const referralSum = await ReferralEarning.sum('commission', { where: referralWhere });
