...
function createBotDb(bid) {
   ...
   * Uses bot timezone for "today". Caps at 1 spin per day.
  ...
  async function ensureDailySpinEligibility(telegramUserId) {
    ...
    if (!eligible) return;
    if (lastGrant && lastGrant >= todayStr) return;
    ...
    await User.update(
      { wheel_spins_available_today: 1, last_spin_grant_date: todayStr },
  ...
  async function getAdminStats(opts = {}) {
    ...
    const [depositByMethod] = await sequelize.query(
      `SELECT method, COALESCE(SUM(amount), 0) AS total FROM transactions ${monthDepositWhere} GROUP BY method`,
      { replacements: monthDepositReplacements }
    );
