...
function createBotDb(bid) {
  ...
  async function getAdminStats(opts = {}) {
    ...
    const monthDepositReplacements = monthEnd
      ? [bid, ...realPaymentMethods, monthStart, monthEnd]
      : [bid, ...realPaymentMethods, monthStart];
    ...
    const monthWithdrawReplacements = monthEnd
      ? [bid, ...realWithdrawalMethods, monthStart, monthEnd]
      : [bid, ...realWithdrawalMethods, monthStart];
    ...
    if (!isPreviousMonth && todayStart && weekStart) {
      const [todayDepositSum] = await sequelize.query(
        ...
        { replacements: [bid, ...realPaymentMethods, todayStart] }
      );
      ...
      const [todayWithdrawRows] = await sequelize.query(
        ...
        { replacements: [bid, ...realWithdrawalMethods, todayStart] }
      );
      ...
      const [weekDepositSum] = await sequelize.query(
        `SELECT COALESCE(SUM(amount), 0) AS total FROM transactions WHERE bot_id = ? AND type = 'deposit' AND status = 'confirmed' AND method IN (${methodPlaceholders}) AND created_at >= ?`,
        { replacements: [bid, ...realPaymentMethods, weekStart] }
      );
      weekDeposits = Number(weekDepositSum?.[0]?.total ?? 0);
      const [weekWithdrawRows] = await sequelize.query(
        `SELECT method, amount FROM transactions WHERE bot_id = ? AND type = 'withdrawal' AND status = 'confirmed' AND method IN (${wMethodPlaceholders}) AND created_at >= ?`,
        { replacements: [bid, ...realWithdrawalMethods, weekStart] }
      );
    ...
    let boxProfits = 0;
    try {
      const gameDateReplacements = monthEnd ? [bid, monthStart, monthEnd] : [bid, monthStart];
