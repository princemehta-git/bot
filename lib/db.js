 ...
 * @returns {{ monthStart: Date, todayStart: Date }}
 */
...
function createBotDb(bid) {
  ...
  async function getAdminStats(opts = {}) {
    if (!User || !DeletedUser || !Transaction || !ReferralEarning || !GiftCode || !GiftCodeRedemption || !PaymentProvider) throw new Error('DB not initialized');
    const now = new Date();
    const currencyMultiple = Math.max(1, Number(opts.currencyMultiple) || 100);
    const bw = { bot_id: bid };

    // Use bot timezone from DB for date boundaries (month, today)
    const timeZone = (await getConfigValue('timezone', 'Asia/Damascus')) || 'Asia/Damascus';
    const { monthStart, todayStart } = getStatsDateBoundsInTimezone(now, timeZone);
    ...
    // Monthly: total deposits (after multiplier = wallet credit) — confirmed only
    const [monthlyDepositSum] = await sequelize.query(
      ...
      { replacements: [bid, monthStart] }
    );
    const totalDeposits = Number(monthlyDepositSum?.[0]?.total ?? 0);

    // Monthly: total withdrawals (before divide: syriatel amount/currencyMultiple, sham as-is) — confirmed only
    ...
    // Pending withdrawals sum (all pending; same conversion)
    ...
    // Today / week: deposits (confirmed) and withdrawals (confirmed, with conversion)
    const [todayDepositSum] = await sequelize.query(
      `SELECT COALESCE(SUM(amount), 0) AS total FROM transactions WHERE bot_id = ? AND type = 'deposit' AND status = 'confirmed' AND created_at >= ?`,
      { replacements: [bid, todayStart] }
    );
    const todayDeposits = Number(todayDepositSum?.[0]?.total ?? 0);
    ...
    const weekDeposits = Number(weekDepositSum?.[0]?.total ?? 0);
    ...
    // Monthly: total bonus from deposits (bonus = amount * (p/(100+p)) per method)
    const [depositByMethod] = await sequelize.query(
      `SELECT method, COALESCE(SUM(amount), 0) AS total FROM transactions WHERE bot_id = ? AND type = 'deposit' AND status = 'confirmed' AND created_at >= ? GROUP BY method`,
      { replacements: [bid, monthStart] }
    );
    ...
    // Monthly: referral profits
    const referralSum = await ReferralEarning.sum('commission', {
      where: { ...bw, created_at: { [Op.gte]: monthStart } },
    });
    const referralProfits = Number(referralSum ?? 0);

    // Monthly: code/coupon profits (gift code redemptions)
    let codeProfits = 0;
    try {
