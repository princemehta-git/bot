...
function createBotDb(bid) {
  ...
  async function getDepositSumLast24h(telegramUserId) {
    ...
    const sum = await Transaction.sum('amount', {
      where: {
        ...
        type: 'deposit',
        status: 'confirmed',
        method: { [Op.in]: ['syriatel', 'sham_syp', 'sham_usd'] },
  ...
  async function getActiveReferralsCountLast24h(telegramUserId) {
    ...
    const [rows] = await sequelize.query(
      `SELECT COUNT(DISTINCT t.telegram_user_id) AS cnt
       ...
       WHERE t.bot_id = ? AND t.type = 'deposit' AND t.status = 'confirmed' AND t.method IN ('syriatel', 'sham_syp', 'sham_usd') AND t.created_at >= ?`,
  ...
  async function getAdminStats(opts = {}) {
    ...
    const realPaymentMethods = ['syriatel', 'sham_syp', 'sham_usd'];
    const realWithdrawalMethods = ['syriatel', 'sham_syp', 'sham_usd'];
  ...
  async function getTransactionsForExport(startDate, endDate) {
    ...
    const methodMap = { syriatel: 'SYRIATEL_CASH', sham_syp: 'sham_cash', sham_usd: 'sham_cash', spin_wheel: 'عجلة الحظ', box_game: 'لعبة الصناديق' };
