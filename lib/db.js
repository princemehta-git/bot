...
async function initDb() {
  ...
  User = sequelize.define('User', {
    ...
    last_spin_grant_date: { type: DataTypes.DATE, allowNull: true },
...
function createBotDb(bid) {
  ...
  /**
   * Grant 1 spin if EITHER deposit >= deposit_required_ls OR active referrals >= active_referrals_required (last 24h).
   * Max 1 spin per 24-hour window. Uses a rolling 24h cooldown from last_spin_grant_date.
   * No new spins can be granted (even with additional deposits) until 24h have passed.
   */
  async function ensureDailySpinEligibility(telegramUserId) {
    if (!User) return;
    const userRow = await User.findOne({ where: { bot_id: bid, telegram_user_id: telegramUserId }, attributes: ['id', 'wheel_spins_available_today', 'last_spin_grant_date'] });
    if (!userRow) return;
    const plain = userRow.get ? userRow.get({ plain: true }) : userRow;
    const now = new Date();
    const lastGrant = plain.last_spin_grant_date ? new Date(plain.last_spin_grant_date) : null;
    // If a spin was granted within the last 24h, do nothing — no new spins regardless of deposits
    if (lastGrant && (now.getTime() - lastGrant.getTime()) < 24 * 60 * 60 * 1000) {
      return;
    }
    // 24h have passed (or first time): reset and check eligibility
    if (lastGrant) {
    ...
    const [depositSum, activeRefs] = await Promise.all([
    ...
    await User.update(
      {
        wheel_spins_available_today: 1,
        last_spin_grant_date: now,
      },
  ...
  async function redeemGiftCode(code, telegramUserId) {
    ...
    try {
      ...
      const newGifts = Number(existing?.gifts ?? 0) + amount;
      if (existing) {
        await existing.update({ balance: newBalance, gifts: newGifts }, { transaction: t });
      } else {
        await User.create({ bot_id: bid, telegram_user_id: telegramUserId, balance: newBalance, gifts: amount }, { transaction: t });
      }
      await t.commit();
      return { amount };
    } catch (err) {
  ...
  async function distributeAllReferralCommissions(levelPercents) {
    ...
    try {
      ...
      for (const u of users) {
        ...
        // Log referral commission to transaction history
